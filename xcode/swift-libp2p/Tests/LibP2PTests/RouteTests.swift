...
extension LibP2PTests {
    ...
    struct LibP2PRouteTests {
        ...
        @Test func testLibP2PTestPing() async throws {
            ...
            try await withApp(configure: configure) { app in
                let addr = try Multiaddr("/ip4/127.0.0.1/tcp/10000").encapsulate(
                    proto: .p2p,
                    address: app.peerID.b58String
                )
                ...
                try await app.testing().test(
                    ...
                    #expect(
                        response.payload.getBytes(
                            at: response.payload.readerIndex,
                            length: response.payload.readableBytes
                        )
                            == payload
                    )
                    print(response)
