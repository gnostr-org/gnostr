//
//  LegacyEncryptedDirectMessageEventTests.swift
//
//
...
final class LegacyEncryptedDirectMessageEventTests: XCTestCase, EventCreating, EventVerifying, FixtureLoading {

    func testCreateLegacyEncryptedDirectMessageEvent() throws {
        let content = "Secret message."
        let recipientPubKey = Keypair.test.publicKey
        let recipientTag = Tag.pubkey(recipientPubKey.hex)

        let event = try legacyEncryptedDirectMessage(withContent: content, toRecipient: recipientPubKey, signedBy: Keypair.test)
    ...
    func testDecodeLegacyEncryptedDirectMessage() throws {
        let event: LegacyEncryptedDirectMessageEvent = try decodeFixture(filename: "legacy_encrypted_direct_message")
        ...
        XCTAssertEqual(event.kind, .legacyEncryptedDirectMessage)
