...
internal final class Negotiator {
    ...
    public func initialize(compact: Bool = false) -> Bytes? {
        precondition(
            self.state == .initialized,
            "MSS: Initialized called on a MSS.Negotiater object with invalid state: \(self.state)"
        )
    ...
    public func consumeMessage(_ bytes: Bytes) -> Response {
        ...
        guard self.state == .initialized || self.state == .speaksMSS else {
            return errorState(.negotationFailed, leftover: bytes)
        }
        ...
        case .initiator:
            for message in mssMessages {
                ...
                default:
                    guard case .speaksMSS = self.state else {
                        self.logger.warning(
                            "Unknown message 1: '\(String(data: Data(message), encoding: .utf8) ?? "")'"
                        )
                        return errorState(Errors.unknownMSSMessage)
                    }
                    // Ensure that we've sent at least one protocol
                    guard protocolsTried > 0 else {
                        self.logger.warning(
                            "Unknown message 2: '\(String(data: Data(message), encoding: .utf8) ?? "")'"
                        )
                        return errorState(Errors.unknownMSSMessage)
                    }
                    ...
                    if handledProtocols[protocolsTried - 1] == message {
                        ...
                        return Response.negotiated(
                            proto: String(data: Data(handledProtocols[protocolsTried - 1]), encoding: .utf8)!,
                            response: nil,
                            leftover: leftover
                        )
                    } else {
                        self.logger.warning(
                            "Unknown message 3: '\(String(data: Data(message), encoding: .utf8) ?? "")'"
                        )
            ...
            for message in mssMessages {
                ...
                default:
                    ...
                    if let match = handledProtocols.first(where: { message == $0 }) {
                        ...
                        return Response.negotiated(
                            proto: String(data: Data(match), encoding: .utf8)!,
                            response: res,
                            leftover: leftover
                        )
        ...
        guard !returnMessages.isEmpty else {
            self.logger.error("Reached end of switch without generating any return messages")
            return errorState(.failedToParseMSSMessage)
        }
        let res = returnMessages.count == 1 ? encode(returnMessages.first!) : encodeList(returnMessages)
        ...
        self.logger.trace(
            "\(returnMessages.map { String(data: Data($0), encoding: .utf8) ?? "NIL" }.joined(separator: ","))"
        )
        self.logger.trace("-----------------------------------")
    ...
    private func decode(_ bytes: Bytes) -> (message: Bytes?, leftover: Bytes)? {
        ...
        //print(r.bytesRead)
        //print(r.value)
        guard bytes[ei] == self.newLine else {
    ...
    private func encode(_ bytes: Bytes) -> Bytes {
        //print("encode")
        putUVarInt(UInt64(bytes.count + 1))  // uVarInt Length Prefix
            + bytes  // mssMessage
            + [self.newLine]  // NewLine Char '\n'
        // NewLine Char '\n'
    }
