...
pub enum ServerProtocol {
	...
	UnauthHttps, /* used for read to enable non-interactive
	              * failures over https */
	UnauthHttp, /* used for read to enable non-interactive
	             * failures over https */
}
	...
	fn from_str(s: &str) -> Result<ServerProtocol> {
		match s {
			...
			"http (unauthenticated)" => {
				Ok(ServerProtocol::UnauthHttp)
			}
			"https (unauthenticated)" => {
				Ok(ServerProtocol::UnauthHttps)
			}
...
impl std::str::FromStr for NostrUrlDecoded {
	...
	fn from_str(url: &str) -> Result<Self> {
		...
		for (name, value) in Url::parse(url)?.query_pairs() {
			if name.contains("relay") {
				let mut decoded = urlencoding::decode(&value)
					.context(
						"could not parse relays in nostr git url",
					)?
					.to_string();
				if !decoded.starts_with("ws://")
					&& !decoded.starts_with("wss://")
				{
					decoded = format!("wss://{decoded}");
				}
				let url = Url::parse(&decoded).context(
					"could not parse relays in nostr git url",
				)?;
		...
		if let Ok(coordinate) = Coordinate::parse(part) {
			if coordinate
				.kind
				.eq(&nostr_sdk::Kind::GitRepoAnnouncement)
			{
				coordinates.insert(coordinate);
			} else {
				bail!(
					"naddr doesnt point to a git repository announcement"
				);
			}
...
impl FromStr for CloneUrl {
	...
	fn from_str(s: &str) -> Result<Self> {
		...
		let host =
			url.host_str().context("Missing host")?.to_string();
...
fn contains_port(s: &str) -> bool {
	if let Some(after_host) =
		s.split('@').nth(1).unwrap_or(s).split(':').nth(1)
	{
	...
	pub fn format_as(
		...
		if *protocol == ServerProtocol::Filesystem {
			if self.protocol == ServerProtocol::Filesystem {
				// If converting from Filesystem to Filesystem, return
				// the original string
				return Ok(self.original_string.clone());
			} else {
				// If converting to Filesystem from any other
				// protocol, return an error
				bail!(
		...
		if *protocol == ServerProtocol::Ssh {
			formatted_url = formatted_url.replace(
				"ssh://",
				format!("{}@", user.as_deref().unwrap_or("git"))
					.as_str(),
			);
			if !contains_port(&formatted_url) {
				formatted_url = replace_first_occurrence(
					&formatted_url,
					'/',
					':',
				);
...
mod tests {
		...
		mod when_user_specified {
			...
			mod but_not_in_original_url {
				...
				fn https_to_ssh_uses_specified_user() {
					...
					assert_eq!(
						result,
						"user1@github.com:user/repo.git"
					);
				}
			}
		...
		mod input_all_formats_to_from_str_and_correctly_format_as_https {
			...
			mod with_unspecified_and_additional_features {
				...
				fn port() {
					...
					assert_eq!(
						result,
						"https://github.com:1000/user/repo.git"
					);
				}
			...
			mod ssh_input {
				...
				fn port_specified_with_path() {
					...
					assert_eq!(
						result,
						"https://github.com:2222/repo.git"
					);
				}
				...
				fn path_with_fragment() {
					let result =
						"user1@github.com:/user/repo.git#readme"
							.parse::<CloneUrl>()
							.unwrap()
							.format_as(&ServerProtocol::Https, &None)
							.unwrap();
				...
				fn path_with_parameters() {
					let result =
						"user@github.com:/user/repo.git?ref=main"
							.parse::<CloneUrl>()
							.unwrap()
							.format_as(&ServerProtocol::Https, &None)
							.unwrap();
	...
	mod convert_clone_url_to_https {
		...
		fn test_http_url_with_credentials() {
			let url =
				"http://username:password@github.com/user/repo.git";
			let result = convert_clone_url_to_https(url).unwrap();
			assert_eq!(result, "https://github.com/user/repo.git");
		}
	...
	mod nostr_git_url_paramemters_from_str {
		...
		mod from_npub_slash_identifier {
			...
			mod with_url_parameters {
				...
				#[test]
				fn with_relay_without_scheme_defaults_to_wss()
				-> Result<()> {
