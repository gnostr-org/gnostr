...
pub async fn launch(
	...
	let most_recent_proposal_patch_chain =
		get_most_recent_patch_with_ancestors(commit_events)
	...
	.context(
		"latest patch event commit tag isn't a valid SHA1 hash",
	)?;
	...
	.context(
		"latest patch event parent-commit tag isn't a valid SHA1 hash",
	)?;
	...
	if args.force {
		...
		sub_commands::send::launch(
			//cli_args,
			//`bunker_app_key`, `bunker_uri`, `disable_cli_spinners` and 2 other fields
