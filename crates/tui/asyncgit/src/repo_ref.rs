...
use crate::{
	...
	ngit::{Repo, RepoActions, nostr_url::NostrUrlDecoded},
};
...
impl TryFrom<nostr::Event> for RepoRef {
	...
	fn try_from(event: nostr::Event) -> Result<Self> {
		...
		if let Some(t) =
			event.tags.iter().find(|t| t.as_vec()[0].eq("d"))
		{
			r.identifier = t.as_vec()[1].clone();
		}

		if let Some(t) =
			event.tags.iter().find(|t| t.as_vec()[0].eq("name"))
		{
		...
		if let Some(t) =
			event.tags.iter().find(|t| t.as_vec()[0].eq("clone"))
		{
		...
		if let Some(t) =
			event.tags.iter().find(|t| t.as_vec()[0].eq("web"))
		{
		...
		if let Some(t) =
			event.tags.iter().find(|t| t.as_vec()[0].eq("relays"))
		{
...
impl RepoRef {
		...
		sign_event(
			nostr_sdk::EventBuilder::new(
				...
				[
					vec![
						Tag::identifier(
							if self.identifier.to_string().is_empty()
							{
								// fiatjaf thought a random string.
								// its not in the draft nip.
								// thread_rng()
								...
								// an identifier based on first commit
								// is better so that users dont
								// accidentally create two seperate
								// identifiers for the same repo
								// there is a hesitancy to use the
								// commit id in another
								// conversaion with fiatjaf he
								// suggested the first 6
								// character of the commit id
								// here we are using 7 which is the
								// standard for shorthand commit id
								self.root_commit.to_string()[..7]
									.to_string()
							} else {
						...
						Tag::custom(
							...
							vec![
								self.root_commit.to_string(),
								"euc".to_string(),
							],
						),
						Tag::from_standardized(TagStandard::Name(
							self.name.clone(),
						)),
						...
						Tag::custom(
							nostr::TagKind::Custom(
								std::borrow::Cow::Borrowed("clone"),
							),
							self.git_server.clone(),
						),
						Tag::custom(
							nostr::TagKind::Custom(
								std::borrow::Cow::Borrowed("web"),
							),
							self.web.clone(),
						),
						Tag::custom(
							nostr::TagKind::Custom(
								std::borrow::Cow::Borrowed("relays"),
							),
							self.relays.clone(),
						),
						Tag::custom(
	...
	/// coordinates without relay hints
	pub fn coordinates_with_timestamps(
		&self,
	) -> Vec<(Coordinate, Option<Timestamp>)> {
		self.coordinates()
			.iter()
			.map(|c| {
				(c.clone(), self.events.get(c).map(|e| e.created_at))
			})
...
pub async fn try_and_get_repo_coordinates(
	...
	let mut repo_coordinates =
		get_repo_coordinates_from_git_config(git_repo)?;

	if repo_coordinates.is_empty() {
		repo_coordinates =
			get_repo_coordinates_from_nostr_remotes(git_repo)?;
	}
	...
	if repo_coordinates.is_empty() {
		if prompt_user {
			...
			bail!(
				"couldn't find repo coordinates in git config nostr.repo or in maintainers.yaml"
			);
		}
	}
	Ok(repo_coordinates)
...
fn get_repo_coordinates_from_git_config(
	git_repo: &Repo,
) -> Result<HashSet<Coordinate>> {
	let mut repo_coordinates = HashSet::new();
	if let Some(repo_override) =
		git_repo.get_git_config_item("nostr.repo", Some(false))?
	{
...
fn get_repo_coordinates_from_nostr_remotes(
	git_repo: &Repo,
) -> Result<HashSet<Coordinate>> {
	let mut repo_coordinates = HashSet::new();
	for remote_name in git_repo.git_repo.remotes()?.iter().flatten() {
		if let Some(remote_url) =
			git_repo.git_repo.find_remote(remote_name)?.url()
		{
			if let Ok(nostr_url_decoded) =
				NostrUrlDecoded::from_str(remote_url)
			{
...
async fn get_repo_coordinates_from_maintainers_yaml(
	...
	if let Ok(repo_config) = get_repo_config_from_yaml(git_repo) {
		...
		if let Some(identifier) = repo_config.identifier {
			...
			// this will only apply for a few repositories created
			// before ngit v1.3 that haven't updated their
			// maintainers.yaml
			if let Ok(Some(current_user_npub)) =
				git_repo.get_git_config_item("nostr.npub", None)
			{
				if let Ok(current_user) =
					PublicKey::parse(current_user_npub)
				{
					for m in &repo_config.maintainers {
						if let Ok(maintainer) = PublicKey::parse(m) {
...
fn get_repo_coordinates_from_user_prompt(
	git_repo: &Repo,
) -> Result<HashSet<Coordinate>> {
	let mut repo_coordinates = HashSet::new();
	...
	let c = ask_for_naddr()?;
	// PROBLEM: we are saving this before checking whether it actually
	// exists, which means next time the user won't be prompted and
	// may not know how to change the selected repo
	git_repo.save_git_config_item(
		"nostr.repo",
		&c.to_bech32()?,
		false,
...
#[derive(
	Serialize, Deserialize, Default, Clone, Debug, PartialEq, Eq,
)]
...
pub fn get_repo_config_from_yaml(
	git_repo: &Repo,
) -> Result<RepoConfigYaml> {
	...
	let repo_config_yaml: RepoConfigYaml =
	...
	for s in pk_strings {
		pks.push(
			nostr_sdk::prelude::PublicKey::from_bech32(s.clone())
				.context(format!(
					"cannot convert {s} into a valid nostr public key"
				))?,
		);
	}
	Ok(pks)
...
pub fn save_repo_config_to_yaml(
	...
	let file = if path.exists() {
		...
		std::fs::File::create(path)
			.context("cannot create maintainers.yaml file")?
	};
...
mod tests {
	...
	async fn create() -> nostr::Event {
		RepoRef {
			...
			root_commit: "5e664e5a7845cd1373c79f580ca4fe29ab5b34d2"
				.to_string(),
	...
	mod try_from {
		...
		async fn name() {
			assert_eq!(
				RepoRef::try_from(create().await).unwrap().name,
				"test name",
			)
		}
		...
		mod root_commit_is_empty_if_no_r_tag_which_is_sha1_format {
			...
			use super::*;
			async fn create_with_incorrect_first_commit_ref(
				s: &str,
			) -> nostr::Event {
				nostr::Event::from_json(
					create().await.as_json().replace(
						"5e664e5a7845cd1373c79f580ca4fe29ab5b34d2",
						s,
					),
				)
			...
			async fn less_than_40_characters() {
				...
				assert_eq!(
					RepoRef::try_from(
						create_with_incorrect_first_commit_ref(s)
							.await
					)
					.unwrap()
					.root_commit,
			...
			async fn identifier() {
				assert!(
					create()
						...
						.tags
						.iter()
						.any(|t| t.as_vec()[0].eq("d")
							&& t.as_vec()[1].eq("123412341"))
				)
			}
			...
			async fn name() {
				assert!(
					create()
						...
						.tags
						.iter()
						.any(|t| t.as_vec()[0].eq("name")
							&& t.as_vec()[1].eq("test name"))
				)
			}

			#[tokio::test]
			async fn alt() {
				assert!(create().await.tags.iter().any(|t| {
					t.as_vec()[0].eq("alt")
						&& t.as_vec()[1]
							.eq("git repository: test name")
				}))
			}
			...
			async fn description() {
				assert!(
					create()
						...
						.tags
						.iter()
						.any(|t| t.as_vec()[0].eq("description")
							&& t.as_vec()[1].eq("test description"))
				)
			}
