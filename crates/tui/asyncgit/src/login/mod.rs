...
use nostr_sdk::{
	...
	SingleLetterTag, Timestamp, ToBech32,
};
...
use crate::{
	cli_interactor::{
		Interactor, InteractorPrompt, PromptConfirmParms,
...
pub async fn launch(
	...
	.await
	{
				...
				.eq("git config item nostr.nsec is an ncryptsec")
			{
				println!(
					...
					if let Ok(public_key) = PublicKey::from_bech32(
						...
						if let Ok(user_ref) = get_user_details(
							&public_key,
							client,
							git_repo.get_path()?,
							silent,
				...
				loop {
					...
					let password = Interactor::default()
						...
						.context(
							"failed to get password input from interactor.password",
						)?;
					if let Ok(keys) =
		...
		bail!(
			"TODO: enable interactive login in nostr git remote helper"
		);
	} else {
...
fn print_logged_in_as(
	...
	if !offline_mode
		&& user_ref.metadata.created_at.eq(&Timestamp::from(0))
	{
		...
		&& user_ref
			.metadata
			.name
			.eq(&user_ref.public_key.to_bech32()?)
	{
		println!(
			"cannot extract account name from account metadata..."
		);
	} else if !offline_mode
		&& user_ref.relays.created_at.eq(&Timestamp::from(0))
	{
		println!(
...
async fn get_signer_without_prompts(
	...
	if let Some(nsec) = nsec {
		...
		if let Some(bunker_app_key) = bunker_app_key {
			let signer = get_nip46_signer_from_uri_and_key(
				bunker_uri,
				bunker_app_key,
			)
			.await
		...
		get_signer_with_git_config_nsec_or_bunker_without_prompts(
			git_repo,
		)
		.await
	} else {
...
fn get_keys_from_nsec(
	...
	if save_local {
		...
		.context(
			"failed to save encrypted nsec in local git config nostr.nsec",
		)?;
	}
	Ok(keys)
}
...
fn save_to_git_config(
	...
	if let Err(error) = silently_save_to_git_config(
		git_repo, npub, nsec, bunker, global,
	) {
		println!(
		...
		if let Some(nsec) = nsec {
			if nsec.contains("ncryptsec") {
				println!(
					"manually set git config nostr.nsec to: {nsec}"
				);
			} else {
...
fn silently_save_to_git_config(
	...
	// must do this first otherwise it might remove the global items
	// just added
	if global {
		...
		git_repo
			.remove_git_config_item("nostr.bunker-app-key", false)?;
	}
	if let Some(bunker) = bunker {
	...
	if let Some(nsec) = nsec {
		...
		git_repo
			.remove_git_config_item("nostr.bunker-uri", global)?;
		git_repo
			.remove_git_config_item("nostr.bunker-app-key", global)?;
	}
...
fn get_keys_with_password(
	...
	decrypt_key(
		&git_repo
			...
			.context(
				"git config item nostr.nsec doesn't exist so cannot decrypt it",
			)?,
		password,
	)
	.context(
		"failed to decrypt stored nsec key with provided password",
	)
}
...
async fn get_nip46_signer_from_uri_and_key(
	...
	let signer = NostrSigner::nip46(
		Nip46Signer::new(
			uri,
			nostr::Keys::from_str(app_key)
				.context("invalid app key")?,
...
async fn get_signer_with_git_config_nsec_or_bunker_without_prompts(
		...
		.context("git local config item nostr.nsec doesn't exist")
	{
		...
		Ok(NostrSigner::Keys(
			nostr::Keys::from_str(local_nsec)
				.context("invalid nsec parameter")?,
		))
	} else if let Ok((uri, app_key)) =
		...
		.context("git global config item nostr.nsec doesn't exist")
	{
		...
		Ok(NostrSigner::Keys(
			nostr::Keys::from_str(global_nsec)
				.context("invalid nsec parameter")?,
		))
	} else if let Ok((uri, app_key)) =
...
fn get_git_config_bunker_uri_and_app_key(
	...
	Ok((
		git_repo
			...
			.context("failed get local git config")?
			.context(
				"git local config item nostr.bunker-uri doesn't exist",
			)?
			.to_string(),
		git_repo
			...
			.context("failed get local git config")?
			.context(
				"git local config item nostr.bunker-app-key doesn't exist",
			)?
			.to_string(),
	))
}
...
async fn fresh_login(
	...
	let signer = loop {
		...
		if let Ok(keys) = nostr::Keys::from_str(&input) {
			if let Err(error) =
				save_keys(git_repo, &keys, always_save)
			{
				println!("{error}");
			}
		...
		match get_nip46_signer_from_uri_and_key(
			&uri.to_string(),
			&app_key,
		)
		.await
...
pub async fn fetch_nip46_uri_from_nip05(
	nip05: &str,
	...
	match res {
		Ok(profile) => {
			if profile.nip46.is_empty() {
				println!(
					"nip05 provider isn't configured for remote login"
				);
				bail!(
					"nip05 provider isn't configured for remote login"
				)
		...
		Err(error) => {
			println!(
				"error contacting login service provider: {error}"
			);
			Err(error)
				.context("error contacting login service provider")
		}
	}
}
	...
	if always_save
		|| Interactor::default().confirm(
			PromptConfirmParms::default()
				.with_prompt("save login details?"),
		)? {
...
fn extract_user_metadata(
	...
	let event = events
		.iter()
		.filter(|e| {
			e.kind.eq(&nostr::Kind::Metadata)
				&& e.pubkey.eq(public_key)
		})
	...
	let metadata: Option<nostr::Metadata> = if let Some(event) = event
	{
		Some(
			nostr::Metadata::from_json(event.content.clone())
				.context(
					"metadata cannot be found in kind 0 event content",
				)?,
		)
	} else {
		None
...
fn extract_user_relays(
	...
	let event = events
		.iter()
		.filter(|e| {
			e.kind.eq(&nostr::Kind::RelayList)
				&& e.pubkey.eq(public_key)
		})
		.max_by_key(|e| e.created_at);
...
async fn get_user_details(
	...
	if let Ok(user_ref) =
		get_user_ref_from_cache(git_repo_path, public_key).await
	{
		Ok(user_ref)
	} else {
		let empty = UserRef {
...
pub async fn get_user_ref_from_cache(
	...
	let events =
		get_event_from_global_cache(git_repo_path, filters.clone())
			.await?;

	if events.is_empty() {
		bail!(
			"no metadata and profile list in cache for selected public key"
		);
	}
	Ok(UserRef {
		public_key: public_key.to_owned(),
