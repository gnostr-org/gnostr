...
pub fn get_commit_id_from_patch(event: &Event) -> Result<String> {
	...
	if value.is_ok() {
		value
	} else if event.content.starts_with("From ")
		&& event.content.len().gt(&45)
	{
...
pub async fn generate_patch_event(
	...
	let relay_hint =
		repo_ref.relays.first().map(nostr::UncheckedUrl::from);
	...
	sign_event(
		EventBuilder::new(
			...
			git_repo
				...
				.context(format!(
					"cannot make patch for commit {commit}"
				))?,
			[
				repo_ref
					.maintainers
				...
				vec![
					...
					// enables easier location of code comments
					// againt code that makes it into the main
					// branch, assuming the commit id is correct
					Tag::from_standardized(TagStandard::Reference(
						commit.to_string(),
					)),
					Tag::custom(
						TagKind::Custom(std::borrow::Cow::Borrowed(
							"alt",
						)),
						vec![format!(
							"git patch: {}",
				...
				} else if let Some(event_ref) =
					root_proposal_id.clone()
				{
					vec![
						Tag::hashtag("root"),
						Tag::hashtag("revision-root"),
						// TODO check if id is for a root proposal
						// (perhaps its for an issue?)
				...
				// see comment on branch names in cover letter event
				// creation
				...
				// repo reference. maintainers of large repos will
				// not be interested in every patch.
				repo_ref
					.maintainers
					.iter()
					...
					.collect(),
				vec![
					// a fallback is now in place to extract this
					// from the patch
					Tag::custom(
						TagKind::Custom(std::borrow::Cow::Borrowed(
							"commit",
					...
					// this is required as patches cannot be relied
					// upon to include the 'base commit'
					Tag::custom(
						TagKind::Custom(std::borrow::Cow::Borrowed(
							"parent-commit",
						)),
					...
					// this is required to ensure the commit id
					// matches
					Tag::custom(
						TagKind::Custom(std::borrow::Cow::Borrowed(
							"commit-pgp-sig",
...
pub fn event_tag_from_nip19_or_hex(
	...
	loop {
		...
		if prompt_for_correction {
			...
			bail!(format!(
				"not a valid {reference_name} event reference"
			));
		}
	...
	pub fn get_branch_name(&self) -> Result<String> {
		Ok(format!(
			...
			&self
				.event_id
				.context(
					"proposal root event_id must be know to get it's branch name"
				)?
				.to_hex()
				.as_str()[..8],
		))
...
pub fn commit_msg_from_patch(patch: &nostr::Event) -> Result<String> {
	if let Ok(msg) = tag_value(patch, "description") {
		...
		let start_index = patch.content.find("] ").context(
			"event is not formatted as a patch or cover letter",
		)? + 2;
...
pub fn commit_msg_from_patch_oneliner(
	patch: &nostr::Event,
) -> Result<String> {
...
pub fn event_to_cover_letter(
	event: &nostr::Event,
) -> Result<CoverLetter> {
	if !event_is_patch_set_root(event) {
		bail!(
			"event is not a patch set root event (root patch or cover letter)"
		)
	}
	...
	Ok(CoverLetter {
		title: title.clone(),
		description,
		// TODO should this be prefixed by
		// format!("{}-"e.id.to_string()[..5]?)
		branch_name: if let Ok(name) =
			match tag_value(event, "branch-name") {
				Ok(name) => {
...
pub fn get_most_recent_patch_with_ancestors(
	...
	let youngest_patch =
		patches.last().context("no patches found")?;

	let patches_with_youngest_created_at: Vec<&nostr::Event> =
		patches
			.iter()
		...
		.find(|e| e.id.to_string().eq(&event_id_to_search))
	{
		...
		event_id_to_search =
			get_event_parent_id(event).unwrap_or_default();
	}
	Ok(res)
}

fn get_event_parent_id(event: &nostr::Event) -> Result<String> {
	Ok(if let Some(reply_tag) = event.tags.iter().find(|t| {
		t.as_vec().len().gt(&3) && t.as_vec()[3].eq("reply")
	}) {
		reply_tag
	} else {
		event
			...
			.iter()
			.find(|t| {
				t.as_vec().len().gt(&3) && t.as_vec()[3].eq("root")
			})
...
pub fn is_event_proposal_root_for_branch(
	...
	let branch_name =
		branch_name_or_refstr.replace("refs/heads/", "");
