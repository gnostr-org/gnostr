...
mod two_branches_in_batch_one_added_one_updated {
	...
	async fn updates_branch_on_git_server() -> Result<()> {
		...
		let source_git_repo =
			GitTestRepo::recreate_as_bare(&git_repo)?;

		std::fs::write(
			git_repo.dir.join("commit.md"),
			"some content",
		)?;
		let main_commit_id =
			git_repo.stage_and_commit("commit.md")?;

		git_repo.create_branch("vnext")?;
		git_repo.checkout("vnext")?;
		std::fs::write(
			git_repo.dir.join("vnext.md"),
			"some content",
		)?;
		let vnext_commit_id =
	...
	-> Result<()> {
		...
		assert_eq!(
			...
			HashSet::from([
				...
				vec![
					"refs/heads/vnext".to_string(),
					vnext_commit_id.to_string()
				],
			]),
		);
		Ok(())
		...
		mod already_deleted_on_git_server {
			...
			-> Result<()> {
...

				{
					...
					let tmp_repo =
						GitTestRepo::clone_repo(&source_git_repo)?;
					let mut remote =
						tmp_repo.git_repo.find_remote("origin")?;
					remote.push(
						&[":refs/heads/example-branch"],
						None,
					)?;
				}
				...
				let cli_tester_handle = std::thread::spawn(
					move || -> Result<()> {
                        ...
                        cli_tester_after_nostr_fetch_and_sent_list_for_push_responds(&git_repo)?;
						p.send_line(
							"push :refs/heads/example-branch",
						)?;
						p.send_line("")?;
						p.expect("ok refs/heads/example-branch\r\n")?;
						p.expect_eventually("\r\n")?;
...
-> Result<()> {
	let (state_event, source_git_repo) =
		generate_repo_with_state_event().await?;
	let second_source_git_repo =
		GitTestRepo::duplicate(&source_git_repo)?;
...
-> Result<()> {
	...
	let cli_tester_handle =
		std::thread::spawn(move || -> Result<(String, Oid)> {
			let branch_name = get_proposal_branch_name_from_events(
				&events,
			...
			p.expect(
				format!(
					"fetching {} ref list over filesystem...\r\n",
					source_path
				)
				.as_str(),
			)?;
			...
			p.expect_eventually(
				": create nostr proposal status event\r\n",
			)?;
	...
	let proposal = r55
		...
		.find(|e| {
			e.tags()
				...
				.is_some_and(|t| {
					t.as_vec()[1].eq(FEATURE_BRANCH_NAME_1)
				})
		})
		.unwrap();
...
async fn push_2_commits_to_existing_proposal() -> Result<()> {
	...
	assert_eq!(
		output,
		format!(
			"   eb5d678..7de5e41  {branch_name} -> {branch_name}\r\n"
		)
		.as_str(),
	);
	...
	let previous_proposal_tip_event =
		r55.events
			.iter()
			.find(|e| {
				e.tags().iter().any(|t| {
...
async fn force_push_creates_proposal_revision() -> Result<()> {
	...
	let revision_root_patch = new_events
		.iter()
		.find(|e| {
			e.tags().iter().any(|t| t.as_vec()[1].eq("revision-root"))
		})
		.unwrap();
