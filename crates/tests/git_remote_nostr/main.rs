...
fn prep_git_repo() -> Result<GitTestRepo> {
	...
	test_repo
		.add_remote(NOSTR_REMOTE_NAME, &get_nostr_remote_url()?)?;
	test_repo.populate()?;
	Ok(test_repo)
}
...
fn cli_tester(git_repo: &GitTestRepo) -> CliTester {
	CliTester::new_remote_helper_from_dir(
		&git_repo.dir,
		&get_nostr_remote_url().unwrap(),
	)
}

fn cli_tester_after_fetch(
...
async fn generate_repo_with_state_event()
-> Result<(nostr::Event, GitTestRepo)> {
	let git_repo = prep_git_repo()?;
	git_repo.create_branch("example-branch")?;
	let main_commit_id =
		git_repo.get_tip_of_local_branch("main")?.to_string();
