...
-> Result<()> {
	...
	std::fs::write(
		source_git_repo.dir.join("commit.md"),
		"some content",
	)?;
	let main_commit_id =
		source_git_repo.stage_and_commit("commit.md")?;

	source_git_repo.create_branch("vnext")?;
	source_git_repo.checkout("vnext")?;
	std::fs::write(
		source_git_repo.dir.join("vnext.md"),
		"some content",
	)?;
	let vnext_commit_id =
		source_git_repo.stage_and_commit("vnext.md")?;
	...
	let cli_tester_handle =
		std::thread::spawn(move || -> Result<()> {
			assert!(
				git_repo
					.git_repo
					.find_commit(main_commit_id)
					.is_err()
			);
			assert!(
				git_repo
					.git_repo
					.find_commit(vnext_commit_id)
					.is_err()
			);

			let mut p = cli_tester_after_fetch(&git_repo)?;
			p.send_line(
				format!("fetch {main_commit_id} main").as_str(),
			)?;
			p.send_line(
				format!("fetch {vnext_commit_id} vnext").as_str(),
			)?;
			p.send_line("")?;
			p.expect(
				format!(
					"fetching {source_path} over filesystem...\r\n"
				)
				.as_str(),
			)?;
			...
			assert!(
				git_repo.git_repo.find_commit(main_commit_id).is_ok()
			);
			assert!(
				git_repo
					.git_repo
	...
	-> Result<()> {
		...
		let cli_tester_handle =
			std::thread::spawn(move || -> Result<()> {
				...
				p.expect(
					format!(
						"fetching {error_path} over filesystem...\r\n"
					)
					.as_str(),
				)?;
...
-> Result<()> {
	...
	let cli_tester_handle =
		std::thread::spawn(move || -> Result<()> {
			...
			assert!(
				git_repo.git_repo.find_commit(proposal_tip).is_err()
			);

			let mut p = cli_tester_after_fetch(&git_repo)?;
			p.send_line(
				format!(
					"fetch {proposal_tip} refs/heads/{branch_name}"
				)
				.as_str(),
			)?;
			p.send_line("")?;
			...
			assert!(
				git_repo.git_repo.find_commit(proposal_tip).is_ok()
			);
