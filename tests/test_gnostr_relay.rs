...
mod tests {
    ...
    async fn test_server_starts_and_websocket_connects() -> Result<()> {
        let srv = start(|| {
            let (app_data, _server_address) =
                create_test_app_instance("test_server_starts_and_websocket_connects").unwrap();
            app_data.web_app()
        });
        ...
        let (mut ws_stream, _) = retry_strategy
            .run_async(move || {
                ...
                async move {
                    connect_async(&ws_url_clone)
                        .await
                        .map_err(|e| Box::new(e) as Box<dyn std::error::Error>)
                }
            })
            .await
            .expect("Failed to connect to websocket after retries");
    ...
    async fn test_event_submission_and_retrieval() -> Result<()> {
        let srv = start(|| {
            let (app_data, _server_address) =
                create_test_app_instance("test_event_submission_and_retrieval").unwrap();
            app_data.web_app()
        });
        ...
        let retry_strategy = GnostrRetry::new_exponential_async(1, 3); // 1 second initial delay, 3 retries
        let (ws_stream_raw, _) = retry_strategy
            .run_async(move || {
                let ws_url_clone = ws_url.clone();
                async move {
                    connect_async(&ws_url_clone)
        ...
        ws_stream
            .lock()
            .await
            .send(Message::Text(event_json.clone()))
            .await?;
        ...
        let msg = {
            ...
            retry_strategy
                ...
                .await
                .expect("Failed to receive OK message after retries")
        };
        ...
        let msg = {
            ...
            retry_strategy
                ...
                .await
                .expect("Failed to receive EVENT message after retries")
        };
        let text = msg.to_text()?;
        ...
        let msg = {
            ...
            retry_strategy
                ...
                .await
                .expect("Failed to receive EOSE message after retries")
        };
        let text = msg.to_text()?;
