...
mod tests {
    use gnostr_asyncgit::sync::{
        self, create_branch, get_commit_details, get_head, get_stashes, get_status,
        stage_add_file, stash_save, checkout_branch, get_head_tuple, StatusItemType, RepoPath,
    };
    ...
    fn test_complex_git_workflow() {
        let (_tmp_dir, repo_path) = setup_test_repo();
        let repo_path_str = repo_path.git_path().unwrap();
        ...
        // 4. Stash the changes
        let stash_hash = stash_save(&repo_path, Some("test stash")).unwrap();
