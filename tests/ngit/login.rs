...
static EXPECTED_LOCAL_REPOSITORY_PROMPT: &str =
...
static EXPECTED_SET_PASSWORD_CONFIRM_PROMPT: &str =
	"confirm password";
				...
				async fn run_test_displays_correct_name(
					...
					let cli_tester_handle =
						std::thread::spawn(move || -> Result<()> {
							...
							p.expect_confirm(
								EXPECTED_LOCAL_REPOSITORY_PROMPT,
								Some(false),
							)?
							.succeeds_with(Some(true))?;

							p.expect_confirm(
								EXPECTED_REQUIRE_PASSWORD_PROMPT,
								Some(false),
							)?
...
mod with_offline_flag {
	...
	mod when_first_time_login {
		...
		fn succeeds_with_text_logged_in_as_npub() -> Result<()> {
			...
			p.expect_password(EXPECTED_SET_PASSWORD_PROMPT)?
				.with_confirmation(
					EXPECTED_SET_PASSWORD_CONFIRM_PROMPT,
				)?
				.succeeds_with(TEST_PASSWORD)?;
	...
	mod when_weak_password {
		...
		-> Result<()> {
			let test_repo = GitTestRepo::default();
			let mut p = CliTester::new_with_timeout_from_dir(
				15000,
				&test_repo.dir,
				["login", "--offline"],
			);
