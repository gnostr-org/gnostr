		...
		async fn run_async_repo_event_ref_needed(
			...
			let cli_tester_handle = std::thread::spawn(
				move || -> Result<()> {
					...
					if invalid_input {
						let mut input =
							p.expect_input("repository naddr")?;
					...
					if naddr {
						...
						input.succeeds_with(
							&Coordinate {
								...
								public_key: TEST_KEY_1_KEYS
									.public_key(),
								identifier: repo_event
									.identifier()
									.unwrap()
									.to_string(),
								relays: vec![
						...
						p.expect_end_with(
							"no proposals found... create one? try `ngit send`\r\n",
						)?;
					}
			...
			mod when_first_proposal_selected {
				...
				-> Result<()> {
					let (originating_repo, test_repo) =
                        prep_proposals_repo_and_repo_with_proposal_pulled_and_checkedout(1).await?;
					assert_eq!(
						originating_repo.get_tip_of_local_branch(
							FEATURE_BRANCH_NAME_1
						)?,
		...
		mod when_main_is_checked_out {
			...
			mod when_latest_revision_rebases_branch {
				...
				-> Result<(GitTestRepo, GitTestRepo)> {
					...
					let cli_tester_handle: JoinHandle<
						Result<(GitTestRepo, GitTestRepo)>,
