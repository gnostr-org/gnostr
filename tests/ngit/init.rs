...
fn expect_msgs_after(p: &mut CliTester) -> Result<()> {
	p.expect_after_whitespace(
		"maintainers.yaml created. commit and push.\r\n",
	)?;
	p.expect(
			...
			async fn async_run_test() -> Result<()> {
				...
				let cli_tester_handle =
					std::thread::spawn(move || -> Result<()> {
						...
						let yaml_path =
							git_repo.dir.join("maintainers.yaml");
						...
						std::thread::spawn(move || -> Result<()> {
							...
							let mut file =
								std::fs::File::create(&yaml_path)
									.expect(
										"cannot create maintainers.yaml file",
									);
							write!(
								file,
								"\
			...
			async fn d_replaceable_event_identifier() -> Result<()> {
				...
				for relay in [&r53, &r55, &r56, &r57] {
					let event: &nostr::Event = relay
						...
						.iter()
						.find(|e| {
							e.kind.eq(&Kind::GitRepoAnnouncement)
						})
						.unwrap();

					assert!(event.tags.iter().any(|t| {
						t.as_vec()[0].eq("d")
							&& t.as_vec()[1].eq("example-identifier")
					}));
				}
				Ok(())
			}
			...
			async fn alt() -> Result<()> {
				...
				for relay in [&r53, &r55, &r56, &r57] {
					...
					assert!(event.tags.iter().any(|t| {
						t.as_vec()[0].eq("alt")
							&& t.as_vec()[1]
								.eq("git repository: example-name")
					}));
				}
				Ok(())
			}
