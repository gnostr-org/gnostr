...
fn when_no_main_or_master_branch_return_error() -> Result<()> {
	...
	p.expect(
		"Error: the default branches (main or master) do not exist",
	)?;
	Ok(())
}
...
mod when_commits_behind_ask_to_proceed {
	...
	fn when_response_is_false_aborts() -> Result<()> {
		...
		p.expect_end_with(
			"Error: aborting so commits can be rebased\r\n",
		)?;
...
fn expect_msgs_after(p: &mut CliTester) -> Result<()> {
	p.expect_after_whitespace(
		"view in gitworkshop.dev: https://gitworkshop.dev/repo",
	)?;
	p.expect_eventually("\r\n")?;
...
async fn prep_run_create_proposal(
	...
	let cli_tester_handle =
		std::thread::spawn(move || -> Result<()> {
			let mut p = cli_tester_create_proposal(
				&git_repo,
...
mod when_cover_letter_details_specified_with_range_of_head_2_sends_cover_letter_and_2_patches_to_3_relays {
	...
	#[tokio::test]
	#[serial]
	async fn only_1_cover_letter_event_sent_to_each_relay()
	-> Result<()> {
		let (_, _, r53, r55, r56) =
			prep_run_create_proposal(true).await?;
	...
	#[tokio::test]
	#[serial]
	async fn only_2_patch_kind_events_sent_to_each_relay()
	-> Result<()> {
		let (r51, r52, r53, r55, r56) =
			prep_run_create_proposal(true).await?;
		for relay in [&r51, &r52, &r53, &r55, &r56] {
			assert_eq!(
				relay.events.iter().filter(|e| is_patch(e)).count(),
				2,
			);
		}
		Ok(())
	...
	#[tokio::test]
	#[serial]
	async fn patch_content_contains_patch_in_email_format_with_patch_series_numbers()
	-> Result<()> {
		let (_, _, r53, r55, r56) =
			prep_run_create_proposal(true).await?;
		for relay in [&r53, &r55, &r56] {
	...
	mod cover_letter_tags {
		...
		async fn root_commit_as_r() -> Result<()> {
			let (_, _, r53, r55, r56) =
				prep_run_create_proposal(true).await?;
			for relay in [&r53, &r55, &r56] {
				let cover_letter_event: &nostr::Event = relay
					.events
					.iter()
					.find(|e| is_cover_letter(e))
					.unwrap();
		...
		#[tokio::test]
		#[serial]
		async fn a_tag_for_repo_event_of_each_maintainer()
		-> Result<()> {
			let (_, _, r53, r55, r56) =
				prep_run_create_proposal(true).await?;
			for relay in [&r53, &r55, &r56] {
			...
			for relay in [&r53, &r55, &r56] {
				for m in maintainers {
					let cover_letter_event: &nostr::Event = relay
						...
						.find(|e| is_cover_letter(e))
						.unwrap();
					assert!(cover_letter_event.tags().iter().any(
						|t| {
							t.as_vec()[0].eq("p")
								&& t.as_vec()[1].eq(m)
						}
		...
		async fn t_tag_root() -> Result<()> {
			...
			for relay in [&r53, &r55, &r56] {
				let cover_letter_event: &nostr::Event = relay
					...
					.unwrap();
				assert!(cover_letter_event.tags().iter().any(|t| {
					t.as_vec()[0].eq("t") && t.as_vec()[1].eq(&"root")
				}));
			}
			Ok(())
		}
	...
	mod patch_tags {
		...
		async fn commit_and_commit_r() -> Result<()> {
			...
			assert!(
				most_recent_patch
					.tags
					.iter()
					.any(|t| t.as_vec()[0].eq("r")
						&& t.as_vec()[1].eq(COMMIT_ID))
			);
			assert!(
				most_recent_patch
					.tags
					.iter()
					.any(|t| t.as_vec()[0].eq("commit")
						&& t.as_vec()[1].eq(COMMIT_ID))
			);
			Ok(())
		}
		...
		async fn p_tags_for_maintainers() -> Result<()> {
			...
			for m in maintainers {
				assert!(prep().await?.tags().iter().any(|t| {
					t.as_vec()[0].eq("p") && t.as_vec()[1].eq(m)
				}));
			}
			Ok(())
		}
	...
	mod cli_ouput {
		...
		async fn check_cli_output() -> Result<()> {
					...
					Some(&|relay,
					 ...
					 -> Result<()> {
						relay.respond_events(
							...
							&subscription_id,
							&vec![
								generate_test_key_1_metadata_event(
									"fred",
								),
								generate_test_key_1_relay_list_event(
								),
			...
			let cli_tester_handle =
				std::thread::spawn(move || -> Result<()> {
					let mut p =
						cli_tester_create_proposal(&git_repo, true);
					...
					relay::expect_send_with_progress(
						...
						vec![
							...
							(
								" [default] ws://localhost:8051",
								true,
								"",
							),
							(
								" [default] ws://localhost:8052",
								true,
								"",
							),
						],
...
mod when_no_cover_letter_flag_set_with_range_of_head_2_sends_2_patches_without_cover_letter {
	...
	mod cli_ouput {
		...
		async fn check_cli_output() -> Result<()> {
			...
			let cli_tester_handle =
				std::thread::spawn(move || -> Result<()> {
					let mut p =
						cli_tester_create_proposal(&git_repo, false);
	...
	async fn no_cover_letter_event() -> Result<()> {
		let (_, _, r53, r55, r56) =
			prep_run_create_proposal(false).await?;
		for relay in [&r53, &r55, &r56] {
			assert_eq!(
				relay
					.events
...
mod when_range_ommited_prompts_for_selection_defaulting_ahead_of_main {
	...
	fn expect_msgs_first(p: &mut CliTester) -> Result<()> {
		...
		let mut selector = p.expect_multi_select(
			...
			vec![
				"(Joe Bloggs) add t4.md [feature] fe973a8"
					.to_string(),
		...
		p.expect(
			"posting 2 patches without a covering letter...\r\n",
		)?;
		Ok(())
	}
	...
	async fn two_patch_events_sent() -> Result<()> {
		let (_, _, r53, r55, r56) =
			prep_run_create_proposal().await?;
		for relay in [&r53, &r55, &r56] {
			assert_eq!(
				relay.events.iter().filter(|e| is_patch(e)).count(),
				2
...
mod root_proposal_specified_using_in_reply_to_with_range_of_head_2_and_cover_letter_details_specified {
	...
	fn cli_tester_create_proposal(
		...
		let proposal_root_bech32 =
			get_pretend_proposal_root_event().id.to_bech32().unwrap();
		let args = vec![
			"--nsec",
			TEST_KEY_1_NSEC,
			"--password",
	...
	fn expect_msgs_first(
		...
		p.expect("fetching updates...\r\n")?;
		p.expect(
			"updates: 1 new maintainer, 1 announcement update, 1 proposal\r\n",
		)?;
		let proposal_root_bech32 =
			get_pretend_proposal_root_event().id.to_bech32().unwrap();
		p.expect(format!(
...
mod in_reply_to_mentions_issue {
	...
	fn cli_tester_create_proposal(
		...
		let issue_bech32 =
			get_pretend_issue_event().id.to_bech32().unwrap();
		let args = vec![
			"--nsec",
			TEST_KEY_1_NSEC,
			"--password",
	...
	#[tokio::test]
	#[serial]
	async fn issue_event_mentioned_in_tagged_cover_letter()
	-> Result<()> {
		let (_, _, r53, r55, r56) =
			prep_run_create_proposal().await?;
		for relay in [&r53, &r55, &r56] {
			...
			assert!(cover_letter_event.tags().iter().any(|t| {
				t.as_vec()[0].eq("e")
					&& t.as_vec()[1]
						.eq(&get_pretend_issue_event().id.to_hex())
