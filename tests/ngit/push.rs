...
mod when_proposal_isnt_associated_with_branch_name {
	...
	mod cli_prompts {
		...
		async fn cli_show_error() -> Result<()> {
			...
			r51.events
				.push(generate_test_key_1_metadata_event("fred"));
...
mod when_branch_is_checked_out {
	...
	mod when_branch_is_up_to_date {
		...
		mod cli_prompts {
			...
			async fn cli_show_up_to_date() -> Result<()> {
				...
				r51.events
					.push(generate_test_key_1_relay_list_event());
				r51.events
					.push(generate_test_key_1_metadata_event("fred"));
				r51.events.push(generate_repo_ref_event());
				...
				let cli_tester_handle = std::thread::spawn(
					move || -> Result<()> {
						...
						p.expect_eventually("\r\n")?; // some updates listed here
						p.expect_end_with(
							"Error: proposal already up-to-date with local branch\r\n",
						)?;
			...
			async fn cli_show_proposal_ahead_error() -> Result<()> {
				...
				let cli_tester_handle = std::thread::spawn(
					move || -> Result<()> {
						...
						p.expect_eventually("\r\n")?; // some updates listed here
						p.expect(
							"Error: proposal is ahead of local branch\r\n",
						)?;
						p.expect_end()?;
	...
	mod when_branch_is_ahead {
		...
		async fn prep_and_run()
		-> Result<(GitTestRepo, Vec<nostr::Event>)> {
			// fallback (51,52) user write (53, 55) repo (55, 56)
			let (mut r51, mut r52, mut r53, mut r55, mut r56) = (
				Relay::new(8051, None, None),
				Relay::new(8052, None, None),
			...
			async fn cli_shows_unpublished_rebase_error() -> Result<()>
			{
				...
				let cli_tester_handle = std::thread::spawn(
					move || -> Result<()> {
                        ...
                        create_proposals_and_repo_with_proposal_pulled_and_checkedout(1)?;
						let branch_name =
							tmp_repo.get_checked_out_branch_name()?;
				...
				async fn cli_shows_revision_sent() -> Result<()> {
					...
					let cli_tester_handle = std::thread::spawn(
						move || -> Result<()> {
							...
							p.expect_eventually("\r\n")?; // some updates listed here
							p.expect(
								"preparing to force push proposal revision...\r\n",
							)?;
							...
							p.expect_eventually("\r\n")?;
							p.expect(
								"creating proposal from 2 commits:\r\n",
							)?;
							...
							p.expect_after_whitespace(
								"force pushed proposal revision\r\n",
							)?;
							p.expect_end()?;
