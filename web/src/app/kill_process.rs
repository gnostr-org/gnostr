...
pub fn kill_gnostr_asyncgit_process() {
    #[cfg(unix)]
    {
        let output = Command::new("pkill").arg("gnostr-asyncgit").output();
        ...
        match output {
            Ok(out) => {
                if out.status.success() {
                    ...
                    eprintln!(
                        "Failed to kill gnostr-asyncgit process: {}",
    ...
    {
        println!(
            "Killing gnostr-asyncgit process not implemented for Windows without external crates."
        );
