...
impl HookPaths {
    ...
    /// doing.
    pub fn new(repo: &Repository, other_paths: Option<&[&str]>, hook: &str) -> Result<Self> {
        ...
        if let Some(config_path) = Self::config_hook_path(repo)? {
            ...
            let hook =
                shellexpand::full(hook.as_os_str().to_str().ok_or(HooksError::PathToString)?)?;

            let hook = PathBuf::from_str(hook.as_ref()).map_err(|_| HooksError::PathToString)?;
    ...
    /// path
    fn find_hook(repo: &Repository, other_paths: Option<&[&str]>, hook: &str) -> PathBuf {
        let mut paths = vec![DEFAULT_HOOKS_PATH.to_string()];
        if let Some(others) = other_paths {
            paths.extend(others.iter().map(|p| p.trim_end_matches('/').to_string()));
        }
...
fn find_bash_executable() -> Option<PathBuf> {
    if cfg!(windows) {
        Command::new("where.exe")
            ...
            .output()
            .ok()
            .map(|out| PathBuf::from(Into::<String>::into(String::from_utf8_lossy(&out.stdout))))
            .as_deref()
