...
pub fn hooks_commit_msg(
    ...
    let res = hook.run_hook(&[temp_file.as_os_str().to_string_lossy().as_ref()])?;
...
pub fn hooks_pre_commit(repo: &Repository, other_paths: Option<&[&str]>) -> Result<HookResult> {
...
pub fn hooks_post_commit(repo: &Repository, other_paths: Option<&[&str]>) -> Result<HookResult> {
...
pub fn hooks_prepare_commit_msg(
    ...
    let hook = HookPaths::new(repo, other_paths, HOOK_PREPARE_COMMIT_MSG)?;
