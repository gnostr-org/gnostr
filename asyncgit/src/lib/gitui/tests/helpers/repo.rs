...
impl RepoTestContext {
    pub fn setup_init() -> Self {
        ...
        run(remote_dir.path(), &["git", "init", "--initial-branch=main"]);
        set_config(dir.path());
...
pub fn set_env_vars() {
    ...
    unsafe { env::set_var("GIT_CONFIG_GLOBAL", "/dev/null") };
    unsafe { env::set_var("GIT_CONFIG_SYSTEM", "/dev/null") };
    unsafe { env::set_var("GIT_AUTHOR_NAME", "Author Name") };
    unsafe { env::set_var("GIT_AUTHOR_EMAIL", "author@email.com") };
    unsafe { env::set_var("GIT_AUTHOR_DATE", "Fri Feb 16 11:11 2024 +0100") };
    unsafe { env::set_var("GIT_COMMITTER_NAME", "Committer Name") };
    unsafe { env::set_var("GIT_COMMITTER_EMAIL", "committer@email.com") };
    unsafe { env::set_var("GIT_COMMITTER_DATE", "Sun Feb 18 14:00 2024 +0100") };
    unsafe { env::set_var("LC_ALL", "C") };
}
