...
pub fn reset_stage(repo_path: &RepoPath, path: &str) -> Result<()> {
    ...
    if let Ok(id) = get_head_repo(&repo) {
        let obj = repo.find_object(id.into(), Some(ObjectType::Commit))?;
...
///
pub fn reset_repo(repo_path: &RepoPath, commit: CommitId, kind: ResetType) -> Result<()> {
...
1   end";
    ...
    fn test_reset_only_unstaged() {
        ...
        let res = get_status(repo_path, StatusType::WorkingDir, None).unwrap();
        assert_eq!(res.len(), 0);
    ...
    fn test_reset_folder() -> Result<()> {
...

        {
            ...
            File::create(root.join("foo/file1.txt"))?.write_all(b"file1")?;
            File::create(root.join("foo/file2.txt"))?.write_all(b"file1")?;
            File::create(root.join("file3.txt"))?.write_all(b"file3")?;
        }
...

        {
            ...
            File::create(root.join("foo/file4.txt"))?.write_all(b"file4")?;
            File::create(root.join("foo/file5.txt"))?.write_all(b"file5")?;
            File::create(root.join("file3.txt"))?.write_all(b"file3\nadded line")?;
        }

        assert_eq!(get_statuses(repo_path), (5, 0));

        stage_add_file(repo_path, Path::new("foo/file5.txt")).unwrap();
