...
async fn handle_socket(socket: WebSocket, tx: broadcast::Sender<String>) {
    ...
    // Spawn a task to send messages from the broadcast channel to the client
        ...
        while let Some(Ok(msg)) = receiver.next().await {
            match msg {
                Message::Text(text) => {
                    ...
                    // Echo back the message for now, and broadcast it
                    if tx.send(format!("Echo: {}", text)).is_err() {
                        break; // No receivers
                    }
                }
                Message::Binary(bytes) => {
                    info!("Received binary message from WebSocket: {:?}", bytes);
                    // Optionally handle binary messages
                }
                Message::Ping(_) | Message::Pong(_) => {
                    // Ping/Pong messages are often handled automatically or not strictly echoed
                    // For an echo server, we can ignore them in the recv_task
                }
                Message::Close(close_frame) => {
                    info!("Received close frame from WebSocket: {:?}", close_frame);
                    break;
                }
            }
        }
...
async fn run_as_service() -> Result<(), anyhow::Error> {
    info!("Starting gnostr-nip34 in daemon mode...");
    ...
    cmd.env("GNOSTR_NIP34_MODE", "daemon");
    ...
    println!("gnostr-nip34 started in background");
...
async fn main() -> Result<(), anyhow::Error> {
    ...
    let app = Router::new()
        ...
        .layer(CorsLayer::new())
        .with_state(tx.clone()); // Add the broadcast sender as state
