...
use crate::{
    error::{Error, Result},
    sync::{
        cred::BasicAuthCredential, remotes::push::ProgressNotification, repository::repo, utils,
    },
    ProgressPercent,
};
...
pub fn get_default_remote_for_fetch(repo_path: &RepoPath) -> Result<String> {
    let repo = repo(repo_path)?;
...
pub(crate) fn get_default_remote_for_fetch_in_repo(repo: &Repository) -> Result<String> {
...
mod tests {
    ...
    fn test_default_remote() {
        ...
        let first = get_default_remote_in_repo(&repo(repo_path).unwrap()).unwrap();
    ...
    fn test_default_remote_inconclusive() {
        ...
        let default_remote = get_default_remote_in_repo(&repo(repo_path).unwrap());

        assert!(matches!(default_remote, Err(Error::NoDefaultRemoteFound)));
    }
    ...
    fn test_default_remote_for_fetch() {
        ...
        let default_fetch_remote = get_default_remote_for_fetch_in_repo(&repo);

        assert!(matches!(default_fetch_remote, Ok(remote_name) if remote_name == "branchremote"));
    }
    ...
    fn test_default_remote_for_push() {
        ...
        let default_push_remote = get_default_remote_for_push_in_repo(&repo);

        assert!(matches!(default_push_remote, Ok(remote_name) if remote_name == "branchremote"));

        config.set_str("remote.pushDefault", "pushdefault").unwrap();

        let default_push_remote = get_default_remote_for_push_in_repo(&repo);

        assert!(matches!(default_push_remote, Ok(remote_name) if remote_name == "pushdefault"));
