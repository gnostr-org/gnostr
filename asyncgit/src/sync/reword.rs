...
fn reword_internal(repo: &Repository, commit: Oid, message: &str) -> Result<Oid> {
    ...
    if let Ok(Some(branch)) = get_current_branch(repo) {
        ...
        let top_branch_commit = repo.find_annotated_commit(branch.get().peel_to_commit()?.id())?;
        ...
        repo.branch(&cur_branch_name, &repo.find_commit(target)?, true)?;
...
mod tests {
    ...
    fn test_reword() {
        ...
        let branch = repo.branches(None).unwrap().next().unwrap().unwrap().0;
        ...
        let reworded = reword(repo_path, oid2.into(), "NewCommitMessage").unwrap();
