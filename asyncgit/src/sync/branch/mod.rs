...
use crate::{
    error::{Error, Result},
    sync::{
        remotes::get_default_remote_for_push_in_repo, repository::repo, utils::get_head_repo,
        CommitId,
    },
};
...
///
pub fn branch_compare_upstream(repo_path: &RepoPath, branch: &str) -> Result<BranchCompare> {
...
pub fn checkout_commit(repo_path: &RepoPath, commit_hash: CommitId) -> Result<()> {
...
///
pub fn checkout_remote_branch(repo_path: &RepoPath, branch: &BranchInfo) -> Result<()> {
...
/// HEAD to new branch
pub fn create_branch(repo_path: &RepoPath, name: &str) -> Result<String> {
...
mod tests_checkout_commit {
    ...
    fn test_smoke() {
        ...
        let commit = write_commit_file(&repo, "test_1.txt", "test", "commit1");
        write_commit_file(&repo, "test_2.txt", "test", "commit2");

        checkout_commit(repo_path, commit).unwrap();

        assert!(repo.head_detached().unwrap());
        assert_eq!(repo.head().unwrap().target().unwrap(), commit.get_oid());
    }
}
