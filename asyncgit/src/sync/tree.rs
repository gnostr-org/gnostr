...
pub fn tree_files(repo_path: &RepoPath, commit: CommitId) -> Result<Vec<TreeFile>> {
...
pub fn tree_file_content(repo_path: &RepoPath, file: &TreeFile) -> Result<String> {
...
mod tests {
    ...
    fn test_smoke() {
        ...
        let c1 = write_commit_file(&repo, "test.txt", "content", "c1");
        ...
        let c2 = write_commit_file(&repo, "test.txt", "content2", "c2");

        let content = tree_file_content(repo_path, &files[0]).unwrap();
    ...
    fn test_sorting_folders() {
        ...
        assert_eq!(
            list.iter()
                .map(|f| f.path.to_string_lossy())
                .collect::<Vec<_>>(),
            vec![String::from("afolder/file"), String::from("bfolder/file"),]
        );
    }
    ...
    fn test_path_file_cmp() {
        assert_eq!(
            path_cmp(&PathBuf::from("a"), &PathBuf::from("afolder/file")),
            Ordering::Greater
        );
    }
}
