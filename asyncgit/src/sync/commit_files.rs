...
/// Sort two commits.
pub fn sort_commits(repo: &Repository, commits: (CommitId, CommitId)) -> Result<OldNew<CommitId>> {
    if repo.graph_descendant_of(commits.0.get_oid(), commits.1.get_oid())? {
        Ok(OldNew {
            old: commits.1,
            new: commits.0,
        })
...
pub fn get_commit_files(
    ...
    let diff = if let Some(other) = other {
        get_compare_commits_diff(&repo, sort_commits(&repo, (id, other))?, None, None)?
    } else {
        get_commit_diff(
            &repo,
            id,
...
pub fn get_compare_commits_diff(
    ...
    let diff: Diff<'_> =
        repo.diff_tree_to_tree(Some(&trees.old), Some(&trees.new), Some(&mut opts))?;
