...
use git2::{Repository, RepositoryOpenFlags, Submodule, SubmoduleUpdateOptions};
...
impl SubmoduleInfo {
    ///
    pub fn get_repo_path(&self, repo_path: &RepoPath) -> Result<RepoPath> {
...
fn submodule_to_info(s: &Submodule, r: &Repository) -> SubmoduleInfo {
    let status = r
        .submodule_status(s.name().unwrap_or_default(), git2::SubmoduleIgnore::None)
...
///
pub fn get_submodules(repo_path: &RepoPath) -> Result<Vec<SubmoduleInfo>> {
...
///
pub fn update_submodule(repo_path: &RepoPath, name: &str) -> Result<()> {
...
pub fn submodule_parent_info(repo_path: &RepoPath) -> Result<Option<SubmoduleParentInfo>> {
...
mod tests {
    ...
    fn test_smoke() {
        ...
        let info = submodule_parent_info(&subs[0].get_repo_path(&repo_p).unwrap())
            .unwrap()
            .unwrap();
