...
fn rebase_branch_repo(
    ...
    let annotated = repo.reference_to_annotated_commit(&branch.into_reference())?;
...
pub fn conflict_free_rebase(
    ...
    last_commit.ok_or_else(|| Error::Generic(String::from("no commit rebased")))
}
...
/// rebase
pub fn rebase(repo: &git2::Repository, commit: &git2::AnnotatedCommit) -> Result<RebaseState> {
    let mut rebase = repo.rebase(None, Some(commit), None, None)?;
    let signature = crate::sync::commit::signature_allow_undefined_name(repo)?;
...
///
pub fn get_rebase_progress(repo: &git2::Repository) -> Result<RebaseProgress> {
    let mut rebase = repo.open_rebase(None)?;
...
mod test_conflict_free_rebase {
    ...
    ///
    fn test_rebase_branch_repo(repo_path: &RepoPath, branch_name: &str) -> CommitId {
    ...
    fn test_smoke() {
        ...
        let c1 = write_commit_file(&repo, "test1.txt", "test", "commit1");

        create_branch(repo_path, "foo").unwrap();

        let c2 = write_commit_file(&repo, "test2.txt", "test", "commit2");
...
mod test_rebase {
    ...
    use crate::sync::{
        checkout_branch, create_branch,
        rebase::{abort_rebase, get_rebase_progress, RebaseProgress, RebaseState},
    ...
    fn test_conflicted_abort() {
        ...
        let r = rebase_branch(repo_path, "master", BranchType::Local).unwrap();
