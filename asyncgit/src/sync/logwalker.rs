...
impl<'a> LogWalker<'a> {
    ...
    ///
    #[must_use]
    pub fn filter(self, filter: Option<SharedCommitFilterFn>) -> Self {
        Self { filter, ..self }
    }
    ...
    pub fn read(&mut self, out: &mut Vec<CommitId>) -> Result<usize> {
        ...
        while let Some(c) = self.commits.pop() {
            ...
            let commit_should_be_included = if let Some(ref filter) = self.filter {
                filter(self.repo, &id)?
            } else {
                true
            };
...
mod tests {
    ...
    use crate::{
        ...
        sync::{
            ...
            diff_contains_file, filter_commit_by_search, get_commits_info, stage_add_file,
    ...
    fn test_logwalker_with_filter() -> Result<()> {
        ...
        let mut items = Vec::new();
        let mut walker = LogWalker::new(&repo, 100)?.filter(Some(diff_contains_baz));
        ...
        let mut items = Vec::new();
        let mut walker = LogWalker::new(&repo, 100)?.filter(Some(diff_contains_bar));
        walker.read(&mut items).unwrap();
    ...
    fn test_logwalker_with_filter_search() {
        ...
        let second_commit_id = write_commit_file(&repo, "baz", "a", "my commit msg (#2)");
        write_commit_file(&repo, "foo", "b", "commit3");

        let log_filter = filter_commit_by_search(LogFilterSearch::new(LogFilterSearchOptions {
        ...
        let mut items = Vec::new();
        let mut walker = LogWalker::new(&repo, 100).unwrap().filter(Some(log_filter));
        walker.read(&mut items).unwrap();
