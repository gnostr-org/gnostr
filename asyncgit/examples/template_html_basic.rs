...
async fn main() {
    ...
    let index_html_content = format!(
        "{}\n{}\n{}\n{}\n{}",
        &base_html[..base_html
            .find("    <link rel=\"stylesheet\" href=\"/css/vars.css?v=1\" />")
            .unwrap_or(base_html.len())],
        link_tags,
        &base_html[base_html
            .find("    <link rel=\"stylesheet\" href=\"/css/vars.css?v=1\" />")
            .unwrap_or(base_html.len())
            ..base_html
                .find("    <script defer src=\"/js/util.js?v=5\"></script>")
                .unwrap_or(base_html.len())],
        script_tags,
        &base_html[base_html
            .find("    <script defer src=\"/js/util.js?v=5\"></script>")
    ...
    let index_html_route =
        warp::path::end().map(move || warp::reply::html(index_html_content.clone()));
    ...
    let js_route = warp::path("js")
        ...
        .and_then(
            |filename: String, assets: Arc<HashMap<String, &'static [u8]>>| async move {
