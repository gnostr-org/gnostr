...
function event_get_tagged_pubkeys(ev) {
  const keys = []
  for (const tag of ev.tags) {
    if (tag.length >= 2 && tag[0] == "p") keys.push(tag[1])
  }
  return keys
}
...
function event_get_tag_values(ev) {
  const keys = []
  for (const tag of ev.tags) {
    if (tag.length >= 2) keys.push(tag[1])
  }
  return keys
}
...
function event_get_tag_refs(tags) {
  ...
  if (!(root && reply) && ids.length > 0) {
    if (ids.length === 1) return { root: ids[0], reply: ids[0], pubkeys }
    else if (ids.length >= 2) return { root: ids[0], reply: ids[1], pubkeys }
    return { pubkeys }
  }
...
function event_is_spam(ev, contacts, pow) {
  if (contacts.friend_of_friends.has(ev.pubkey)) return true
  return ev.pow >= pow
}
...
function event_refs_event(a, b) {
  for (const tag of a.tags) {
    if (tag.length >= 2 && tag[0] === "e" && tag[1] == b.id) return true
  }
  return false
}
...
function event_get_last_tags(ev) {
  let o = {}
  for (const tag of ev.tags) {
    if (tag.length >= 2 && (tag[0] === "e" || tag[0] === "p")) o[tag[0]] = tag[1]
  }
  return o
}
...
function event_is_dm(ev, mykey) {
  ...
  if (ev.pubkey != mykey && event_tags_pubkey(ev, mykey)) return true
