...
const URL_REGEX = /(^|\s)(https?:\/\/[^\s]+)[,:)]?(\w|$)/gm
...
function arr_bsearch(arr, item, cmp) {
  ...
  while (start <= end) {
    ...
    if (x > 0) start = middle + 1
    else if (x < 0) end = middle - 1
    else return middle
  }
  return -1
}
...
function get_profile_pic(profile) {
  if (profile && profile.data && profile.data.picture) return html`${profile.data.picture}`
  return IMG_NO_USER
}
...
function get_picture(profile) {
  if (!profile || !profile.picture) return IMG_NO_USER
...
async function decrypt_dms(model) {
  ...
  for (const item of model.dms) {
    let dm = item[1]
    if (!dm.needs_decryption) continue
    for (const ev of dm.events) {
      if (ev.decrypted != undefined) continue
      let str
      try {
