...
function delete_post_confirm(evid) {
  ...
  if (reason.toLowerCase() === "cancel") return
...
function reply(evid) {
  const ev = GNOSTR.all_events[evid]
  const modal = document.querySelector("#reply-modal")
  ...
  replying_to.innerHTML = render_event_nointeract(GNOSTR, ev, {
    is_composing: true,
    nobar: true,
  })
...
function update_notifications(model) {
  ...
  for (const item of model.dms) {
    if (item[1].new_count) dm_count += item[1].new_count
  }
  ...
  update_favicon(total ? "/gnostr_notif-{{ crate::GNOSTR_NOTIF_SVG_HASH.get().unwrap() }}.svg" : "/gnostr-{{ crate::GNOSTR_SVG_HASH.get().unwrap() }}.svg")
  update_notification_markers(count, "activity")
  update_notification_markers(dm_count, "dm")
  // slight hack :)
  find_node("#header-tools button[action='mark-all-read']").disabled = dm_count == 0
}
...
function on_click_show_event_details(evid) {
  console.log("on_click_show_event_details called with evid:", evid)
  const model = GNOSTR
  const ev = model.all_events[evid]
  if (!ev) {
    console.log("Event not found for evid:", evid)
    return
  }
