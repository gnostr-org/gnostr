...
function format_content(model, ev) {
  ...
  if (ev.kind === KIND_REACTION) {
    if (ev.content === "" || ev.content === "+") return "❤️"
...
function fmt_profile_name(profile = {}, fallback = "Anonymous") {
  const name = profile.display_name || profile.user || profile.name || fallback
  return html`${name}`
}

function fmt_name(profile = { data: {} }) {
  const { data } = profile
  const name = (data && data.display_name) || (data && data.user) || (data && data.name) || fmt_pubkey(profile.pubkey)
  return html`${name}`
}
