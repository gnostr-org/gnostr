const nostrjs = (function nostrlib() {
  ...
  function RelayPool(relays, opts) {
    if (!(this instanceof RelayPool)) return new RelayPool(relays)
  ...
  function Relay(relay, opts = {}) {
    if (!(this instanceof Relay)) return new Relay(relay, opts)

    this.url = relay
    this.opts = opts

    if (opts.reconnect == null) opts.reconnect = true
  ...
  function init_websocket(me) {
    ...
    try {
      ...
      // Re-throw the error so the calling function can handle it more specifically.
      throw e
    }
    return new Promise((resolve, reject) => {
      ...
      ws.onclose = () => {
        if (me.onfn.close) me.onfn.close()
        if (me.reconnecting) return reject(new Error("close during reconnect"))
        if (!me.manualClose && me.opts.reconnect) reconnect(me)
      }
      ws.onerror = () => {
        if (me.onfn.error) me.onfn.error()
        if (me.reconnecting) return reject(new Error("error during reconnect"))
        if (me.opts.reconnect) reconnect(me)
      }
      ws.onopen = () => {
        if (me.onfn.open) me.onfn.open()
        else console.log("no onopen???", me)
  ...
  function hex_decode(str, buflen) {
    ...
    let slen = str.length
    while (slen > 1) {
      if (-1 == (c1 = char_to_hex(str[j])) || -1 == (c2 = char_to_hex(str[j + 1]))) return null
...
if (typeof module !== "undefined" && module.exports) module.exports = nostrjs
