...
export const RqbitWebUI = (props: { title: string; menuButtons?: JSX.Element[] }) => {
  ...
  const setTorrentsLoading = useTorrentStore((state) => state.setTorrentsLoading)
  const setRefreshTorrents = useTorrentStore((state) => state.setRefreshTorrents)
    ...
    <div className="dark:bg-gray-900 dark:text-gray-200 min-h-screen">
      ...
      <div className="relative">
        ...
        <div className="absolute top-0 start-0 pl-2 z-10">
          {props.menuButtons && props.menuButtons.map((b, i) => <span key={i}>{b}</span>)}
