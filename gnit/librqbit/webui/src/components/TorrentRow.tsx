...
export const TorrentRow: React.FC<{
  ...
  const statusIcon = (className: string) => {
    return <StatusIcon className={className} error={!!error} live={!!statsResponse?.live} finished={finished} />
  }
          ...
          <>
            <div>
              <ProgressBar
                ...
                label={error}
                variant={state == STATE_INITIALIZING ? "warn" : finished ? "success" : "info"}
              />
            </div>
