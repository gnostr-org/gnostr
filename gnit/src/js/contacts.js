...
function contacts_process_event(contacts, our_pubkey, ev) {
  if (ev.pubkey !== our_pubkey) return
  if (contacts.event && ev.created_at < contacts.event.created_at) return
  contacts.event = ev
...
async function dbcall(fn) {
  return new Promise((resolve, reject) => {
    var open = indexedDB.open("gnostr", 5)
    open.onupgradeneeded = (ev) => {
      const db = ev.target.result
      if (!db.objectStoreNames.contains("friends")) db.createObjectStore("friends", { keyPath: "pubkey" })
      if (!db.objectStoreNames.contains("events")) db.createObjectStore("events", { keyPath: "id" })
      if (!db.objectStoreNames.contains("settings")) db.createObjectStore("settings", { keyPath: "pubkey" })
    }
    open.onsuccess = (ev) => {
