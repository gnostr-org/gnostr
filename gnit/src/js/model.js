...
function model_process_event(model, relay, ev) {
  ...
  switch (ev.kind) {
    ...
    case KIND_RELAY_LIST:
      fn = model_process_event_relay_list
      break
  }
  if (fn) fn(model, ev, !!relay)
  ...
  if (nip34_kinds.has(ev.kind) && model.contacts.friends.has(ev.pubkey)) {
    ...
    switch (ev.kind) {
      case KIND_REPO_ANNOUNCE:
        log_info(`NIP-34 Repository Announcement from followed profile ${ev.pubkey}:\n${eventData}`)
        break
      case KIND_REPO_STATE_ANNOUNCE:
        log_info(`NIP-34 Repository State Announcement from followed profile ${ev.pubkey}:\n${eventData}`)
        break
      case KIND_REPO_PATCH:
        log_info(`NIP-34 Repository Patch from followed profile ${ev.pubkey}:\n${eventData}`)
        break
      case KIND_REPO_PULL_REQ:
        log_info(`NIP-34 Repository Pull Request from followed profile ${ev.pubkey}:\n${eventData}`)
        break
      case KIND_REPO_PULL_REQ_UPDATE:
        log_info(`NIP-34 Repository Pull Request Update from followed profile ${ev.pubkey}:\n${eventData}`)
        break
      case KIND_REPO_ISSUE:
      ...
      case KIND_REPO_STATUS_OPEN:
        ...
        break
      case KIND_REPO_STATUS_APPLIED:
        log_info(`NIP-34 Repository Status Applied from followed profile ${ev.pubkey}:\n${eventData}`)
        break
      case KIND_REPO_STATUS_CLOSED:
        log_info(`NIP-34 Repository Status Closed from followed profile ${ev.pubkey}:\n${eventData}`)
        break
      case KIND_REPO_STATUS_DRAFT:
        log_info(`NIP-34 Repository Status Draft from followed profile ${ev.pubkey}:\n${eventData}`)
        break
      default:
...
function model_que_profile(model, relay, pubkey) {
  ...
  if (que.profiles.indexOf(pubkey) >= 0) return
...
function model_process_event_metadata(model, ev, update_view) {
  ...
  if (profile.evid && evs[ev.id].created_at < evs[profile.evid].created_at) return
  profile.evid = ev.id
  profile.data = safe_parse_json(ev.content, "profile contents")
  if (update_view) view_timeline_update_profiles(model, profile.pubkey)
 ...
 */
async function model_process_event_relay_list(model, ev, update_view) {
  if (ev.kind !== KIND_RELAY_LIST) {
    log_warn("model_process_event_relay_list called with non-NIP-65 event kind:", ev.kind)
    return
  }
    ...
    if (tag[0] === "r" && tag[1]) {
      ...
      if (tag.length > 2) {
        if (tag[2] === "read") {
          ...
          policy = { write: true }
        } else if (tag[2] === "readwrite") {
          policy = { read: true, write: true }
        }
      }
      relays.push([url, policy])
    }
  }

  if (relays.length > 0) {
    await add_nip65_relays_to_db(ev.pubkey, relays)
    log_debug(`Stored NIP-65 relays for pubkey ${ev.pubkey}:`, relays)
  } else {
    log_debug(`No NIP-65 relays found in event ${ev.id} for pubkey ${ev.pubkey}.`)
  }
}
...
function model_process_event_dm(model, ev, update_view) {
  if (!event_is_dm(ev, model.pubkey)) return
  ...
  for (; i < dm.events.length; i++) {
    const b = dm.events[i]
    if (ev.created_at > b.created_at) break
  }
...
function model_process_event_reaction(model, ev, update_view) {
  ...
  if (!model.reactions_to[reaction.e]) model.reactions_to[reaction.e] = new Set()
  model.reactions_to[reaction.e].add(ev.id)
  if (update_view) view_timeline_update_reaction(model, ev)
}
...
function model_process_event_deletion(model, ev, update_view) {
  for (const tag of ev.tags) {
    if (tag.length >= 2 && tag[0] === "e" && tag[1]) {
      ...
      if (model.deleted[evid]) continue
...
function model_remove_reaction(model, evid, update_view) {
  ...
  if (!reaction) return
  if (model.reactions_to[reaction.e]) model.reactions_to[reaction.e].delete(target_ev.id)
  if (update_view) view_timeline_update_reaction(model, target_ev)
}

function model_is_event_deleted(model, evid) {
  // we've already know it's deleted
  if (model.deleted[evid]) return model.deleted[evid]
  ...
  function _settings_load(ev, resolve, reject) {
    ...
    req.onsuccess = (ev) => {
      ...
      if (settings) {
        ...
        if (settings.relays.length) model.relays = new Set(settings.relays)
...
async function model_load_nip34_events(model) {
  try {
    const events = await get_all_nip34_events_from_db()
    log_info(`Loaded ${events.length} NIP-34 events from cache.`)
...
function new_model() {
  const model = {
    ...
    relay_que: new Map(),
    relays: new Set(["wss://relay.nostr.band", "wss://relay.damus.io", "wss://nos.lol"]),
...
/**
 * Subscribes to NIP-34 events across all active relays.
 * Unsubscribes from any previous NIP-34 subscription before creating a new one.
 * @param {object} model The global model object.
 */
function subscribe_nip34_events(model) {
  if (model.nip34_sub_id) {
    log_debug(`Unsubscribing from old NIP-34 subscription: ${model.nip34_sub_id}`)
    model.pool.unsubscribe(model.nip34_sub_id)
  }
  ...
  model.nip34_sub_id = sub_id
}

async function test_and_add_local_relay(model) {
  const local_relay_url = "wss://relay.damus.io"
  try {
    const relay = nostrjs.Relay(local_relay_url, { reconnect: false })
    ...
    // Local relay not available or failed to connect
  }
}
