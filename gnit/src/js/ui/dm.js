function view_dm_update(model) {
  ...
  model.dms.forEach((dm, pubkey, m) => {
    if (!dm.events.length) return
    const i = arr_bsearch_insert(order, dm, dm_cmp)
    order.splice(i, 0, dm)
    if (!dm.needs_redraw) return
  ...
  for (let i = 0; i < order.length; i++) {
    ...
    if (dm.pubkey == xel.dataset.pubkey) continue
...
function onclick_dm(ev) {
  ...
  view_timeline_apply_mode(GNOSTR, VM_DM_THREAD, { pubkey: el.dataset.pubkey })
}
