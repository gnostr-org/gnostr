const PROFILE_FIELDS = ["name", "display_name", "picture", "banner", "website", "nip05", "lud06", "about"]

function open_profile(pubkey) {
  view_timeline_apply_mode(GNOSTR, VM_USER, { pubkey })
}
...
function init_profile() {
  ...
  find_node("button[name='message-user']", el).addEventListener("click", onclick_message_user)
  find_node("button[name='edit-profile']", el).addEventListener("click", onclick_edit_profile)
  find_node("button[name='copy-pk']", el).addEventListener("click", onclick_copy_pubkey)
  find_node("button[name='follow-user']", el).addEventListener("click", onclick_follow_user)
}

function onclick_message_user(ev) {
  const pubkey = ev.target.dataset.pubkey
  view_timeline_apply_mode(GNOSTR, VM_DM_THREAD, { pubkey })
}
...
function view_update_profile(model, pubkey) {
  ...
  find_node("button[name='edit-profile']", el).classList.toggle("hide", pubkey != model.pubkey)
...
function onclick_edit_profile() {
  const p = model_get_profile(GNOSTR, GNOSTR.pubkey)
...
function click_update_profile() {
  ...
  const p = Object.assign({}, model_get_profile(GNOSTR, GNOSTR.pubkey).data, {
