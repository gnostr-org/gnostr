...
pub fn sign_event(
    ...
    let event_id_bytes =
        blossom_rs::protocol::compute_event_id(&pubkey_hex, created_at, kind, &tags, content);
...
pub fn kind10063_server_list(hex_seckey: &str, servers: &[String]) -> Result<Value, SignError> {
    let tags = servers
        .iter()
