...
pub fn seckey_to_nsec(
	hex_seckey: &str,
) -> Result<String, Nip19Error> {
	let bytes = hex::decode(hex_seckey)
		.map_err(|_| Nip19Error::InvalidHex)?;
	if bytes.len() != 32 {
		return Err(Nip19Error::WrongLength);
	}
...
pub fn event_to_note(
	hex_event_id: &str,
) -> Result<String, Nip19Error> {
	let bytes = hex::decode(hex_event_id)
		.map_err(|_| Nip19Error::InvalidHex)?;
	if bytes.len() != 32 {
		return Err(Nip19Error::WrongLength);
	}
...
pub fn npub_to_pubkey(npub: &str) -> Result<String, Nip19Error> {
	let (hrp, data) =
		bech32::decode(npub).map_err(|_| Nip19Error::Bech32)?;
	if hrp.as_str() != "npub" {
...
pub fn nsec_to_seckey(nsec: &str) -> Result<String, Nip19Error> {
	let (hrp, data) =
		bech32::decode(nsec).map_err(|_| Nip19Error::Bech32)?;
	if hrp.as_str() != "nsec" {
		return Err(Nip19Error::WrongHrp);
	}
	if data.len() != 32 {
...
pub fn note_to_event(note: &str) -> Result<String, Nip19Error> {
	let (hrp, data) =
		bech32::decode(note).map_err(|_| Nip19Error::Bech32)?;
	if hrp.as_str() != "note" {
		return Err(Nip19Error::WrongHrp);
	}
	if data.len() != 32 {
...
pub fn decode_any(
	encoded: &str,
) -> Result<(String, String), Nip19Error> {
	let (hrp, data) =
		bech32::decode(encoded).map_err(|_| Nip19Error::Bech32)?;
...
impl std::fmt::Display for Nip19Error {
	fn fmt(
		...
		match self {
			...
			Self::WrongLength => {
				write!(f, "key must be exactly 32 bytes")
			}
			Self::WrongHrp => {
				write!(f, "unexpected bech32 human-readable part")
			}
