...
pub use types::{
	...
	Nip34EventItem, SortField,
};

use std::{cmp::Reverse, io::Stdout, path::PathBuf, time::Duration};

use blossom_rs::{
	BlobDescriptor, BlossomClient, BlossomSigner, Signer,
};
use crossterm::event::{
...
impl App {
	pub fn new(
		...
		Self {
			...
			filebrowser_cwd: std::env::current_dir()
			...
			nip34_status: "Press 'c' to connect to a NIP-34 relay."
				.into(),
	...
	pub fn apply(&mut self, msg: AppMsg) {
		match msg {
			...
			AppMsg::UploadDone(path, desc) => {
				...
				self.notification =
					Some(("Upload successful!".into(), false));
			...
			AppMsg::UploadBatchItemDone(idx, path, desc) => {
				...
				self.upload_filebrowser_select_next_after(&path);
				let all_done = self.upload_items.iter().all(|i| {
					matches!(
						i.status,
						BatchStatus::Done(_) | BatchStatus::Failed(_)
					)
				});
				if all_done {
					...
					self.upload_loading = false;
					self.upload_ok =
						self.upload_items.iter().all(|i| {
							matches!(i.status, BatchStatus::Done(_))
						});
					self.upload_msg = Some(format!(
			...
			AppMsg::UploadBatchItemError(idx, path, e) => {
				...
				if all_done {
					...
					self.upload_loading = false;
					self.upload_ok = false;
					self.upload_msg = Some(
						"Queued uploads finished with errors".into(),
					);
					self.notification =
			...
			AppMsg::MirrorError(e) => {
				self.notification =
					Some((format!("Mirror failed: {e}"), true));
			}
			...
			AppMsg::Nip94Published(relay) => {
				self.notification = Some((
					format!("NIP-94 event published to {relay}"),
					false,
				));
			}
			AppMsg::Nip94PublishError(e) => {
				self.notification = Some((
					format!("NIP-94 publish failed: {e}"),
					true,
				));
			}
	...
	pub fn git_scroll_down(&mut self, visible_lines: usize) {
		let max = self.git_output.len().saturating_sub(visible_lines);
		self.git_output_scroll =
	...
	pub fn start_upload(&mut self) {
		...
		if path_str.is_empty() {
			self.upload_msg = Some(
				"Enter a file path first (press i to edit).".into(),
			);
			self.upload_ok = false;
			return;
		}
		...
		tokio::spawn(async move {
			let signer = match Signer::from_secret_hex(&key) {
				Ok(s) => s,
				Err(e) => {
					tx.send(AppMsg::UploadError(format!(
						"invalid key: {e}"
					)))
					.ok();
					return;
			...
			match client.upload_file(&path, &mime).await {
				Ok(desc) => {
					...
					tx.send(AppMsg::UploadDone(path.clone(), desc))
						.ok();
				}
				Err(e) => {
	...
	pub fn filebrowser_load(&mut self) {
		...
		let sel =
			self.filebrowser_list.selected().unwrap_or(0).min(
				self.filebrowser_entries.len().saturating_sub(1),
			);
		self.filebrowser_list.select(
	...
	fn upload_filebrowser_select_next_after(
		&mut self,
		path: &std::path::Path,
	) {
		let cwd = path
	...
	fn start_upload_queue(&mut self) {
		...
		if self.secret_key.is_none() {
			self.notification = Some((
				"A secret key is required to upload.".into(),
				true,
			));
			return;
		}
		...
		for item in &mut self.upload_items {
			if matches!(
				item.status,
				BatchStatus::Pending | BatchStatus::Failed(_)
			) {
		...
		tokio::spawn(async move {
			...
			for (idx, path) in paths {
				...
				handles.push(tokio::spawn(async move {
					let _permit = permit;
					let signer =
						match Signer::from_secret_hex(&secret_key) {
							Ok(s) => s,
							Err(e) => {
								tx.send(
									AppMsg::UploadBatchItemError(
										idx,
					...
					let client =
						BlossomClient::new(vec![server], signer);
	...
	pub fn batch_filebrowser_load(&mut self) {
		...
		self.batch_filebrowser_list.select(
	...
	fn batch_filebrowser_sync_path(&mut self) {
		if let Some(idx) = self.batch_filebrowser_list.selected() {
			if let Some(entry) =
				self.batch_filebrowser_entries.get(idx)
			{
				self.batch_input =
					entry.path.to_string_lossy().into_owned();
			}
	...
	pub fn batch_filebrowser_enter(&mut self) {
		...
		let Some(entry) = self.batch_filebrowser_entries.get(idx)
		else {
			return;
		};
		if entry.name == "." {
	...
	pub fn download_filebrowser_load(&mut self) {
		...
		if let Ok(rd) =
			std::fs::read_dir(&self.download_filebrowser_cwd)
		{
			for entry in rd.flatten() {
				let e = FileBrowserEntry::new(entry.path());
				if e.is_dir {
		...
		self.download_filebrowser_list.select(
	...
	fn download_filebrowser_sync_path(&mut self) {
		if let Some(idx) = self.download_filebrowser_list.selected() {
			if let Some(entry) =
				self.download_filebrowser_entries.get(idx)
			{
				self.modal_input =
					self.download_path_for_entry(entry);
			}
		}
	}
	...
	pub fn download_filebrowser_scroll_up(&mut self) {
		let i =
			self.download_filebrowser_list.selected().unwrap_or(0);
		if i > 0 {
	...
	pub fn download_filebrowser_scroll_down(&mut self) {
		let max =
			self.download_filebrowser_entries.len().saturating_sub(1);
		let i =
			self.download_filebrowser_list.selected().unwrap_or(0);
		self.download_filebrowser_list
			.select(Some((i + 1).min(max)));
		self.download_filebrowser_sync_path();
	}

	pub fn download_filebrowser_enter(&mut self) {
		let Some(idx) = self.download_filebrowser_list.selected()
		else {
			return;
		};
		let Some(entry) = self.download_filebrowser_entries.get(idx)
		else {
			return;
		};
		if entry.name == "." {
	...
	pub fn refresh_status(&mut self) {
		...
		tokio::spawn(async move {
			let url =
				format!("{}/status", server.trim_end_matches('/'));
			...
			match client.get(&url).send().await {
				Ok(resp) if resp.status().is_success() => {
					match resp.json::<serde_json::Value>().await {
						...
						Err(e) => {
							tx.send(AppMsg::StatusError(format!(
								"parse: {e}"
							)))
							.ok();
						}
	...
	pub fn generate_keypair(&mut self) {
		...
		let nsec = nip19::seckey_to_nsec(&hex_secret)
			.unwrap_or_else(|_| "?".into());
		let pubkey = signer.public_key_hex();
		let npub = nip19::pubkey_to_npub(&pubkey)
			.unwrap_or_else(|_| "?".into());
	...
	pub fn prev_tab(&mut self) {
		self.tab =
			self.tab.checked_sub(1).unwrap_or(TAB_NAMES.len() - 1);
		self.on_tab_enter();
	}
	...
	pub fn copy_selected_sha256(&mut self) {
		...
		match arboard::Clipboard::new()
			.and_then(|mut cb| cb.set_text(sha.clone()))
		{
			Ok(()) => {
				self.notification =
					Some((format!("Copied SHA256: {sha}"), false))
			}
			Err(e) => {
				self.notification =
	...
	pub fn copy_selected_url(&mut self) {
		...
		let url = match &blob.url {
			...
			None => {
				self.notification =
					Some(("Selected blob has no URL.".into(), true));
				return;
			}
		};
		drop(visible);
			...
			.and_then(|mut cb| cb.set_text(url.clone()))
		{
			Ok(()) => {
				self.notification =
					Some((format!("Copied URL: {url}"), false))
			}
			Err(e) => {
				self.notification =
					Some((format!("Clipboard error: {e}"), true))
	...
	pub fn copy_keygen_field(&mut self, field: u8) {
		let Some(kp) = &self.keygen_data else {
			self.notification = Some((
				"Press g to generate a keypair first.".into(),
				true,
			));
			return;
		};
		...
		match arboard::Clipboard::new()
			.and_then(|mut cb| cb.set_text(value.clone()))
		{
			Ok(()) => {
				self.keygen_copied = Some(field);
				self.notification =
					Some((format!("Copied {label}: {value}"), false))
			}
			Err(e) => {
				self.notification =
					Some((format!("Clipboard error: {e}"), true))
	...
	fn visible_blobs(&self) -> Vec<&BlobDescriptor> {
		...
		match self.sort_field {
			SortField::Date => visible.sort_by_key(|b| {
				std::cmp::Reverse(b.uploaded.unwrap_or(0))
			}),
			SortField::Size => {
				visible.sort_by_key(|b| std::cmp::Reverse(b.size))
			}
			SortField::Hash => {
				visible.sort_by(|a, b| a.sha256.cmp(&b.sha256))
			}
	...
	pub fn prompt_mirror(&mut self) {
		if self.secret_key.is_none() {
			self.notification = Some((
				"A secret key is required to mirror.".into(),
				true,
			));
			return;
		}
	...
	pub fn confirm_mirror(&mut self) {
		...
		tokio::spawn(async move {
			let signer = secret_key
				...
				.and_then(|k| Signer::from_secret_hex(k).ok())
				.unwrap_or_else(Signer::generate);
			let auth_event = blossom_rs::auth::build_blossom_auth(
				&signer, "upload", None, None, "",
			);
			let auth_header =
				blossom_rs::auth::auth_header_value(&auth_event);
			let endpoint =
				format!("{}/mirror", server.trim_end_matches('/'));
				...
				.await
			{
				Ok(resp) if resp.status().is_success() => {
					match resp.json::<BlobDescriptor>().await {
						Ok(desc) => {
							tx.send(AppMsg::MirrorDone(desc)).ok()
						}
						Err(e) => tx
							.send(AppMsg::MirrorError(format!(
								"parse: {e}"
							)))
							.ok(),
					}
				...
				Ok(resp) => {
					...
					tx.send(AppMsg::MirrorError(format!(
						"server: {text}"
					)))
					.ok()
				}
	...
	pub fn refresh_admin(&mut self) {
		...
		if !self.admin_stats_loading {
			...
			tokio::spawn(async move {
				let url = format!(
					"{}/admin/stats",
					server2.trim_end_matches('/')
				);
				match reqwest::get(&url).await {
					Ok(r) if r.status().is_success() => {
						match r.json::<serde_json::Value>().await {
							Ok(v) => tx2
								.send(AppMsg::AdminStatsLoaded(v))
								.ok(),
							Err(e) => tx2
								.send(AppMsg::AdminStatsError(
									format!("parse: {e}"),
								))
								.ok(),
						}
					...
					Ok(r) => {
						...
						tx2.send(AppMsg::AdminStatsError(format!(
							"server: {t}"
						)))
						.ok()
					}
					Err(e) => tx2
						.send(AppMsg::AdminStatsError(format!(
							"request: {e}"
						)))
						.ok(),
				};
			...
			tokio::spawn(async move {
				...
				match reqwest::get(&url).await {
					Ok(r) if r.status().is_success() => {
						match r.json::<serde_json::Value>().await {
							Ok(v) => tx
								.send(AppMsg::AdminUsersLoaded(v))
								.ok(),
							Err(e) => tx
								.send(AppMsg::AdminUsersError(
									format!("parse: {e}"),
								))
								.ok(),
						}
					...
					Ok(r) => {
						let t = r.text().await.unwrap_or_default();
						tx.send(AppMsg::AdminUsersError(format!(
							"server: {t}"
						)))
						.ok()
					}
					Err(e) => tx
						.send(AppMsg::AdminUsersError(format!(
							"request: {e}"
						)))
						.ok(),
				};
		...
		tokio::spawn(async move {
			...
			match reqwest::get(&url).await {
				Ok(r) if r.status().is_success() => {
					match r.json::<serde_json::Value>().await {
						Ok(v) => {
							tx.send(AppMsg::RelayPolicyLoaded(v)).ok()
						}
						Err(e) => tx
							.send(AppMsg::RelayPolicyError(format!(
								"parse: {e}"
							)))
							.ok(),
					}
				...
				Ok(r) => {
					let t = r.text().await.unwrap_or_default();
					tx.send(AppMsg::RelayPolicyError(format!(
						"server: {t}"
					)))
					.ok()
				}
	...
	pub fn refresh_nip96(&mut self) {
		...
		tokio::spawn(async move {
			...
			let info_url =
				format!("{}/.well-known/nostr/nip96.json", base);
			match reqwest::get(&info_url).await {
					...
					.await
				{
					...
					Err(e) => tx
						.send(AppMsg::Nip96InfoError(format!(
							"parse: {e}"
						)))
						.ok(),
				},
			...
			if let Some(key) = &secret_key {
				if let Ok(signer) =
					blossom_rs::auth::Signer::from_secret_hex(key)
				{
					let auth_event =
						blossom_rs::auth::build_nip98_auth(
							&signer, &files_url, "GET",
						);
					let token = blossom_rs::auth::auth_header_value(
						&auth_event,
					);
			...
			match req.send().await {
				Ok(r) if r.status().is_success() => {
					match r.json::<serde_json::Value>().await {
						Ok(v) => {
							tx.send(AppMsg::Nip96FilesLoaded(v)).ok()
						}
						Err(e) => tx
							.send(AppMsg::Nip96FilesError(format!(
								"parse: {e}"
							)))
							.ok(),
					}
	...
	pub fn connect_nip34_relay(&mut self) {
		...
		if relay.is_empty() {
			self.nip34_status =
				"Enter a relay URL first (press 'r' to edit).".into();
			return;
		}
		self.nip34_connected = false;
		...
		tokio::spawn(async move {
			...
			let (mut ws, _) = match conn {
				Ok(pair) => pair,
				Err(e) => {
					tx.send(AppMsg::Nip34Error(format!(
						"connect failed: {e}"
					)))
					.ok();
					return;
			...
			let kinds: Vec<u64> = vec![
				30617, 30618, 1617, 1618, 1619, 1621, 1630, 1631,
				1632, 1633, 10317,
			];
			...
			loop {
				if std::time::Instant::now() > deadline {
					tx.send(AppMsg::Nip34Error(
						"session timeout — press 'c' to reconnect"
							.into(),
					))
					.ok();
					break;
				...
				.await
				{
					Ok(Some(Ok(WsMessage::Text(text)))) => {
						...
						if let Ok(arr) = serde_json::from_str::<
							serde_json::Value,
						>(&text)
						{
							if arr.get(0).and_then(|v| v.as_str())
								== Some("EVENT")
							{
		...
		tokio::spawn(async move {
			...
			for (idx, path) in paths {
				...
				handles.push(tokio::spawn(async move {
					...
					match client.upload_file(p, &mime).await {
						Ok(desc) => tx
							.send(AppMsg::BatchItemDone(idx, desc))
							.ok(),
						Err(e) => tx
							.send(AppMsg::BatchItemError(idx, e))
							.ok(),
					};
				}));
			}
...
pub fn draw_tabs(f: &mut Frame, app: &App, area: Rect) {
	let titles: Vec<Line> =
		TAB_NAMES.iter().map(|&t| Line::from(t)).collect();
...
pub fn draw_blobs_tab(f: &mut Frame, app: &mut App, area: Rect) {
	...
	if app.filter_mode {
		...
		let filter_bar =
			Paragraph::new(format!("/{}", app.filter_str))
	...
	match app.sort_field {
		SortField::Date => {
			visible.sort_by_key(|b| Reverse(b.uploaded.unwrap_or(0)))
		}
	...
	let type_header_label =
		if app.sort_field == SortField::ContentType {
	...
	let rows: Vec<Row> = visible
		...
		.map(|b| {
			...
			let ctype =
				b.content_type.clone().unwrap_or_else(|| "—".into());
			let uploaded = b
				.uploaded
				.map(format_unix_ts)
				.unwrap_or_else(|| "—".into());
...
pub fn draw_upload_tab(f: &mut Frame, app: &mut App, area: Rect) {
	...
	let nip94_line = Line::from(vec![
		...
		Span::raw(format!(
			": Publish NIP-94 event {nip94_toggle}  relay: "
		)),
	...
	if app.publish_relay_edit {
		f.set_cursor_position((
			chunks[1].x
				+ 1 + "  p: Publish NIP-94 event [x]  relay: ".len()
				as u16 + app.publish_relay.len() as u16,
			chunks[1].y + 1,
		));
	}
	...
	let hints_para = Paragraph::new(hints).block(
	...
	let queue_block =
		Block::default().borders(Borders::ALL).title(queue_title);
...
fn draw_git_panel(f: &mut Frame, app: &mut App, area: Rect) {
	let repo_name = app
		...
		.unwrap_or_else(|| {
			app.git_repo_path.to_string_lossy().into_owned()
		});
	...
	let running_marker =
		if app.git_action_running { " …" } else { "" };
	let title =
		format!(" git — {repo_name}{branch_label}{running_marker} ");
	...
	let scroll_hint = if total > visible_height {
		format!(" Output [{}/{}] ↑/↓ scroll ", scroll + 1, total)
	} else {
		" Output ".into()
	};
	...
	f.render_widget(
		Paragraph::new(lines).block(
			Block::default().borders(Borders::ALL).title(scroll_hint),
		),
		output_area,
	);
}
...
fn draw_upload_filebrowser(f: &mut Frame, app: &mut App, area: Rect) {
	...
	let cwd_label =
		app.filebrowser_cwd.to_string_lossy().into_owned();
	...
	let items: Vec<ListItem> = app
		...
		.iter()
		.map(|e| {
			upload_filebrowser_list_item(
				e,
				app.upload_queue_contains(&e.path),
			)
		})
		.collect();
...
fn upload_filebrowser_list_item(
	e: &FileBrowserEntry,
	queued: bool,
) -> ListItem<'static> {
	if e.name == "." {
...
pub fn draw_nips_tab(f: &mut Frame, app: &mut App, area: Rect) {
	...
	let nip_titles: Vec<Line> =
		NIP_TAB_NAMES.iter().map(|&t| Line::from(t)).collect();
	let sub_tabs = Tabs::new(nip_titles)
		...
		.select(app.nip_tab)
		.style(
			Style::default().fg(COLOR_DIM).bg(Color::Rgb(16, 32, 16)),
		)
		.highlight_style(
			Style::default()
				.fg(Color::Green)
...
pub fn draw_nip65_tab(f: &mut Frame, app: &App, area: Rect) {
	...
	f.render_widget(
		...
		.block(
			Block::default()
				.borders(Borders::ALL)
				.title(" Nostr Relay (for publishing kind:10002) "),
		),
		outer[0],
	);
...
pub fn draw_nipb7_tab(f: &mut Frame, app: &App, area: Rect) {
	...
	f.render_widget(
		...
		.block(
			Block::default()
				.borders(Borders::ALL)
				.title(" Nostr Relay (for publishing kind:10063) "),
		),
		outer[0],
	);
...
pub fn draw_nip96_tab(f: &mut Frame, app: &App, area: Rect) {
	...
	let info_text = if app.nip96_info_loading {
		...
		if let Some(api) =
			info.get("api_url").and_then(|v| v.as_str())
		{
			lines.push(format!("api_url:      {api}"));
		}
		if let Some(dl) =
			info.get("download_url").and_then(|v| v.as_str())
		{
		...
		if let Some(max) =
			info.get("max_byte_size").and_then(|v| v.as_u64())
		{
			let mb = max / (1024 * 1024);
			lines
				.push(format!("max_size:     {max} bytes ({mb} MB)"));
		}
...
pub fn draw_nip34_tab(f: &mut Frame, app: &mut App, area: Rect) {
	...
	f.render_widget(
		Paragraph::new(app.nip34_status.as_str())
	...
	match app.modal.clone() {
		...
		Some(Modal::Download { shas }) => {
			...
			let cwd_label = app
				...
				.into_owned();
			let max_cwd =
				popup_area.width.saturating_sub(20) as usize;
			let cwd_display = if cwd_label.len() > max_cwd {
				format!(
					"…{}",
					&cwd_label
						[cwd_label.len().saturating_sub(max_cwd)..]
				)
			} else {
				cwd_label
			};
			...
			let block = Block::default()
				...
				.border_style(if app.download_filebrowser_active {
					Style::default()
						.fg(COLOR_ACCENT)
						.add_modifier(Modifier::BOLD)
				} else {
			...
			if app.download_filebrowser_entries.is_empty()
...
pub fn draw_profile_tab(f: &mut Frame, app: &mut App, area: Rect) {
	...
	// ── Relay bar ────────────────────────────────────────────────────────────
	let relay_label = if app.profile_relay_edit {
		"Relay [editing]: "
	} else {
		"Relay: "
	};
	...
	let row_h = 3u16;
	let max_fields = ((rows_area.height as usize) / row_h as usize)
		.min(fields.len());

	let constraints: Vec<Constraint> =
		(0..max_fields).map(|_| Constraint::Length(row_h)).collect();
	let row_chunks = Layout::default()
		.direction(Direction::Vertical)
		.constraints(constraints)
		.split(rows_area);

	for (i, ((label, value), chunk)) in fields
		.iter()
		.take(max_fields)
		.zip(row_chunks.iter())
		.enumerate()
	{
		let is_active =
			i == app.profile_edit_field && app.profile_editing;
...
pub fn draw_help_popup(
	...
	let (tab_title, tab_lines): (&str, Vec<Line>) = match tab {
		...
		0 => (
			...
			vec![
				...
				kv(
					"  s                ",
					"Cycle sort (Date / Size / Hash / Type)",
				),
				kv(
					"  /                ",
		...
		1 => (
			...
			vec![
				kv(
					"  f                ",
					"Open browser at the current path",
				),
				kv("  i                ", "Edit file path manually"),
				kv(
					"  Tab              ",
					"Toggle browser / edit focus",
				),
				kv("  p                ", "Toggle NIP-94 publish"),
				kv("  R                ", "Edit relay URL"),
				kv(
					"  Enter            ",
					"Upload queued files / upload current path",
				),
				kv(
					"  Esc              ",
					"Cancel editing / go up or close browser",
				),
				Line::from(""),
				Line::from(Span::styled(
					"  File browser",
				...
				kv(
					"  Space            ",
					"Queue/remove file for batch upload",
				),
		...
		5 => match nip_tab {
			0 => (
				...
				vec![
					...
					kv(
						"  m                ",
						"Cycle marker (both/read/write)",
					),
					kv(
						"  R                ",
						"Set publish relay URL",
					),
					kv(
						"  P                ",
			...
			3 => (
				...
				vec![
					...
					kv(
						"  P                ",
						"Publish kind:10063 server list",
					),
					kv("  ↑ / ↓            ", "Move selection"),
				],
			),
		...
		7 => (
			...
			vec![
				kv(
					"  g                ",
					"Generate new BIP-340 keypair",
				),
				kv(
					"  1                ",
					"Copy secret key (hex) to clipboard",
				),
				kv(
					"  2                ",
					"Copy nsec (NIP-19 bech32) to clipboard",
				),
				kv(
					"  3                ",
					"Copy public key (hex) to clipboard",
				),
				kv(
					"  4                ",
					"Copy npub (NIP-19 bech32) to clipboard",
				),
			],
		),
		_ => ("", vec![]),
	...
	let popup_h =
		(lines.len() as u16 + 2).min(area.height.saturating_sub(4));
...
pub fn draw_docs_fullscreen(
	...
	let h2 = |s: &'static str| -> Line<'static> {
		Line::from(Span::styled(s, heading2))
	};
	let blank = || -> Line<'static> { Line::from("") };
	let note_ln = |s: &'static str| -> Line<'static> {
			...
			Event::Key(key) if key.kind == KeyEventKind::Press => {
					...
					match app.modal.clone() {
						...
						Some(Modal::Download { .. }) => {
							match key.code {
								...
								_ if app
									.download_filebrowser_active =>
								{
									match key.code {
										KeyCode::Up
										| KeyCode::Char('k') => app
											.download_filebrowser_scroll_up(
											),
										KeyCode::Down
										| KeyCode::Char('j') => app
											.download_filebrowser_scroll_down(
											),
										KeyCode::Backspace
										| KeyCode::Char('h')
										| KeyCode::Char('-') => {
				...
				if app.show_docs {
					match key.code {
						...
						KeyCode::Up | KeyCode::Char('k') => {
							app.docs_scroll =
								app.docs_scroll.saturating_sub(1);
						}
						KeyCode::Down | KeyCode::Char('j') => {
							app.docs_scroll =
								app.docs_scroll.saturating_add(1);
						}
						KeyCode::PageUp => {
							app.docs_scroll =
								app.docs_scroll.saturating_sub(20);
						}
						KeyCode::PageDown => {
							app.docs_scroll =
								app.docs_scroll.saturating_add(20);
						}
				...
				match app.tab {
					...
					1 => {
						...
						if app.filebrowser_active {
							match key.code {
								...
								KeyCode::Char(' ') => {
									app.upload_queue_toggle_selected()
								}
								KeyCode::Up | KeyCode::Char('k') => {
					...
					6 => match key.code {
						...
						KeyCode::Char('2') => {
							app.copy_keygen_field(2)
						}
						KeyCode::Char('3') => {
							app.copy_keygen_field(3)
						}
						KeyCode::Char('4') => {
							app.copy_keygen_field(4)
						}
						_ => {}
					},
					_ => {}
...
// ────────────────────────────────────────────
pub use helpers::{
	chrono_fmt_unix, days_to_ymd, decode_secret_key, encode_nsec,
	format_size, format_unix_ts, mime_from_path,
};
