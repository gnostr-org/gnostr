...
pub use crossterm;
pub use git::{
    FileBrowserEntry, GitAction, GitRepoInfo, GitRepoKind, GitRepoState,
    detect_git_info, /*, detect_git_repo*/
    find_git_root, run_git_command,
};
pub use ratatui;
pub use state::{TuiState, load_state, save_state, state_path};
pub use types::{AppMsg, BatchItem, BatchStatus, KeygenResult, Modal, Nip34EventItem, SortField};
...
pub const TAB_NAMES: &[&str] = &[
    " Blobs ", " Upload ", " Admin ", " Relay ", " NIPs ", " Status ", " Keygen ",
];

pub const NIP_TAB_NAMES: &[&str] = &[" NIP-65 ", " NIP-96 ", " NIP-34 ", " NIP-B7 ", " Profile "];
...
impl App {
    pub fn new(
        ...
        Self {
            ...
            batch_running: false,
            batch_filebrowser_cwd: std::env::current_dir().unwrap_or_else(|_| PathBuf::from("/")),
    ...
    pub fn apply(&mut self, msg: AppMsg) {
        match msg {
            ...
            AppMsg::DownloadBatchDone { downloaded, failed } => {
                let msg = if failed == 0 {
                    format!("Downloaded {downloaded} selected blobs.")
                } else {
                    format!("Downloaded {downloaded} selected blobs; {failed} failed.")
                };
            ...
            AppMsg::GitError(e) => {
                self.git_action_running = false;
                self.git_action_task = None;
                self.git_output = format!("error: {e}").lines().map(String::from).collect();
                self.git_output_scroll = 0;
            }
        }
    }
    ...
    pub fn git_scroll_down(&mut self, visible_lines: usize) {
        ...
        self.git_output_scroll = (self.git_output_scroll + 1).min(max);
    }
    ...
    pub fn run_git_action(&mut self, action: GitAction) {
        ...
        let handle = tokio::spawn(async move {
            let result = run_git_command(&repo, action, &commit_msg).await;
            match result {
    ...
    fn sort_filebrowser_entries(entries: &mut Vec<FileBrowserEntry>, sort: FileBrowserSort) {
        match sort {
    ...
    pub fn filebrowser_load(&mut self) {
        ...
        self.filebrowser_entries = Self::filebrowser_special_entries(&self.filebrowser_cwd)
            .into_iter()
            .chain(dirs)
            .chain(files)
            .collect();
        ...
        self.filebrowser_list
            .select(if self.filebrowser_entries.is_empty() {
                None
            } else {
                Some(sel)
            });
    ...
    pub fn batch_filebrowser_load(&mut self) {
        ...
        self.batch_filebrowser_entries =
            Self::filebrowser_special_entries(&self.batch_filebrowser_cwd)
                .into_iter()
                .chain(dirs)
                .chain(files)
                .collect();
    ...
    fn batch_filebrowser_sync_path(&mut self) {
        if let Some(idx) = self.batch_filebrowser_list.selected() {
            if let Some(entry) = self.batch_filebrowser_entries.get(idx) {
                self.batch_input = entry.path.to_string_lossy().into_owned();
            }
        }
    }
    ...
    pub fn batch_filebrowser_scroll_down(&mut self) {
        let max = self.batch_filebrowser_entries.len().saturating_sub(1);
        let i = self.batch_filebrowser_list.selected().unwrap_or(0);
    ...
    pub fn batch_filebrowser_parent(&mut self) {
        if let Some(parent) = self.batch_filebrowser_cwd.parent().map(|p| p.to_path_buf()) {
    ...
    pub fn download_filebrowser_load(&mut self) {
        ...
        Self::sort_filebrowser_entries(&mut dirs, self.download_filebrowser_sort);
        Self::sort_filebrowser_entries(&mut files, self.download_filebrowser_sort);
        ...
        self.download_filebrowser_list
            .select(if self.download_filebrowser_entries.is_empty() {
                None
            } else {
                Some(sel)
            });
    ...
    fn download_default_name(&self) -> String {
        match &self.modal {
            Some(Modal::Download { shas }) if shas.len() == 1 && !shas[0].is_empty() => {
                shas[0].clone()
            }
            _ => "blob".into(),
        }
    ...
    pub fn download_filebrowser_enter(&mut self) {
        ...
        if entry.is_dir {
            ...
            if self.download_is_batch() {
                self.modal_input = self.download_filebrowser_cwd.to_string_lossy().into_owned();
            } else {
    ...
    pub fn on_tab_enter(&mut self) {
        match self.tab {
            ...
            5 if self.status_data.is_none() && !self.status_loading => self.refresh_status(),
            _ => {}
        }
    }
    ...
    pub fn open_selected_blob(&mut self) {
        ...
        let url = match &blob.url {
            ...
            None => {
                ...
                format!("{}/{}", self.server.trim_end_matches('/'), blob.sha256)
            }
        };
        drop(visible);
        match open::that(&url) {
    ...
    pub fn confirm_download(&mut self) {
        let Some(Modal::Download { shas }) = self.modal.take() else {
            return;
        };
        ...
        tokio::spawn(async move {
            ...
            if batch_mode {
                if let Err(e) = tokio::fs::create_dir_all(&target_dir).await {
                    tx.send(AppMsg::DownloadError(format!("create dir: {e}")))
                        .ok();
                    return;
                }
            }
...
pub fn draw_blobs_tab(f: &mut Frame, app: &mut App, area: Rect) {
    ...
    let rows: Vec<Row> = visible
        ...
        .map(|b| {
            ...
            let sha = if b.sha256.len() > 64 {
                //detect width of parent widget
                format!("{}…{}", &b.sha256[..32], &b.sha256[b.sha256.len() - 28..])
            } else {
                b.sha256.clone()
            };
...
pub fn draw_upload_tab(f: &mut Frame, app: &mut App, area: Rect) {
    ...
    if app.upload_items.is_empty() {
        let placeholder =
            Paragraph::new("Space queues files here.").style(Style::default().fg(COLOR_DIM));
        ...
        let rows: Vec<Row> = app
            ...
            .collect();
        let table = Table::new(rows, [Constraint::Min(40), Constraint::Length(12)]).header(
            Row::new(vec![
    ...
    if app.upload_running {
        let loading =
            Paragraph::new("Uploading queued files…").style(Style::default().fg(COLOR_DIM));
        ...
        let placeholder = Paragraph::new("No upload yet.").style(Style::default().fg(COLOR_DIM));
...
fn draw_git_panel(f: &mut Frame, app: &mut App, area: Rect) {
    ...
    let split = Layout::default()
        .direction(Direction::Vertical)
        .constraints([Constraint::Length(menu_height), Constraint::Min(1)])
        .split(inner);
    ...
    if app.git_commit_edit {
        ...
        f.set_cursor_position((
            split[0].x + 1 + "msg: ".len() as u16 + app.git_commit_msg.len() as u16,
            split[0].y + 3,
        ));
    } else {
        f.render_widget(
    ...
    f.render_widget(
        Paragraph::new(lines).block(Block::default().borders(Borders::ALL).title(scroll_hint)),
        output_area,
    );
}
...
fn draw_upload_filebrowser(f: &mut Frame, app: &mut App, area: Rect) {
    ...
    if !app.filebrowser_active && app.filebrowser_entries.is_empty() {
        let hint =
            Paragraph::new("  Press f to browse files").style(Style::default().fg(COLOR_DIM));
...
fn render_git_hint(f: &mut Frame, area: Rect) {
    f.render_widget(
        Paragraph::new(Line::from(vec![
            ...
            Span::styled(": open git panel", Style::default().fg(COLOR_DIM)),
        ])),
        area,
    );
}
...
fn draw_batch_filebrowser(f: &mut Frame, app: &mut App, area: Rect) {
    ...
    let cwd_label = app.batch_filebrowser_cwd.to_string_lossy().into_owned();
    let max_cwd = area.width.saturating_sub(4) as usize;
    let cwd_display = if cwd_label.len() > max_cwd {
        format!("…{}", &cwd_label[cwd_label.len().saturating_sub(max_cwd)..])
    } else {
    ...
    if !app.batch_filebrowser_active && app.batch_filebrowser_entries.is_empty() {
        f.render_widget(
            Paragraph::new("  Press f to browse files").style(Style::default().fg(COLOR_DIM)),
            inner,
        );
    ...
    f.render_stateful_widget(list, list_area, &mut app.batch_filebrowser_list);
}
...
pub fn draw_nips_tab(f: &mut Frame, app: &mut App, area: Rect) {
    ...
    let nip_titles: Vec<Line> = NIP_TAB_NAMES.iter().map(|&t| Line::from(t)).collect();
    let sub_tabs = Tabs::new(nip_titles)
        ...
        .select(app.nip_tab)
        .style(Style::default().fg(COLOR_DIM).bg(Color::Rgb(16, 32, 16)))
        .highlight_style(
            Style::default()
                .fg(Color::Green)
...
pub fn draw_nip65_tab(f: &mut Frame, app: &App, area: Rect) {
    ...
    let list_title = format!(" NIP-65 Relay List — {} relays ", app.nip65_relays.len());
...
pub fn draw_status_bar(f: &mut Frame, app: &App, area: Rect) {
    let content = if let Some((msg, is_err)) = &app.notification {
        ...
        let hints = match app.tab {
            0 => {
                " Space:select  d:delete  o:download  m:mirror  s:sort  /:filter  y:copy-sha  u:copy-url  Enter:open  ↑↓/jk  Tab  ?  q"
            }
            1 => {
                if app.publish_relay_edit {
...
pub fn draw_modal_input(f: &mut Frame, app: &mut App, area: Rect) {
    match app.modal.clone() {
        ...
        Some(Modal::Download { shas }) => {
            ...
            let path =
                Paragraph::new(app.modal_input.as_str()).style(Style::default().fg(Color::White));
            ...
            let list = List::new(items)
                ...
                .highlight_symbol("› ");
            f.render_stateful_widget(list, chunks[2], &mut app.download_filebrowser_list);
        }
        Some(Modal::Mirror) => {
...
pub fn draw_profile_tab(f: &mut Frame, app: &mut App, area: Rect) {
    ...
    let constraints: Vec<Constraint> = (0..max_fields).map(|_| Constraint::Length(row_h)).collect();
    ...
    let hints =
        Paragraph::new(format!("{hint_left}  {hint_right}")).style(Style::default().fg(COLOR_DIM));
    ...
    if let Some(err) = &app.profile_error {
        ...
        f.render_widget(
            Paragraph::new(format!("⚠ {err_msg}")).style(Style::default().fg(COLOR_ERR)),
            err_area,
        );
    }
}
    ...
    let (tab_title, tab_lines): (&str, Vec<Line>) = match tab {
        ...
        2 => (
            ...
            vec![
                ...
                kv("  ↑ / k            ", "Navigate up"),
                kv("  ↓ / j            ", "Navigate down"),
                kv("  Enter            ", "Enter dir / add file to queue"),
                kv("  Backspace / h / -", "Go to parent directory"),
                kv("  Esc              ", "Go up (close at root)"),
                kv("  g                ", "Open git panel (on git repos)"),
                kv("  f                ", "Close file browser"),
...
pub fn draw_docs_fullscreen(f: &mut Frame, area: Rect, tab: usize, nip_tab: usize, scroll: u16) {
    ...
    let url_ln =
        |s: &'static str| -> Line<'static> { Line::from(Span::styled(format!("  → {s}"), dim)) };
    ...
    let title = match tab {
        ...
        t => format!(
            " blossom-tui docs — {} ",
            [
                "Blobs", "Upload", "Batch", "Admin", "Relay", "NIPs", "Status", "Keygen"
            ]
            .get(t)
            .copied()
            .unwrap_or("?")
            ...
            Event::Key(key) if key.kind == KeyEventKind::Press => {
                ...
                if app.modal.is_some() {
                    match app.modal.clone() {
                        Some(Modal::Delete { .. }) => match key.code {
                            KeyCode::Char('d') => {
                                if let Some(Modal::Delete { shas }) = app.modal.take() {
                        ...
                        Some(Modal::Download { .. }) => match key.code {
                            KeyCode::Tab => {
                                app.download_filebrowser_active = !app.download_filebrowser_active;
                            }
                            KeyCode::Char('o') => {
                            ...
                            _ if app.download_filebrowser_active => match key.code {
                                ...
                                KeyCode::Backspace | KeyCode::Char('h') | KeyCode::Char('-') => {
                ...
                if app.tab == 1 && app.filebrowser_active {
                    match key.code {
                        ...
                        KeyCode::Char('m') => {
                            app.filebrowser_set_sort(FileBrowserSort::Modified);
                            continue;
                        }
                        _ => {}
                    }
                ...
                match app.tab {
                    ...
                    1 => {
                        ...
                        if app.filebrowser_active {
                            match key.code {
                                KeyCode::Tab | KeyCode::Char('i') => app.upload_focus_input(),
                                KeyCode::Char(' ') => app.upload_queue_toggle_selected(),
                                KeyCode::Up | KeyCode::Char('k') => app.filebrowser_scroll_up(),
                                KeyCode::Down | KeyCode::Char('j') => app.filebrowser_scroll_down(),
                                ...
                                KeyCode::Char('f') => app.filebrowser_active = false,
                                _ => {}
                            }
                        ...
                        // 	}
                            ...
                            match key.code {
                                KeyCode::Char('f') => app.upload_focus_browser(),
                    ...
                    4 => {
                        ...
                        match key.code {
                            KeyCode::Char('[') => {
                                app.nip_tab = app.nip_tab.saturating_sub(1);
                            }
                            KeyCode::Char(']') => {
                                app.nip_tab = (app.nip_tab + 1).min(NIP_TAB_NAMES.len() - 1);
                            }
                            _ => {
                                match app.nip_tab {
                                    // NIP-65 Relay List
                                    0 => {
                                        if app.nip65_relay_edit {
                                            ...
                                            match key.code {
                                                KeyCode::Enter => {
                                                    let url = app.nip65_input.trim().to_string();
                                                    if !url.is_empty() {
                                                        let marker = app.nip65_marker.clone();
                                                        app.nip65_relays.push((url, marker));
                                                    }
                                            ...
                                            match key.code {
                                                ...
                                                KeyCode::Char('m') => {
                                                    ...
                                                    app.nip65_marker = match app.nip65_marker_idx {
                                                        0 => "".into(),
                                                        1 => "read".into(),
                                                        _ => "write".into(),
                                                    };
                                                    let sel = app.nip65_selected;
                                                    if let Some(r) = app.nip65_relays.get_mut(sel) {
                                                ...
                                                KeyCode::Char('P') => {
                                                    if let Some(sk) = &app.secret_key {
                                                        let relays = app.nip65_relays.clone();
                                                ...
                                                KeyCode::Down | KeyCode::Char('j') => {
                                                    if !app.nip65_relays.is_empty() {
                                                        app.nip65_selected = (app.nip65_selected
                                                            + 1)
                                                        .min(app.nip65_relays.len() - 1);
                                                    }
                                                }
                                                _ => {}
                                            }
                                    ...
                                    3 => {
                                        if app.nipb7_relay_edit {
                                            ...
                                            match key.code {
                                                KeyCode::Enter => {
                                                    let url = app.nipb7_input.trim().to_string();
                                                    if !url.is_empty() {
                                            ...
                                            match key.code {
                                                ...
                                                KeyCode::Char('P') => {
                                                    if let Some(sk) = &app.secret_key {
                                                        let servers = app.nipb7_servers.clone();
                                                ...
                                                KeyCode::Down | KeyCode::Char('j') => {
                                                    if !app.nipb7_servers.is_empty() {
                                                        app.nipb7_selected = (app.nipb7_selected
                                                            + 1)
                                                        .min(app.nipb7_servers.len() - 1);
                                                    }
                                                }
                                                _ => {}
                                            }
                                    ...
                                    _ => {
                                        if app.profile_relay_edit {
                                            ...
                                            match key.code {
                                                ...
                                                KeyCode::Char(c) => match app.profile_edit_field {
                                            ...
                                            match key.code {
                                                KeyCode::Up => {
                                                    app.profile_edit_field =
                                                        app.profile_edit_field.saturating_sub(1);
                                                }
                                                KeyCode::Down => {
                                                    app.profile_edit_field =
                                                        (app.profile_edit_field + 1).min(5);
                                                ...
                                                KeyCode::Char('P') => {
                                                    if let Some(sk) = &app.secret_key {
                                                        ...
                                                        match crate::nostr_sign::kind0_metadata(
                                                            sk, &meta,
                                                        ) {
                                                            Ok(ev) => {
                                                                app.notification = Some((
