...
use crate::{App, SortField, NIP_TAB_NAMES, TAB_NAMES};
...
impl TuiState {
	...
	pub fn clamp_tabs(&mut self) {
		...
		if let Some(n) = self.nip_tab {
			self.nip_tab =
				Some(n.min(NIP_TAB_NAMES.len().saturating_sub(1)));
		}
	}
}
...
pub fn save_state(
	...
	let path =
		state_path().ok_or("cannot determine state file path")?;
...
impl App {
	...
	pub fn apply_state(&mut self, state: &TuiState) {
		...
		if let Some(n) = state.nip_tab {
			self.nip_tab =
				n.min(NIP_TAB_NAMES.len().saturating_sub(1));
		}
