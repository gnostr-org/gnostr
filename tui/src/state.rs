...
use crate::{App, NIP_TAB_NAMES, SortField, TAB_NAMES};
...
impl App {
    ...
    pub fn to_state(&self) -> TuiState {
        ...
        TuiState {
            ...
            upload_filebrowser_cwd: Some(self.filebrowser_cwd.to_string_lossy().into_owned()),
            batch_filebrowser_cwd: Some(self.batch_filebrowser_cwd.to_string_lossy().into_owned()),
    ...
    pub fn apply_state(&mut self, state: &TuiState) {
        ...
        if let Some(v) = &state.profile_name {
            self.profile_name = v.clone();
        }
        if let Some(v) = &state.profile_about {
            self.profile_about = v.clone();
        }
        if let Some(v) = &state.profile_picture {
            self.profile_picture = v.clone();
        }
        if let Some(v) = &state.profile_nip05 {
            self.profile_nip05 = v.clone();
        }
        if let Some(v) = &state.profile_website {
            self.profile_website = v.clone();
        }
        if let Some(v) = &state.profile_lud16 {
            self.profile_lud16 = v.clone();
        }
