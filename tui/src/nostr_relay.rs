...
pub fn spawn(
	url: String,
) -> (mpsc::Sender<RelayCmd>, mpsc::Receiver<RelayMsg>) {
	...
	match verb {
		"EVENT" => {
			...
			if let Some(ev_val) = arr.get(2) {
				if let Ok(event) = serde_json::from_value::<RelayEvent>(
					ev_val.clone(),
				) {
					let _ = tx
						.send(RelayMsg::Event { sub_id, event })
						.await;
				}
			}
		}
		...
		"OK" => {
			let event_id = arr
				...
				.to_owned();
			let accepted =
				arr.get(2).and_then(|v| v.as_bool()).unwrap_or(false);
			let message = arr
				.get(3)
				.and_then(|v| v.as_str())
				.unwrap_or("")
