...
fn main() {
	...
	let build_legit_command = if cfg!(target_os = "windows") {
		Command::new("cmd")
			.args(["/C", "cargo", "install", "--path", "legit"])
			.output()
			.expect("failed to execute process")
	} else if cfg!(target_os = "macos") {
		Command::new("sh")
			.arg("-c")
			.args(["cargo", "install", "--path", "legit"])
			.output()
			.expect("failed to execute process")
	...
	let _build_legit = String::from_utf8(build_legit_command.stdout)
		.map_err(|non_utf8| {
