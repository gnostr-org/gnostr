...
async fn get_github_token_from_machine(
    ...
    if !*tried_credentials {
        if let Some(token_possible) =
            find_regex_in_file_lines(&path!("~/.git-credentials"), re(r"(ghp_[a-zA-Z0-9]+):?"), 0)
                .await
        {
...
async fn octocrab_ssh_keys_create(
    ...
    let octo_key: OctocrabModelSshKey = octocrab_client
        .post(
            "/user/keys",
...
async fn get_or_create_ssh_key(
    ...
    match create_ssh_key_on_machine().await {
        Ok(private_key_possible) => {
            match octocrab_ssh_keys_create(octocrab_client, &private_key_possible).await {
                Ok(k) => Ok(private_key_possible),
                Err(e) => return_cmd_err!(
                    "Failed to save GitHub SSH key, that was just created, error: {e}"
                ),
            }
        }
...
async fn octocrab_repos_create(
    ...
    let gh_repo: octocrab::models::Repository = octocrab_client
        .post(
            "/user/repos",
            Some(&serde_json::json!({
                "private": !*is_public,
...
async fn git2_add_and_commit<'repo, 'sig>(
    ...
    let commit_id = repo.commit(Some("HEAD"), signature, signature, message, &tree, &pref)?;
...
fn git2_credentials(
    ...
    if allowed.contains(git2::CredentialType::SSH_MEMORY) {
        git2::Cred::ssh_key_from_memory(
            ...
            Some(
                ssh_private_key
                    .public_key()
                    .to_openssh()
                    .expect("must be able to convert to openssh format")
                    .deref(),
            ),
            ssh_private_key
        ...
        Err(git2::Error::from_str(
            format!("no callbacks for type: {allowed:?}").as_str(),
        ))
    }
}

fn git2_remote_delete(repo: &git2::Repository, remote_name: &str) -> Result<(), git2::Error> {
    ...
    fn remove_refs(repo: &git2::Repository, spec: &git2::Refspec) -> Result<(), git2::Error> {
        ...
        for reference in repo.references()? {
            ...
            if !spec.dst_matches(reference_name) {
                continue;
            }
...
fn git2_default_branch_name(repo: &git2::Repository) -> GhRsResult<String> {
    let head = repo.head()?;
...
fn git2_push(
    ...
    if *mirror {
        repo.remote_add_push(
            remote_name,
            format!("refs/remotes/{remote_name}/*:refs/heads/*").as_str(),
        )?;
...
async fn git2_fetch_until_commit<'repo>(
    ...
    if repo.find_commit(commit_id.to_owned()).is_ok() {
        return Ok(());
    }
    ...
    loop {
        let token_raw = match token_raw_opt {
            ...
            Some(token_raw_possible) => {
                if !tried_opt
                    && token_raw_possible.len() == 40
                    && re_token().is_match(token_raw_possible)
                {
                    tried_opt = true;
        ...
        match octocrab_client.current().user().await {
            ...
            Err(e) => {
                aprintln!(
                    "Invalid answer received from API, possible access token is wrong, error: {e}"
                );
                if *cli_only {
...
async fn get_repo_name(
    ...
    async fn prompt_repo_name(prompt_default: Option<&str>) -> String {
        loop {
            let repo_name_answer = prompt("GitHub new repo name", prompt_default)
                .await
                .trim()
                .to_string();
    ...
    loop {
        ...
        match octocrab_client
            .repos(username, repo_name_raw.clone())
            .get()
            .await
        {
            Err(e) => {
...
async fn get_repo_description(
    ...
    async fn prompt_repo_description() -> String {
        loop {
            ...
            aprintln!(
                "Too long description, maximum lenght 255, now: {len}",
                len = repo_description_answer.len()
            );
            continue;
        }
    }
...
fn resolve_github_path<'path>(github_path_or_url: &'path str) -> Option<(&'path str, &'path str)> {
    ...
    if !re_repo_name().is_match(repo_name) {
        return None;
    }
...
async fn get_external_path(
    ...
    async fn prompt_external_path() -> (String, String) {
        loop {
            let external_url_answer = prompt("GitHub external repo url", None)
                .await
                .trim()
                .to_string();
            match resolve_github_path(&external_url_answer) {
                Some((username_str, repo_name_str)) => {
                    return (username_str.to_string(), repo_name_str.to_string())
                }
                None => {
                    aprintln!(
    ...
    loop {
        let (external_username, external_repo_name) = match external_path_raw_opt {
            Some(external_path_raw_possible) => {
                match resolve_github_path(external_path_raw_possible) {
                    None => {
                        aprintln!(
                            "Invalid url format, expected {ex_url} or just {ex_repo_path}",
                            ex_url = get_gh_rs_github_url(),
                    ...
                    Some((external_username_str, external_repo_name_str)) => (
                        external_username_str.to_string(),
                        external_repo_name_str.to_string(),
                    ),
                }
            }
            None => match *cli_only {
        ...
        match octocrab_client
            .repos(external_username.clone(), external_repo_name.clone())
            .get()
            .await
        {
            Err(e) => {
...
fn get_readme_text(username: &str, repo_name: &str, repo_description: &str) -> String {
    format!(
        "# {repo_name}
**{repo_description}**
...
async fn run_new_cmd(
    ...
    let _ = match fs::write(
        &readme_path,
        get_readme_text(&username, &repo_name, &repo_description).as_bytes(),
    )
    .await
    {
        Ok(r) => r,
    ...
    let _ = match git2_add_all_and_commit(&repo, &sig, ["."], "Initial commit [gh.rs]").await {
...
async fn run_publish_cmd(
    ...
    let repo_path = path!("./")
        .canonicalize()
        .await
        .expect("must be able to canonicalize");
    let repo_name = repo_path
        .file_name()
        .expect("must be able to get directory name")
        .to_str()
        .expect("must be able decode directory name as utf8");
    let branch_name = "main";
    let repo = match repo_path.join(".git").is_dir().await {
        ...
        false => match git2::Repository::init_opts(
            &repo_path,
            git2::RepositoryInitOptions::new().initial_head(branch_name),
        ) {
            Ok(r) => r,
            Err(e) => return_cmd_err!(
                "Failed to init repo in \"{repo_path_display}\", error: {e}",
...
async fn run_clone_cmd(
    ...
    let (octocrab_client, token, user) = get_auth(token_raw_opt, cli_only).await?;
    let username = user.login.clone();
    let (external_username, external_repo_name, external_gh_repo) =
        get_external_path(&octocrab_client, external_path_raw_opt, cli_only).await?;
...
async fn run_fork_cmd(
    ...
    let last_commit_id = external_repo
        .head()
        .expect("has head")
        .peel_to_commit()
        .expect("has commit")
    ...
    let _ = match git2_set_branch_upstream(&repo, default_branch_name.as_str(), remote_name) {
        Ok(r) => r,
        Err(e) => {
            return_cmd_err!("Failed to set \"{default_branch_name}\" branch upstream, error: {e}")
        }
    };
...
fn cmd_help_expanded_subcommands(
    ...
    fn subcmd_usage(cmd: &clap::Command) -> String {
        cmd.clone().render_usage().to_string().replace(
            format!("Usage: {}", cmd.get_name()).as_str(),
            subcmd_names(cmd).join(", ").as_str(),
        )
    }
    let longest = subcommands.clone().into_iter().fold(2, |m, cmd| {
        std::cmp::max(m, display_width(subcmd_usage(&cmd).as_str()))
    });
    ...
    for (i, subcmd) in subcommands.into_iter().enumerate() {
        ...
        let _ = write!(
            ...
            about = subcmd
                .get_about()
                .unwrap_or(&(clap::builder::StyledStr::new())),
        );
    }
...
async fn async_main() {
    ...
    let description_arg =
        arg!(description: -d --description <description> "Set new repo description")
            .visible_alias("descr");
    let public_arg = arg!(public: -p --public "Make repo public").visible_aliases([
        "pub",
        "is-public",
        "make-public",
    ]);
    ...
    let after_help = format!("gh.rs GitHub: {url}", url = get_gh_rs_github_url(),);
    ...
    let publish_cmd = cmd!(-p -pub --publish "Publish current directory to GitHub")
        .args([&description_arg, &public_arg, &token_arg, &cli_only_arg])
        .after_help(&after_help);

    let clone_cmd = cmd!(-c --clone "Clone GitHub repo")
        .args([&external_arg, &token_arg, &cli_only_arg])
        .after_help(&after_help);
    ...
    let help_full_cmd =
        cmd!(--"help-full" "Print help fully, describing every command").disable_help_flag(true);

    let subcommands = [new_cmd, publish_cmd, clone_cmd, fork_cmd, help_full_cmd];
