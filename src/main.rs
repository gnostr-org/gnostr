...
async fn main() -> anyhow::Result<()> {
    ...
    match &gnostr_cli_args.command {
        Some(GnostrCommands::Chat(sub_command_args)) => {
            ...
            sub_commands::chat::chat(&mut sub_command_args.clone())
                .await
        ...
        Some(GnostrCommands::Tui(sub_command_args)) => {
            ...
            if let Ok(gitdir_env_value) = env::var("GNOSTR_GITDIR") {
                ...
                if let Some(git_dir_value) = gitdir_value {
                    ...
                    result = sub_commands::tui::tui(sub_command_args_mut.clone())
                        .await
                ...
                result = sub_commands::tui::tui(sub_command_args.clone())
                    .await
                    .map_err(|e| anyhow!("Error in tui subcommand: {}", e));
            }
        ...
        Some(GnostrCommands::Gitsh(sub_command_args)) => {
            debug!("sub_command_args:{:?}", sub_command_args);
            sub_commands::gitsh::gitsh(sub_command_args)
                .await
                .map_err(|e| anyhow!("Error in gitsh subcommand: {}", e))
        }
        None => {
            // TODO handle more scenarios
            // Call tui with default commands and propagate its result
            sub_commands::tui::tui(gnostr::core::GnostrSubCommands::default())
                .await
