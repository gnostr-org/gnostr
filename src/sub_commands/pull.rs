...
pub async fn launch() -> Result<()> {
	...
	let logged_in_public_key = if let Ok(Some(npub)) =
	...
	let proposal_root_event = get_proposals_and_revisions_from_cache(
		git_repo_path,
	...
	.iter()
	.find(|e| {
		is_event_proposal_root_for_branch(
			e,
			&branch_name,
	...
	.context(
		"cannot find proposal that matches the current branch name",
	)?
	.clone();
		...
		.does_commit_exist(&proposal_base_commit.to_string())?
	{
		...
		println!(
			"manually run `git pull` on '{main_branch_name}' and try again"
		);
	}
	// if new revision and no local changes (tip of local in proposal
	// history)
