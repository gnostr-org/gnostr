...
use crate::{
	...
	cli_interactor::{
		...
		PromptInputParms, PromptMultiChoiceParms,
	},
	client::{
...
pub async fn launch(
	...
	let (main_branch_name, main_tip) =
		git_repo.get_main_or_master_branch().context(
	...
	let (root_proposal_id, mention_tags) =
		get_root_proposal_id_and_mentions_from_in_reply_to(
			git_repo.get_path()?,
			&args.in_reply_to,
		)
		.await?;
	...
	let mut commits: Vec<Sha1Hash> = {
		if args.since_or_range.is_empty() {
			...
			git_repo
				...
				.context(
					"cannot parse specified starting commit or range",
				)?
		}
	};
	...
	println!(
		...
		if cover_letter_title_description.is_none()
			&& events.len().eq(&1)
			|| cover_letter_title_description.is_some()
				&& events.len().eq(&2)
		{
			""
		} else {
	...
	if root_proposal_id.is_none() {
		if let Some(event) = events.first() {
			let event_bech32 =
				if let Some(relay) = repo_ref.relays.first() {
					Nip19Event::new(event.id(), vec![relay])
						.to_bech32()?
				} else {
					event.id().to_bech32()?
				};
...
fn choose_commits(
	...
	let tip_of_head = git_repo.get_tip_of_branch(
		&git_repo.get_checked_out_branch_name()?,
	)?;
	let most_recent_commit =
		proposed_commits.first().unwrap_or(&tip_of_head);

	let mut last_15_commits = vec![*most_recent_commit];

	while last_15_commits.len().lt(&15) {
		if let Ok(parent_commit) = git_repo
			.get_commit_parent(last_15_commits.last().unwrap())
		{
	...
	let selected_commits = 'outer: loop {
			...
			PromptMultiChoiceParms::default()
				...
				.with_choices(
					last_15_commits
						.iter()
						.map(|h| {
							summarise_commit_for_selection(
								git_repo, h,
							)
							.unwrap()
						})
				...
				.with_defaults(
					last_15_commits
						.iter()
						.map(|h| {
							proposed_commits.iter().any(|c| c.eq(h))
						})
						.collect(),
				),
		)?;
		proposed_commits =
			selected.iter().map(|i| last_15_commits[*i]).collect();
		...
		for (i, selected_i) in selected.iter().enumerate() {
			if i.gt(&0) && selected_i.ne(&(selected[i - 1] + 1)) {
				term.write_line(
					"commits must be consecutive. try again.",
				)?;
				printed_error_line = true;
				continue 'outer;
			}
...
fn summarise_commit_for_selection(
	git_repo: &Repo,
	commit: &Sha1Hash,
) -> Result<String> {
	...
	let dim = Style::new().color256(247);
	let prefix =
		format!("({})", git_repo.get_commit_author(commit)?[0],);
...
async fn get_root_proposal_id_and_mentions_from_in_reply_to(
	...
	let root_proposal_id = if let Some(first) = in_reply_to.first() {
		match event_tag_from_nip19_or_hex(
			first,
			"in-reply-to",
			Marker::Root,
			true,
		...
		.as_standardized()
		{
			Some(nostr_sdk::TagStandard::Event {
				...
				if let Some(first) =
					events.iter().find(|e| e.id.eq(event_id))
				{
