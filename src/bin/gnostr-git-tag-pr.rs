...
fn run(args: Vec<String>, weeble_output: &str, blockheight_output: &str, wobble_output: &str, repo_path: &Path) -> Result<()> {
    ...
    if args.len() > 1 {
        tag_name = format!("pr/{}-{}", tag_name, args[1]);
    }
...
mod tests {
    ...
    #[test]
    //#[ignore]
    fn test_git_tag_pr_no_arg() -> Result<()> {
        ...
        let expected_tag_name = format!("pr/{}/{}/{}/{}/{}", weeble, blockheight, wobble, parent_head, current_head);
    ...
    #[test]
    fn test_git_checkout_pr_with_arg() -> Result<()> {
        let dir = setup_test_repo();
        let repo_path = dir.path();
        ...
        let suffix = "feature";
        let expected_tag_name = format!("pr/1/1/1/{}/{}-{}", parent_head, current_head, suffix);
