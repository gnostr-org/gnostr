...
pub async fn launch(
			...
			.input(
				PromptInputParms::default()
					...
					.with_default(
						...
						{
							if let Ok(fetch_url) =
								convert_clone_url_to_https(&url)
							{
								fetch_url
							} else {
	...
	let maintainers: Vec<PublicKey> = {
		...
		let mut maintainers_string = if !args
			.other_maintainers
			.is_empty()
		{
		...
		'outer: loop {
			...
			for m in maintainers_string.split(' ') {
				if let Ok(m_pubkey) = PublicKey::from_bech32(m) {
					...
					println!(
						"not a valid set of npubs seperated by a space"
					);
					dont_ask = false;
					continue 'outer;
				}
	...
	let earliest_unique_commit = match &args.earliest_unique_commit {
		Some(t) => t.clone(),
		None => {
			let mut earliest_unique_commit =
				if let Some(repo_ref) = &repo_ref {
			...
			loop {
				earliest_unique_commit = Interactor::default()
					.input(
						PromptInputParms::default()
							.with_prompt("earliest unique commit")
							.with_default(
								earliest_unique_commit.clone(),
							),
					)?;
				if let Ok(exists) = git_repo
					.does_commit_exist(&earliest_unique_commit)
				{
					if exists {
						break earliest_unique_commit;
					}
					println!(
						"commit does not exist on current repository"
					);
				} else {
