...
use ngit::{
	...
	git_events::{
		...
		get_most_recent_patch_with_ancestors, status_kinds,
		tag_value,
	},
};
use nostr_sdk::Kind;
...
use crate::{
	cli_interactor::{
		Interactor, InteractorPrompt, PromptChoiceParms,
		PromptConfirmParms,
	},
	client::{
		Client, Connect, fetching_with_report, get_events_from_cache,
		get_repo_ref_from_cache,
	},
	git::{Repo, RepoActions, str_to_sha1},
	git_events::{
		commit_msg_from_patch_oneliner, event_is_revision_root,
...
pub async fn launch() -> Result<()> {
	...
	fetching_with_report(git_repo_path, &client, &repo_coordinates)
		.await?;
		...
		let status = if let Some(e) = statuses
			...
			.filter(|e| {
				status_kinds().contains(&e.kind())
					&& e.tags().iter().any(|t| {
						t.as_vec()[1].eq(&proposal.id.to_string())
					})
			})
	...
	loop {
		let proposals_for_status =
			if selected_status == Kind::GitStatusOpen {
				&open_proposals
		...
		if !selected_status.eq(&Kind::GitStatusOpen)
			&& open_proposals.len().gt(&0)
		{
			choices.push(format!(
				"({}) Open proposals...",
				open_proposals.len()
		...
		if !selected_status.eq(&Kind::GitStatusDraft)
			&& draft_proposals.len().gt(&0)
		{
			choices.push(format!(
				"({}) Draft proposals...",
				draft_proposals.len()
		...
		if !selected_status.eq(&Kind::GitStatusClosed)
			&& closed_proposals.len().gt(&0)
		{
			choices.push(format!(
				"({}) Closed proposals...",
		...
		if !selected_status.eq(&Kind::GitStatusApplied)
			&& applied_proposals.len().gt(&0)
		{
			choices.push(format!(
				"({}) Applied proposals...",
		...
		let cover_letter = event_to_cover_letter(
			proposals_for_status[selected_index],
		)
		.context(
			"cannot extract proposal details from proposal root event",
		)?;
		...
		let patch_text_ref =
			if most_recent_proposal_patch_chain.len().gt(&1) {
				binding_patch_text_ref.as_str()
			} else {
				"1 commit"
			};

		let no_support_for_patches_as_branch =
		...
		let branch_exists = git_repo
			...
			.context(
				"gitlib2 will not show a list of local branch names",
			)?
			.iter()
		...
		let (main_branch_name, master_tip) =
			git_repo.get_main_or_master_branch()?;

		if !git_repo
			.does_commit_exist(&proposal_base_commit.to_string())?
		{
			println!(
				"your '{main_branch_name}' branch may not be up-to-date."
			);
			println!(
				"the proposal parent commit doesnt exist in your local repository."
			);
		...
		let (_, proposal_behind_main) = git_repo
		...
		let local_branch_tip = git_repo
			.get_tip_of_branch(&cover_letter.get_branch_name()?)?;
		...
		let (local_ahead_of_main, local_beind_main) = git_repo
			.get_commits_ahead_behind(
				&master_tip,
				&local_branch_tip,
			)?;
		...
		if let Some(index) = most_recent_proposal_patch_chain
			.iter()
			.position(|patch| {
			...
			return match Interactor::default().choice(
				PromptChoiceParms::default()
					...
					.with_choices(vec![
						format!(
							"checkout proposal branch and apply {} appendments",
							&index,
						),
						format!(
				...
				0 => {
					check_clean(&git_repo)?;
					git_repo
						.checkout(&cover_letter.get_branch_name()?)?;
					let _ = git_repo
						.apply_patch_chain(
							&cover_letter.get_branch_name()?,
							most_recent_proposal_patch_chain,
		...
		// made to up-to-date proposal)
			...
			.ancestor_of(&local_branch_tip, &proposal_tip)?
		{
		...
		println!(
			"to view the latest proposal but retain your changes:"
		);
		println!(
			"  1) create a new branch off the tip commit of this one to store your changes"
		);
		println!(
			"  2) run `ngit list` and checkout the latest published version of this proposal"
		);

		println!(
			"if you are confident in your changes consider running `ngit push --force`"
		);
...
fn launch_git_am_with_patches(
	...
	for patch in patches {
		stdin
			...
			.context(
				"failed to write patch content into git am stdin buffer",
			)?;
	}
	stdin.flush()?;
	let output = am
...
fn save_patches_to_dir(
	...
	for (i, patch) in patches.iter().enumerate() {
		...
		let mut file = std::fs::OpenOptions::new()
			...
			.open(path)
			.context(
				"open new patch file with write and truncate options",
			)?;
	...
	println!(
		"created {} patch files in ./patches/{id}-*",
		patches.len()
	);
	Ok(())
}
