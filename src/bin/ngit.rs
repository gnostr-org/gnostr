...
async fn main() -> Result<()> {
	...
	match &cli.command {
		Commands::Fetch(args) => {
			sub_commands::fetch::launch(args).await
		}
		Commands::Login(args) => {
			sub_commands::login::launch(args).await
		}
		Commands::Init(args) => {
			sub_commands::init::launch(args).await
		}
		Commands::Send(args) => {
			sub_commands::send::launch(args, false).await
		}
		Commands::List => sub_commands::list::launch().await,
		Commands::Pull => sub_commands::pull::launch().await,
		Commands::Push(args) => {
			sub_commands::push::launch(args).await
		}
	}
}
