...
pub fn set_git_nostr_login_config(test_repo: &GitTestRepo) -> Result<()> {
...
pub fn clone_git_repo_with_nostr_url() -> Result<GitTestRepo> {
...
pub fn prep_git_repo_minus_1_commit() -> Result<GitTestRepo> {
    let mut test_repo = GitTestRepo::without_repo_in_git_config();
    set_git_nostr_login_config(&test_repo)?;
    test_repo.add_remote(NOSTR_REMOTE_NAME, &get_nostr_remote_url()?)?;
...
pub fn cli_tester(git_repo: &GitTestRepo) -> CliTester {
    CliTester::new_remote_helper_from_dir(&git_repo.dir, &get_nostr_remote_url().unwrap())
}

pub fn cli_tester_after_fetch(git_repo: &GitTestRepo) -> Result<CliTester> {
    let mut p = cli_tester(git_repo);
    cli_expect_nostr_fetch(&mut p)?;
    Ok(p)
}

pub fn cli_expect_nostr_fetch(p: &mut CliTester) -> Result<()> {
    p.expect("nostr: fetching...\r\n")?;
    p.expect_eventually("updates")?;
    p.expect_eventually("\r\n")?;
    Ok(())
}
...
pub fn cli_tester_after_nostr_fetch_and_sent_list_for_push_responds(
