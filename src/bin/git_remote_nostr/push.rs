...
pub async fn run_push(
	...
	let nostr_state =
		get_state_from_cache(git_repo.get_path()?, repo_ref).await;
	...
	let (rejected_refspecs, remote_refspecs) =
	...
	git_server_refspecs.retain(|refspec| {
		if let Some(rejected) =
			rejected_refspecs.get(&refspec.to_string())
		{
			let (_, to) = refspec_to_from_to(refspec).unwrap();
			println!(
				"error {to} {} out of sync with nostr",
				rejected.join(" ")
			);
			false
		} else {
	...
	if !git_server_refspecs.is_empty() {
		let new_state = generate_updated_state(
			git_repo,
			&existing_state,
			&git_server_refspecs,
		)?;
	...
	if !proposal_refspecs.is_empty() {
		...
		for refspec in &proposal_refspecs {
			...
			let tip_of_pushed_branch =
				git_repo.get_commit_or_tip_of_reference(from)?;
			...
			if let Some((_, (proposal, patches))) =
				...
				.contains(&user_ref.public_key)
				{
					if refspec.starts_with('+') {
						...
						let tip_of_proposal =
							get_commit_id_from_patch(tip_patch)?;
						let tip_of_proposal_commit = git_repo
							.get_commit_or_tip_of_reference(
								&tip_of_proposal,
							)?;
						...
						if behind.is_empty() {
							let thread_id = if let Ok(root_event_id) =
								...
								ahead.iter().enumerate()
							{
								...
								.await
								.context(
									"cannot make patch event from commit",
								)?;
	...
	// TODO check whether tip of each branch pushed is on at least one
	// git server before broadcasting the nostr state
	if !events.is_empty() {
		send_events(
			client,
			git_repo.get_path()?,
	...
	for refspec in
		&[git_server_refspecs.clone(), proposal_refspecs.clone()]
			.concat()
	{
...
fn push_to_remote(
	...
	let protocols_to_attempt = get_write_protocols_to_try(
		git_repo,
		&server_url,
		decoded_nostr_url,
	);
	...
	for protocol in &protocols_to_attempt {
		...
		let formatted_url = server_url
			.format_as(protocol, &decoded_nostr_url.user)?;
		...
		if let Err(error) = push_to_remote_url(
			...
			if !failed_protocols.is_empty() {
				term.write_line(
					format!("push: succeeded over {protocol}")
						.as_str(),
				)?;
...
fn push_to_remote_url(
	...
	let mut git_server_remote =
		git_repo.git_repo.remote_anonymous(git_server_url)?;
		...
		move |updates| {
			...
			for update in updates {
				...
				let msg = if update.dst().is_zero() {
					...
					{
						format!(
							"push: * [new tag]         {dst_refname}"
						)
					} else {
						format!(
							"push: * [new branch]      {dst_refname}"
						)
					}
				} else {
					let force = remote_refspecs.iter().any(|r| {
						r.contains(&dst_refname) && r.contains('+')
					});
					format!(
						"push: {} {}..{}  {} -> {dst_refname}",
						if force { "+" } else { " " },
						oid_to_shorthand_string(update.src())
							.unwrap(),
						oid_to_shorthand_string(update.dst())
							.unwrap(),
						update
							.src_refname()
							.unwrap_or("")
				...
				// other possibilities will result in push to fail but
				// better reporting is needed:
				// deleting a non-existant branch:
				// ! [remote rejected] <old-branch-name> ->
				// <old-branch-name> (not found) adding a branch
				// that already exists: ! [remote rejected]
				// <new-branch-name> -> <new-branch-name> (already
				// exists) pushing without non-fast-forward
				// without force: ! [rejected]
				// <branch-name> -> <branch-name> (non-fast-forward)
	...
	remote_callbacks.push_transfer_progress({
		...
		move |current, total, bytes| {
			let mut reporter = push_reporter.lock().unwrap();
			reporter.process_transfer_progress_update(
				current, total, bytes,
			);
		}
	});
...
impl<'a> PushReporter<'a> {
	...
	fn write_all(&self, lines_to_clear: usize) {
		...
		for msg in &self.remote_msgs {
			let _ = self
				.term
				.write_line(format!("remote: {msg}").as_str());
		}
	...
	fn process_transfer_progress_update(
		&mut self,
		current: usize,
		total: usize,
		bytes: usize,
...
fn create_rejected_refspecs_and_remotes_refspecs(
	...
	for (url, remote_state) in list_outputs {
			...
			if let Some(nostr_value) = nostr_value {
				if let Some(remote_value) = remote_value {
					if nostr_value.eq(remote_value) {
						...
						let is_remote_tip_ancestor_of_commit =
							if let Ok(remote_value_tip) = git_repo
								.get_commit_or_tip_of_reference(
									remote_value,
								) {
								if let Ok((_, behind)) = git_repo
									.get_commits_ahead_behind(
										&remote_value_tip,
										&from_tip,
									) {
						...
						if is_remote_tip_ancestor_of_commit {
							...
							// this is a force push so we need to
							// force push to git server too
							if refspec.starts_with('+') {
								refspecs_for_remote
									.push(refspec.clone());
							} else {
					...
					} else if let Ok(remote_value_tip) = git_repo
						.get_commit_or_tip_of_reference(remote_value)
					{
						if from_tip.eq(&remote_value_tip) {
							// remote already at correct state
							term.write_line(
								format!(
									"{short_name} {to} already up-to-date"
								)
								.as_str(),
							)?;
						}
						let (ahead_of_local, behind_local) = git_repo
							.get_commits_ahead_behind(
								&from_tip,
								&remote_value_tip,
							)?;
						if ahead_of_local.is_empty() {
							...
							// cant soft push
							let (ahead_of_nostr, behind_nostr) =
								git_repo.get_commits_ahead_behind(
									&git_repo
										.get_commit_or_tip_of_reference(
											nostr_value,
										)?,
							...
							if ahead_of_nostr.is_empty() {
								// ancestor of nostr and we are force
								// pushing anyway...
								refspecs_for_remote
									.push(refspec.clone());
							} else {
		...
		if !refspecs_for_remote.is_empty() {
			refspecs_for_remotes
				.insert(url.to_string(), refspecs_for_remote);
		}
	}

	// remove rejected refspecs so they dont get pushed to some
	// remotes
	...
	for (url, value) in &refspecs_for_remotes {
		remotes_refspecs_without_rejected.insert(
			...
			value
				.iter()
				.filter(|refspec| {
					!rejected_refspecs.contains_key(*refspec)
				})
				.cloned()
				.collect(),
		);
...
async fn get_merged_status_events(
	...
	for refspec in refspecs_to_git_server {
		...
		if to.eq("refs/heads/main") || to.eq("refs/heads/master") {
			let tip_of_pushed_branch =
				git_repo.get_commit_or_tip_of_reference(from)?;
			let Ok(tip_of_remote_branch) = git_repo
				.get_commit_or_tip_of_reference(
					&refspec_remote_ref_name(
						&git_repo.git_repo,
						refspec,
						remote_nostr_url,
					)?,
			...
			let (ahead, _) = git_repo.get_commits_ahead_behind(
				...
				if commit.parent_count() > 1 {
					...
					for parent in commit.parents() {
						...
						if let Some(commit_event) =
							commit_events.iter().find(|e| {
								e.tags.iter().any(|t| {
									t.as_vec()[0].eq("commit")
										&& t.as_vec()[1].eq(&parent
											.id()
											.to_string())
								})
							}) {
			...
			[
				vec![
					Tag::custom(
						...
						vec![
							"git proposal merged / applied"
								.to_string(),
						],
					),
					Tag::from_standardized(
						nostr::TagStandard::Event {
							event_id: proposal.id(),
							relay_url: repo_ref
								.relays
								.first()
								.map(nostr::UncheckedUrl::new),
				...
				vec![
					Tag::from_standardized(
						nostr::TagStandard::Reference(
							repo_ref.root_commit.to_string(),
						),
					),
					Tag::from_standardized(
						nostr::TagStandard::Reference(format!(
							"{merge_commit}"
						)),
					),
					Tag::custom(
...
async fn get_proposal_and_revision_root_from_patch(
	...
	let proposal_or_revision =
		if patch.tags.iter().any(|t| t.as_vec()[1].eq("root")) {
			patch.clone()
		} else {
			let proposal_or_revision_id = EventId::parse(
				if let Some(t) =
					patch.tags.iter().find(|t| t.is_root())
				{
					t.clone()
				} else if let Some(t) =
					patch.tags.iter().find(|t| t.is_reply())
				{
					t.clone()
				} else {
					Tag::event(patch.id())
...
fn update_remote_refs_pushed(
	...
	let target_ref_name =
		refspec_remote_ref_name(git_repo, refspec, nostr_remote_url)?;

	if from.is_empty() {
		if let Ok(mut remote_ref) =
			git_repo.find_reference(&target_ref_name)
		{
			remote_ref.delete()?;
		}
	} else {
		let commit = reference_to_commit(git_repo, from).context(
			format!("cannot get commit of reference {from}"),
		)?;
		if let Ok(mut remote_ref) =
			git_repo.find_reference(&target_ref_name)
		{
			remote_ref.set_target(
				commit,
				"updated by nostr remote helper",
			)?;
		} else {
			git_repo.reference(
				&target_ref_name,
...
fn refspec_remote_ref_name(
	...
	let nostr_remote = git_repo
		.find_remote(&get_remote_name_by_url(
			git_repo,
			nostr_remote_url,
		)?)
	...
	Ok(format!(
		...
		to.replace("refs/heads/", ""), /* TODO only replace if it
		                                * begins with this
		                                * TODO what about tags? */
	))
}

fn reference_to_commit(
	git_repo: &Repository,
	reference: &str,
) -> Result<Oid> {
	Ok(git_repo
		...
		.peel_to_commit()
		.context(format!(
			"cannot get commit from reference: {reference}"
		))?
		.id())
}

// this maybe a commit id or a ref: pointer
fn reference_to_ref_value(
	git_repo: &Repository,
	reference: &str,
) -> Result<String> {
...
fn get_refspecs_from_push_batch(
	stdin: &Stdin,
	initial_refspec: &str,
) -> Result<Vec<String>> {
	...
	loop {
		...
		match tokens.as_slice() {
			...
			_ => {
				bail!(
					"after a `push` command we are only expecting another push or an empty line"
				)
			}
		}
	}
...
impl BuildRepoState for RepoState {
	async fn build(
		...
		let event = sign_event(
			EventBuilder::new(STATE_KIND, "", tags),
			signer,
		)
		.await?;
		Ok(RepoState {
...
mod tests {
	...
	mod refspec_to_from_to {
		...
		fn trailing_plus_stripped() {
			let (from, _) =
				refspec_to_from_to("+testing:testingb").unwrap();
