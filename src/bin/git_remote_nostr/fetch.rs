...
pub async fn run_fetch(
	...
	let mut fetch_batch =
		get_oids_from_fetch_batch(stdin, oid, refstr)?;
	...
	if !fetch_batch.is_empty() {
		...
		for (refstr, oid) in fetch_batch {
			if let Some((_, (_, patches))) =
				...
				if !git_repo.does_commit_exist(&oid)? {
					...
					if git_repo.does_commit_exist(&tag_value(
						...
						for patch in &patches_ancestor_first {
							if let Err(error) = git_repo
								.create_commit_from_patch(patch)
							{
								term.write_line(
                                    format!(
						...
						term.write_line(
							format!(
								"WARNING: cannot find parent commit for {refstr}"
							)
							.as_str(),
						)?;
					}
				...
				term.write_line(
					format!(
						"WARNING: cannot find proposal for {refstr}"
					)
					.as_str(),
				)?;
			}
...
fn fetch_from_git_server(
	...
	let protocols_to_attempt = get_read_protocols_to_try(
		git_repo,
		&server_url,
		decoded_nostr_url,
	);
	...
	for protocol in &protocols_to_attempt {
		...
		let res = fetch_from_git_server_url(
			...
			oids,
			&formatted_url,
			[ServerProtocol::UnauthHttps, ServerProtocol::UnauthHttp]
				.contains(protocol),
			term,
		);
		if let Err(error) = res {
			...
			if !failed_protocols.is_empty() {
				term.write_line(
					format!("fetch: succeeded over {protocol}")
						.as_str(),
				)?;
		...
		if let Ok(data) = str::from_utf8(data) {
			...
			for data in data {
				...
				if let Some(last) = self.remote_msgs.last() {
					...
					// same msg
					if (last.starts_with("Enume")
						&& !last.ends_with(", done."))
						|| ((last.starts_with("Compre")
							|| last.starts_with("Count"))
							&& !last.contains(')'))
					{
	...
	fn process_transfer_progress_update(
		...
		if self.transfer_progress_msgs.len() <= updated.len() {
			if self.end_time.is_none()
				&& updated.first().is_some_and(|f| f.contains("100%"))
			{
...
fn fetch_from_git_server_url(
	...
	if git_server_url.parse::<CloneUrl>()?.protocol()
		== ServerProtocol::Ssh
		&& !check_ssh_keys()
	{
	...
	let mut git_server_remote =
		git_repo.remote_anonymous(git_server_url)?;
	let auth = GitAuthenticator::default();
	let mut fetch_options = git2::FetchOptions::new();
	let mut remote_callbacks = git2::RemoteCallbacks::new();
	let fetch_reporter =
		Arc::new(Mutex::new(FetchReporter::new(term)));
...
mod tests {
	...
	fn pass_through_fetch_reporter_proces_remote_msg(
		msgs: Vec<&str>,
	) -> Vec<String> {
