...
async fn main() -> Result<()> {
	...
	let ([_, nostr_remote_url] | [nostr_remote_url]) =
		args.as_slice()
	else {
	...
	let git_repo = Repo::from_path(&PathBuf::from(
		std::env::var("GIT_DIR").context(
			"git should set GIT_DIR when remote helper is called",
		)?,
	))?;
	...
	let decoded_nostr_url =
		NostrUrlDecoded::from_str(nostr_remote_url)
			.context("invalid nostr url")?;

	fetching_with_report_for_helper(
		git_repo_path,
		&client,
		&decoded_nostr_url.coordinates,
	)
	.await?;
