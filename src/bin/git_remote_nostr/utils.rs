...
pub fn get_short_git_server_name(
	git_repo: &Repo,
	url: &str,
) -> std::string::String {
		...
		.find(|r| {
			if let Some(name) = r {
				if let Some(remote_url) =
					git_repo.find_remote(name).unwrap().url()
				{
					url == remote_url
				} else {
					false
...
pub fn get_read_protocols_to_try(
	...
	if server_url.protocol() == ServerProtocol::Filesystem {
		...
		if let Some(protocol) = get_protocol_preference(
			git_repo,
			server_url,
			&Direction::Fetch,
		) {
			if let Some(pos) =
				list.iter().position(|p| *p == protocol)
			{
				list.remove(pos);
...
pub fn get_protocol_preference(
	...
	if let Ok(Some(list)) = git_repo.get_git_config_item(
			...
			if let Some(url) = pair.get(1) {
				if *url == server_short_name {
					if let Some(protocol) = pair.first() {
						if let Ok(protocol) =
							ServerProtocol::from_str(protocol)
						{
...
pub fn set_protocol_preference(
	...
	if let Some(list) = git_repo.get_git_config_item(
		format!("nostr.protocol-{direction}").as_str(),
		Some(false),
	)? {
		for item in list.split(';') {
...
pub fn fetch_or_list_error_is_not_authentication_failure(
	error: &anyhow::Error,
) -> bool {
...
/// to understand whether to try over another protocol
pub fn push_error_is_not_authentication_failure(
	error: &anyhow::Error,
) -> bool {
	!error_might_be_authentication_related(error)
}

pub fn error_might_be_authentication_related(
	error: &anyhow::Error,
) -> bool {
...
fn count_lines_per_msg(
	...
	// Arithmetic
	((msg.chars().count() + prefix_len) + (width - 1) as usize)
		/ width as usize
}

pub fn count_lines_per_msg_vec(
	width: u16,
	msgs: &[String],
	prefix_len: usize,
) -> usize {
