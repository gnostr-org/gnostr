...
impl<'a> Relay<'a> {
    pub fn new(
        ...
        let (event_hub, server_handle, cancellation_token) =
            crate::ws::launch(port).unwrap_or_else(|_| panic!("failed to listen on port {port}"));
        Self {
            port,
            events: vec![],
            reqs: vec![],
    ...
    pub async fn listen_until_close(&mut self) -> Result<()> {
        loop {
            ...
            match self.event_hub.poll_async().await {
                crate::ws::Event::Connect(client_id, responder) => {
                ...
                crate::ws::Event::Message(client_id, message) => {
                    // println!("bla{:?}", &message);
...
impl Drop for Relay<'_> {
    fn drop(&mut self) {
        ...
        if let Some(handle) = self.server_handle.take() {
            handle
                .join()
                .expect("Relay server thread panicked during shutdown");
        }
    }
}
...
pub fn shutdown_relay(port: u64) -> Result<()> {
        ...
        tokio_tungstenite::tungstenite::connect(format!("ws://localhost:{}", port))
    {
        counter += 1;
        if counter == 1 {
            socket.write(tokio_tungstenite::tungstenite::Message::text(
                "shut me down",
            ))?;
        }
        socket.close(None)?;
