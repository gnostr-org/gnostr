...
type ListenerEventFunc<'a> =
	&'a dyn Fn(&mut Relay, u64, nostr_0_34_1::Event) -> Result<()>;
...
impl<'a> Relay<'a> {
	pub fn new(
		...
		let (event_hub, server_handle, cancellation_token) = crate::ws::launch(port)
			.unwrap_or_else(|_| {
	...
	pub fn respond_ok(
		...
		.as_json();
		// bail!(format!("{}", &ok_json));
		Ok(responder.send(crate::ws::Message::Text(ok_json)))
	}
	...
	pub fn respond_events(
		...
		for event in events {
			let res =
				responder.send(crate::ws::Message::Text(
					RelayMessage::Event {
	...
	pub fn respond_standard_req(
		...
		self.respond_events(
			...
			&self
				...
				.iter()
				.filter(|e| {
					filters.iter().any(|filter| filter.match_event(e))
				})
				.filter(|_| true)
				.cloned()
				.collect(),
	...
	/// listen, collect events and responds with event_listener to
	/// events or Ok(eventid) if event_listner is None
	pub async fn listen_until_close(&mut self) -> Result<()> {
		loop {
			println!("{} polling", self.port);
			match self.event_hub.poll_async().await {
				crate::ws::Event::Connect(
					client_id,
					responder,
				) => {
				...
				crate::ws::Event::Disconnect(client_id) => {
				...
				crate::ws::Event::Message(
					...
					if let crate::ws::Message::Text(s) =
						message.clone()
					{
						if s.eq("shut me down") {
							println!(
								"{} recieved shut me down",
								self.port
							);
							break;
						}
					...
					if let Ok(event) = get_nevent(&message) {
						// println!("{:?}", &event);
						// let t: Vec<nostr_0_34_1::Kind> =
						// self.events.iter().map(|e|
						// e.kind).collect(); println!("before{:
						// ?}", t);
						self.events.push(event.clone());
						// let t: Vec<nostr_0_34_1::Kind> =
						// self.events.iter().map(|e|
						// e.kind).collect(); println!("after{:?
						// }", t);
					...
					if let Ok((subscription_id, filters)) =
						get_nreq(&message)
					{
...
pub fn shutdown_relay(port: u64) -> Result<()> {
	let mut counter = 0;
	while let Ok((mut socket, _)) =
		tokio_tungstenite::tungstenite::connect(format!("ws://localhost:{}", port))
	{
		counter += 1;
		if counter == 1 {
			socket
				.write(tokio_tungstenite::tungstenite::Message::text("shut me down"))?;
		}
		socket.close(None)?;
	}
	Ok(())
...
fn is_nclose(message: &crate::ws::Message) -> bool {
	if let crate::ws::Message::Text(s) = message.clone() {
		let cm_result = ClientMessage::from_json(s);
...
/// leaves trailing whitespace and only compatible with
/// --no-cli-spinners flag relays tuple: (title,successful,message)
...
pub fn expect_send_with_progress_exact_interaction(
	...
	let generate_text =
		|title: &str, num: u16, confirmed_complete: bool| -> String {
		...
		for selected_title in &titles {
			for title in &titles {
				if title.eq(selected_title) {
					...
					p.expect(generate_text(
						title,
						*nums.get(title).unwrap(),
						false,
					))?;
				} else {
					p.expect(generate_text(
