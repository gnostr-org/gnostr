...
//TODO this test is a nostr crate test only //use gnostr_types::Unixtime;
...
pub fn make_event_old_or_change_user(
    ...
    unsigned.created_at =
		(nostr::Timestamp::now() - how_old_in_secs).into();
...
impl CliTester {
    pub fn expect_input(&mut self, prompt: &str) -> Result<CliTesterInputPrompt<'_>> {
    ...
    pub fn expect_input_eventually(&mut self, prompt: &str) -> Result<CliTesterInputPrompt<'_>> {
        let mut i = CliTesterInputPrompt {
            tester: self,
    ...
    pub fn expect_password(&mut self, prompt: &str) -> Result<CliTesterPasswordPrompt<'_>> {
...
impl CliTester {
    ...
    pub fn new_remote_helper_from_dir(dir: &PathBuf, nostr_remote_url: &str) -> Self {
        let mut cmd = std::process::Command::new(
            std::env::var("CARGO_BIN_EXE_git-remote-nostr")
                .unwrap_or("git-remote-nostr".to_string()),
        );
        cmd.env("NGITTEST", "TRUE");
        cmd.env("GIT_DIR", dir);
        cmd.env("RUST_BACKTRACE", "0");
    ...
    fn exp_string(&mut self, message: &str) -> Result<String> {
        ...
        {
            Ok(before) => Ok(std::str::from_utf8(before.get(0).unwrap_or(b""))
                .unwrap_or("")
                .to_string()),
            Err(e) => {
    ...
    pub fn expect_end(&mut self) -> Result<()> {
        let before = self
            ...
            .expect(Eof)
            .context("expected immediate end but got timed out")?;
        let before_string = std::str::from_utf8(before.get(0).unwrap_or(b""))
            .unwrap_or("")
            .to_string();
        ensure!(
...
pub fn cli_tester_create_proposal_branches_ready_to_send() -> Result<GitTestRepo> {
    let mut git_repo = GitTestRepo::default();
    git_repo.populate()?;
    create_and_populate_branch(&mut git_repo, FEATURE_BRANCH_NAME_1, "a", false)?;
    create_and_populate_branch(&mut git_repo, FEATURE_BRANCH_NAME_2, "b", false)?;
    create_and_populate_branch(&mut git_repo, FEATURE_BRANCH_NAME_3, "c", false)?;
    Ok(git_repo)
}
...
pub fn create_repo_with_proposal_branch_pulled_and_checkedout(
    ...
    test_repo.populate()?;
    use_ngit_list_to_download_and_checkout_proposal_branch(&mut test_repo, proposal_number)?;
    Ok(test_repo)
}
...
pub fn amend_last_commit(test_repo: &mut GitTestRepo, commit_msg: &str) -> Result<String> {
    let branch_name =
...
pub fn create_proposals_with_first_revised_and_repo_with_unrevised_proposal_checkedout(
    ...
    use_ngit_list_to_download_and_checkout_proposal_branch(&mut originating_repo, 1)?;

    amend_last_commit(&mut originating_repo, "add some ammended-commit.md")?;
