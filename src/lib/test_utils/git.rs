...
impl GitTestRepo {
    ...
    pub fn recreate_as_bare(_existing_repo: &GitTestRepo) -> Result<Self> {
        // create bare repo
        let path = current_dir()?.join(format!("tmpgit-{}", rand::random::<u64>()));
        let git_repo = git2::Repository::init_opts(
            &path,
    ...
    pub fn stage_and_commit(&mut self, message: &str) -> Result<Oid> {
    ...
    pub fn checkout(&mut self, ref_name: &str) -> Result<Oid> {
    ...
    pub fn get_checked_out_branch_name(&self) -> Result<String> {
        match self.git_repo.head() {
            ...
            Err(e) if e.code() == git2::ErrorCode::UnbornBranch => {
                ...
                let head_content =
                    fs::read_to_string(head_path).context("failed to read .git/HEAD")?;
                // content is "ref: refs/heads/main\n"
                let branch_name = head_content.trim_start_matches("ref: refs/heads/").trim();
    ...
    pub fn checkout_remote_branch(&mut self, branch_name: &str) -> Result<Oid> {
...
mod tests {
    ...
    fn test_git_test_repo_duplicate() -> Result<()> {
        ...
        assert_eq!(
            ...
            duplicated_repo.git_repo.head()?.peel_to_commit()?.id()
        );
    ...
    fn test_git_test_repo_clone_repo() -> Result<()> {
        ...
        assert_eq!(
            original_repo.git_repo.head()?.peel_to_commit()?.id(),
            cloned_repo.git_repo.head()?.peel_to_commit()?.id()
        );
