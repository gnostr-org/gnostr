...
use crate::tui::tui::{AsyncAppNotification, SyntaxHighlightProgress};
...
impl SyntaxText {
    pub fn new(
...

        {
            ...
            let mut buffer = AsyncProgressBuffer::new(total_count, Duration::from_millis(200));
...
impl AsyncSyntaxJob {
    pub fn new(content: String, path: String) -> Self {
        Self {
            state: Arc::new(Mutex::new(Some(JobState::Request((content, path))))),
        }
    }
        ...
        if let Some(state) = state_mutex.take() {
            *state_mutex = Some(match state {
                JobState::Request((content, path)) => {
                    let syntax = SyntaxText::new(content, Path::new(&path), &params)?;
