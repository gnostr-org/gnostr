        ...
        .block_on(async {
            ...
            loop {
                tokio::select! {
                    ...
                    Ok((stream, _)) = tokio_listener.accept() => {
...
async fn handle_connection(stream: TcpStream, event_tx: flume::Sender<Event>, id: u64) {
    ...
    event_tx
        ...
        .unwrap_or_else(|e| warn!("Failed to send Connect event: {}", e));
    ...
    event_tx
        ...
        .unwrap_or_else(|e| warn!("Failed to send Disconnect event: {}", e));
}
...
// winapi
// This module and its contents will only be compiled if the target operating system is Windows.

/// windows_process_killer
#[cfg(target_os = "windows")]
mod windows_process_killer {
    ...
    pub fn kill_process_by_pid(process_id: DWORD) -> Result<(), String> {
        ...
        unsafe {
            ...
            let terminated: i32 = TerminateProcess(
                process_handle,
                1
            );
            ...
            if terminated != 0 {
                println!("✅ Process with PID {} successfully terminated.", process_id);
                Ok(())
            } else {
                let error_code = GetLastError();
                Err(format!(
