...
impl ChangesComponent {
                ...
                if let FileTreeItemKind::File(i) = tree_item.kind {
                    ...
                    match i.status {
                        StatusItemType::Deleted => {
                            sync::stage_addremoved(&self.repo.borrow(), path)?;
                        }
                        _ => sync::stage_add_file(&self.repo.borrow(), path)?,
                    };
                } else {
    ...
    fn add_to_ignore(&mut self) -> bool {
        if let Some(tree_item) = self.selection() {
            if let Err(e) = sync::add_to_ignore(&self.repo.borrow(), &tree_item.info.full_path) {
    ...
    fn event(&mut self, ev: &Event) -> Result<EventState> {
        ...
        if self.focused() {
            if let Event::Key(e) = ev {
                return if key_match(e, self.key_config.keys.stage_unstage_item) {
                    try_or_popup!(self, "staging error:", self.index_add_remove());

                    self.queue.push(InternalEvent::Update(NeedsUpdate::ALL));
                    Ok(EventState::Consumed)
                } else if key_match(e, self.key_config.keys.status_stage_all) && !self.is_empty() {
                    if self.is_working_dir {
                        try_or_popup!(self, "staging all error:", self.index_add_all());
                    } else {
                ...
                } else if key_match(e, self.key_config.keys.status_ignore_file)
                    && self.is_working_dir
                    && !self.is_empty()
                {
