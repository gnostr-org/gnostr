...
impl RevisionFilesComponent {
    ...
    pub fn new(env: &Environment) -> Self {
        Self {
            ...
            files: None,
            async_treefiles: AsyncSingleJob::new(env.sender_git.clone()),
            revision: None,
            focus: Focus::Tree,
    ...
    pub fn set_commit(&mut self, commit: CommitId) -> Result<()> {
        ...
        let same_id = self.revision.as_ref().map_or(false, |c| c.id == commit);
        ...
        if !same_id {
            ...
            self.revision = Some(get_commit_info(&self.repo.borrow(), &commit)?);
        }
    ...
    pub fn update(&mut self, ev: AsyncNotification) -> Result<()> {
        ...
        if matches!(ev, AsyncNotification::Git(AsyncGitNotification::TreeFiles)) {
            self.refresh_files()?;
        }
            ...
            if let Some(result) = last.result() {
                    ...
                    .is_some_and(|commit| commit.id == result.commit)
                {
                    if let Ok(last) = result.result {
                        let filenames: Vec<&Path> = last.iter().map(|f| f.path.as_path()).collect();
    ...
    fn tree_item_to_span<'a>(
        ...
        let available_width = width.saturating_sub(indent_str.len() + path_arrow.len());

        let path = format!("{indent_str}{path_arrow}{path:available_width$}");
    ...
    fn open_finder(&self) {
        if let Some(files) = self.files.clone() {
            self.queue.push(InternalEvent::OpenFuzzyFinder(
                files
                    .iter()
                    .map(|a| a.path.to_str().unwrap_or_default().to_string())
                    .collect(),
    ...
    fn selection_changed(&mut self) {
        ...
        if let Some(file) = self.selected_file_path_with_prefix() {
            if let Some(files) = &self.files {
                ...
                if let Some(item) = files.iter().find(|f| f.path == path) {
                    if let Ok(path) = path.strip_prefix("./") {
                        return self
                            .current_file
                            .load_file(path.to_string_lossy().to_string(), item);
                    }
                }
    ...
    fn draw_tree(&self, f: &mut Frame, area: Rect) -> Result<()> {
        ...
        let items =
            self.tree
                ...
                .map(|(item, selected)| {
                    Self::tree_item_to_span(item, &self.theme, tree_width, selected)
                });
        ...
        let block = Block::default()
            .title(Span::styled(title, self.theme.title(is_tree_focused)))
            .borders(Borders::ALL)
    ...
    fn title_within(&self, tree_width: usize) -> Result<String> {
        ...
        let message = self.revision.as_ref().and_then(|c| {
            let _ignore = write!(title, " {{{}}}", c.id.get_short_string());
        ...
        if let Some(message) = message {
            ...
            let available = tree_width.saturating_sub(title.width()).saturating_sub(
    ...
    fn request_files(&mut self, commit: CommitId) {
        self.async_treefiles
            .spawn(AsyncTreeFilesJob::new(self.repo.borrow().clone(), commit));
    }
}
...
impl Component for RevisionFilesComponent {
    ...
    fn event(&mut self, event: &crossterm::event::Event) -> Result<EventState> {
        ...
        if let Event::Key(key) = event {
            let is_tree_focused = matches!(self.focus, Focus::Tree);
            if is_tree_focused && tree_nav(&mut self.tree, &self.key_config, key) {
                self.selection_changed();
...
fn tree_nav_cmds(tree: &FileTree, key_config: &SharedKeyConfig, out: &mut Vec<CommandInfo>) {
    out.push(
        CommandInfo::new(
