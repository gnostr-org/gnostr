...
impl Selection {
    ...
    fn modify(&mut self, direction: Direction, max: usize) {
        ...
        *self = match direction {
            Direction::Up => Self::Multiple(start, old_end.saturating_sub(1)),

            Direction::Down => Self::Multiple(start, cmp::min(old_end + 1, max)),
        };
    }
...
impl DiffComponent {
    ...
    pub fn update(&mut self, path: String, is_stage: bool, diff: FileDiff) {
        ...
        if self.current.hash != hash {
            ...
            self.longest_line = self
                ...
                .map(|line| {
                    let converted_content = tabs_to_spaces(line.content.as_ref().to_string());
    ...
    fn move_selection(&mut self, move_type: ScrollType) {
        if let Some(diff) = &self.diff {
            ...
            let new_start = match move_type {
                ScrollType::Down => self.selection.get_bottom().saturating_add(1),
                ScrollType::Up => self.selection.get_top().saturating_sub(1),
                ScrollType::Home => 0,
                ScrollType::End => max,
                ScrollType::PageDown => self
                    .selection
                    .get_bottom()
                    .saturating_add(self.current_size.get().1.saturating_sub(1) as usize),
                ScrollType::PageUp => self
                    .selection
                    .get_top()
                    .saturating_sub(self.current_size.get().1.saturating_sub(1) as usize),
            };
    ...
    fn update_selection(&mut self, new_start: usize) {
        if let Some(diff) = &self.diff {
            ...
            self.selected_hunk = Self::find_selected_hunk(diff, new_start);
        }
    }
            ...
            let lines_to_copy: Vec<&str> = diff
                ...
                .filter_map(|(i, line)| {
                    if self.selection.contains(i) {
                        Some(line.content.trim_matches(|c| c == '\n' || c == '\r'))
                    } else {
                        None
                    }
                })
    ...
    fn find_selected_hunk(diff: &FileDiff, line_selected: usize) -> Option<usize> {
        ...
        for (i, hunk) in diff.hunks.iter().enumerate() {
            ...
            let hunk_selected = hunk_min <= line_selected && hunk_max > line_selected;
    ...
    fn get_text(&self, width: u16, height: u16) -> Vec<Line> {
        if let Some(diff) = &self.diff {
            return if diff.hunks.is_empty() {
                ...
                for (i, hunk) in diff.hunks.iter().enumerate() {
                    let hunk_selected =
                        self.focused() && self.selected_hunk.map_or(false, |s| s == i);
                        ...
                        for (i, line) in hunk.lines.iter().enumerate() {
                            if line_cursor >= min && line_cursor <= max {
                                res.push(Self::get_line_to_add(
                                    width,
                                    line,
                                    self.focused() && self.selection.contains(line_cursor),
                                    hunk_selected,
                                    i == hunk_len - 1,
                                    &self.theme,
    ...
    fn get_text_binary(&self, diff: &FileDiff) -> Vec<Line> {
        ...
        let delta_byte_size = ByteSize::b(diff.size_delta.unsigned_abs());
    ...
    fn get_line_to_add<'a>(
        ...
        let is_content_line = matches!(line.line_type, DiffLineType::None);

        let left_side_of_line = if end_of_hunk {
            Span::styled(Cow::from(symbols::line::BOTTOM_LEFT), style)
        } else {
            match line.line_type {
                DiffLineType::Header => Span::styled(Cow::from(symbols::line::TOP_LEFT), style),
                _ => Span::styled(Cow::from(symbols::line::VERTICAL), style),
            }
        };

        let content = if !is_content_line && line.content.as_ref().is_empty() {
            theme.line_break()
        } else {
        ...
        Line::from(vec![
            left_side_of_line,
            Span::styled(Cow::from(filled), theme.diff_line(line.line_type, selected)),
        ])
    }

    const fn hunk_visible(hunk_min: usize, hunk_max: usize, min: usize, max: usize) -> bool {
        // full overlap
        ...
        // partly overlap
        if (hunk_min >= min && hunk_min <= max) || (hunk_max >= min && hunk_max <= max) {
            return true;
        }
    ...
    fn stage_hunk(&mut self) -> Result<()> {
        if let Some(diff) = &self.diff {
            if let Some(hunk) = self.selected_hunk {
                if diff.untracked {
                    sync::stage_add_file(&self.repo.borrow(), Path::new(&self.current.path))?;
                } else {
                    #[allow(clippy::needless_pass_by_ref_mut)]
                    let hash = diff.hunks[hunk].header_hash;
                    sync::stage_hunk(
    ...
    fn reset_hunk(&self) {
        if let Some(diff) = &self.diff {
            if let Some(hunk) = self.selected_hunk {
                ...
                self.queue
                    .push(InternalEvent::ConfirmAction(Action::ResetHunk(
    ...
    fn reset_lines(&self) {
        self.queue
            .push(InternalEvent::ConfirmAction(Action::ResetLines(
                self.current.path.clone(),
                self.selected_lines(),
            )));
    }
        ...
        self.diff
            ...
            .map(|diff| {
                diff.hunks
                    ...
                    .enumerate()
                    .filter_map(|(i, line)| {
                        let is_add_or_delete = line.line_type == DiffLineType::Add
    ...
    fn reset_untracked(&self) {
        self.queue
            .push(InternalEvent::ConfirmAction(Action::Reset(ResetItem {
    ...
    fn calc_hunk_move_target(&self, direction: isize) -> Option<usize> {
...
impl Component for DiffComponent {
    ...
    fn commands(&self, out: &mut Vec<CommandInfo>, _force_all: bool) -> CommandBlocking {
        ...
        if !self.is_immutable {
            ...
            out.push(CommandInfo::new(
                strings::commands::diff_lines_revert(&self.key_config),
            ...
            out.push(CommandInfo::new(
                strings::commands::diff_lines_unstage(&self.key_config),
                //TODO: only if any modifications are selected
                true,
                self.focused() && self.is_stage(),
            ));
    ...
    fn event(&mut self, ev: &Event) -> Result<EventState> {
        if self.focused() {
            if let Event::Key(e) = ev {
                return if key_match(e, self.key_config.keys.move_down) {
                ...
                } else if key_match(e, self.key_config.keys.shift_down) {
                    ...
                    Ok(EventState::Consumed)
                } else if key_match(e, self.key_config.keys.diff_hunk_next) {
                    self.diff_hunk_move_up_down(1);
                    Ok(EventState::Consumed)
                } else if key_match(e, self.key_config.keys.diff_hunk_prev) {
                    self.diff_hunk_move_up_down(-1);
                    Ok(EventState::Consumed)
                } else if key_match(e, self.key_config.keys.stage_unstage_item)
                    && !self.is_immutable
                {
                    try_or_popup!(self, "hunk error:", self.stage_unstage_hunk());

                    Ok(EventState::Consumed)
                } else if key_match(e, self.key_config.keys.status_reset_item)
                    && !self.is_immutable
                    && !self.is_stage()
                {
                    if let Some(diff) = &self.diff {
                    ...
                    Ok(EventState::Consumed)
                } else if key_match(e, self.key_config.keys.diff_stage_lines) && !self.is_immutable
                {
                    self.stage_lines();
                    Ok(EventState::Consumed)
                } else if key_match(e, self.key_config.keys.diff_reset_lines)
                    && !self.is_immutable
                    && !self.is_stage()
                {
                    if let Some(diff) = &self.diff {
...
"#
            ...
            let theme = Rc::new(Theme::init(&file.path().to_path_buf()));

            assert_eq!(
                DiffComponent::get_line_to_add(4, &diff_line, false, false, false, &theme, 0)
                    .spans
                    .last()
                    .unwrap(),
                &Span::styled(
