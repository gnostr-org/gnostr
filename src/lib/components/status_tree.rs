...
impl StatusTreeComponent {
    ...
    fn item_to_text<'b>(
        ...
        match file_item_kind {
            FileTreeItemKind::File(status_item) => {
                let status_char = Self::item_status_char(status_item.status);
            ...
            FileTreeItemKind::Path(path_collapsed) => {
                let collapse_char = if path_collapsed.0 { '▸' } else { '▾' };
                ...
                Some(Span::styled(Cow::from(txt), theme.text(true, selected)))
            }
        }
    }
    ...
    fn build_vec_text_draw_info_for_drawing(&self) -> (Vec<TextDrawInfo<'_>>, usize, usize) {
        ...
        for (index, item) in tree_items.iter().enumerate() {
            ...
            let index_above_select = index < self.tree.selection.unwrap_or(0);
                ...
                && tree_items[idx_temp].info.indent < tree_items[idx_temp + 1].info.indent
            {
                ...
                else if self.tree.tree.multiple_items_at_path(idx_temp) {
                    should_skip_over -= 1;
                    break;
                }
                ...
                vec_draw_text_info[vec_draw_text_info_len - 1].name +=
                    &(String::from("/") + &tree_items[idx_temp].info.path);
    ...
    fn copy_file_path(&self) {
        if let Some(item) = self.selection() {
            if crate::clipboard::copy_string(&item.info.full_path).is_err() {
...
impl DrawableComponent for StatusTreeComponent {
    fn draw(&self, f: &mut Frame, r: Rect) -> Result<()> {
        ...
        if self.pending {
            ...
            let (vec_draw_text_info, selection_offset, selection_offset_visible) =
            ...
            ui::draw_list(f, r, self.title.as_str(), items, self.focused, &self.theme);
        }
    ...
    fn event(&mut self, ev: &Event) -> Result<EventState> {
        if self.focused {
            if let Event::Key(e) = ev {
                return if key_match(e, self.key_config.keys.blame) {
                ...
                } else if key_match(e, self.key_config.keys.file_history) {
                    if let Some(status_item) = self.selection_file() {
                        self.hide();
                        self.queue
                    ...
                    if let Some(status_item) = self.selection_file() {
                        self.queue
                            .push(InternalEvent::OpenExternalEditor(Some(status_item.path)));
                    }
                    Ok(EventState::Consumed)
                    ...
                    || key_match(e, self.key_config.keys.shift_down)
                {
                    ...
                    Ok(self.move_selection(MoveSelection::Left).into())
                } else if key_match(e, self.key_config.keys.move_right) {
                    Ok(self.move_selection(MoveSelection::Right).into())
                } else {
...
mod tests {
    ...
    fn test_correct_scroll_position() {
        ...
        let test_backend = ratatui::backend::TestBackend::new(100, 100);
        let mut terminal = ratatui::Terminal::new(test_backend).expect("Unable to set up terminal");
        let mut frame = terminal.get_frame();

        // set up file tree
        let mut ftc = StatusTreeComponent::new(&Environment::test_env(), "title", true);
        ftc.update(&items)
