...
/// helper func to convert unix time since epoch to formatted time
/// string in local timezone
pub fn time_to_string(secs: i64, short: bool) -> String {
    let time = DateTime::<Local>::from(DateTime::<Utc>::from_naive_utc_and_offset(
...
pub fn string_width_align(s: &str, width: usize) -> String {
    ...
    if (len >= width_wo_postfix && len <= width) || (len <= width_wo_postfix) {
        format!("{s:width$}")
    } else {
