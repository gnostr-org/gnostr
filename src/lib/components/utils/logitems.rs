...
impl From<CommitInfo> for LogEntry {
    fn from(c: CommitInfo) -> Self {
        ...
        let time = {
            let date = DateTime::from_timestamp(c.time, 0).map(|d| d.naive_utc());
            if date.is_none() {
                log::error!(
            ...
            DateTime::<Local>::from(DateTime::<Utc>::from_naive_utc_and_offset(
...
impl LogEntry {
    pub fn time_to_string(&self, now: DateTime<Local>) -> String {
        ...
        if delta < Duration::try_minutes(30).unwrap_or_default() {
            let delta_str = if delta < Duration::try_minutes(1).unwrap_or_default() {
                "<1m ago".to_string()
            } else {
...
impl ItemBatch {
    ...
    pub fn set_items(
        ...
        if !commits.is_empty() {
            self.items.extend(commits.into_iter().map(|c| {
                ...
                if highlighted
                    .as_ref()
                    .is_some_and(|highlighted| highlighted.contains(&id))
                {
    ...
    pub fn needs_data(&self, idx: usize, idx_max: usize) -> bool {
        let want_min = idx.saturating_sub(SLICE_OFFSET_RELOAD_THRESHOLD);
        let want_max = idx
...
impl<'a> IntoIterator for &'a ItemBatch {
    type IntoIter = std::slice::Iter<'a, crate::components::utils::logitems::LogEntry>;
