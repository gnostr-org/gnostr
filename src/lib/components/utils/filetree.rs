...
impl TreeItemInfo {
    const fn new(indent: u8, path: String, full_path: String) -> Self {
        Self {
            indent,
            visible: true,
            path,
...
impl FileTreeItem {
    fn new_file(item: &StatusItem) -> Result<Self> {
        ...
        let indent = u8::try_from(item_path.ancestors().count().saturating_sub(2))?;
    ...
    fn new_path(path: &Path, path_string: String, collapsed: bool) -> Result<Self> {
        let indent = u8::try_from(path.ancestors().count().saturating_sub(2))?;
        ...
        for e in list {
            {
                ...
                Self::push_dirs(item_path, &mut items, &mut paths_added, collapsed)?;
            }
    ...
    fn push_dirs<'a>(
        ...
        for c in &ancestors {
            if c.parent().is_some() && !paths_added.contains(c) {
                ...
                nodes.push(FileTreeItem::new_path(c, path_string, is_collapsed)?);
            }
        }
    ...
    pub fn multiple_items_at_path(&self, index: usize) -> bool {
        ...
        if index + 2 < tree_items.len() {
            ...
            while idx_temp_inner < tree_items.len().saturating_sub(1)
                && tree_items[index].info.indent < tree_items[idx_temp_inner].info.indent
            {
                idx_temp_inner += 1;
            }
        } else {
        ...
        tree_items[idx_temp_inner].info.indent == tree_items[index].info.indent
    }
}
...
mod tests {
    ...
    fn test_folder() {
        ...
        assert_eq!(res, vec![String::from("a"), items[0].path.clone(),]);
    }
