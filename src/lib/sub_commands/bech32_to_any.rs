...
use gnostr_types::{NEvent, NostrBech32, PrivateKey};
...
pub fn bech32_to_any(sub_command_args: &Bech32ToAnySubCommand) -> Result<()> {
    ...
    if sub_command_args.event_id {
        // Try nevent first
        if let Ok(nevent) = gnostr_types::NEvent::try_from_bech32_string(bech32) {
            println!("{}", nevent.id.as_hex_string());
            return Ok(());
        }
        // Try note (direct event ID)
        else if let Ok(note) = gnostr_types::Id::try_from_bech32_string(bech32) {
            println!("{}", note.as_hex_string());
            return Ok(());
        }
        // Try naddr (parameterized replaceable event)
        else if let Ok(_naddr) = gnostr_types::NAddr::try_from_bech32_string(bech32) {
...
mod tests {
    use super::*;
    use gnostr_types::{Id, NEvent, NostrBech32};
