...
// I think we can use the async-trait crate and switch to the native
// feature which is currently in nightly. alternatively we can use
// nightly as it looks certain that the implementation is going to
// make it to stable but we don't want to inadvertlty use other
// features of nightly that might be removed.
use std::{
...
use crate::{
	...
	git_events::{
		event_is_cover_letter, event_is_patch_set_root,
		event_is_revision_root, status_kinds,
	},
...
impl Connect for Client {
	fn default() -> Self {
		let fallback_relays: Vec<String> =
		...
		let more_fallback_relays: Vec<String> = if std::env::var(
			"NGITTEST",
		)
		.is_ok()
		{
	...
	async fn connect(&self, relay_url: &Url) -> Result<()> {
		...
		if !relay.is_connected().await {
			...
			relay
				.connect(Some(std::time::Duration::from_secs(
					CONNECTION_TIMEOUT,
				)))
				.await;
		}
	...
	async fn send_event_to(
		...
		let res = self
			.client
			.send_event_to(vec![url], event.clone())
			.await?;
	...
	async fn get_events_per_relay(
		...
		let futures: Vec<_> = relays
			...
			.filter(|r| {
				!r.as_str().contains("nostr.mutinywallet.com")
			})
	...
	async fn fetch_all(
		...
		loop {
			...
			let futures: Vec<_> = relays
				...
				.map(|r| {
					if profile_relays_only.contains(r) {
						// if relay isn't a repo relay, just filter
						// for user profile
						FetchRequest {
				...
				.map(|request| async {
					let relay_column_width =
						request.relay_column_width;

					let relay_url =
						request.selected_relay.clone().context(
							"fetch_all_from_relay called without a relay",
						)?;
			...
			if let Ok(repo_ref) = get_repo_ref_from_cache(
				git_repo_path,
				repo_coordinates,
			)
			.await
			...
			request.user_relays_for_profiles = {
				...
				{
					if let Ok(user_ref) =
						get_user_ref_from_cache(git_repo_path, user)
							.await
					{
	...
	async fn fetch_all_from_relay(
		...
		let mut fresh_coordinates: HashSet<Coordinate> =
			HashSet::new();
		...
		loop {
			...
			let events: Vec<nostr::Event> =
				get_events_of(&relay, filters.clone(), &None)
					.await?
					.iter()
					// don't process events that don't match filters
					.filter(|e| {
						filters.iter().any(|f| f.match_event(e))
					})
					.cloned()
					.collect();
			// TODO: try reconcile
...
fn get_dedup_events(
	relay_results: Vec<Result<Vec<nostr::Event>>>,
) -> Vec<Event> {
...
fn pb_style() -> Result<ProgressStyle> {
	Ok(ProgressStyle::with_template(
		" {spinner} {prefix} {msg} {timeout_in}",
	)?
	.with_key(
		"timeout_in",
		|state: &ProgressState, w: &mut dyn Write| {
			if state.elapsed().as_secs() > 3
				&& state.elapsed().as_secs() < GET_EVENTS_TIMEOUT
			{
				let dim = Style::new().color256(247);
				write!(
					w,
...
async fn get_global_cache_database(
	git_repo_path: &Path,
) -> Result<SQLiteDatabase> {
...
pub async fn get_event_from_global_cache(
	...
	get_global_cache_database(git_repo_path)
		...
		.await
		.context(
			"cannot execute query on opened ngit nostr cache database",
		)
}
...
pub async fn get_repo_ref_from_cache(
	...
	loop {
		...
		let events = [
			get_event_from_global_cache(git_repo_path, vec![
				repo_events_filter.clone(),
			])
			.await?,
			get_events_from_cache(git_repo_path, vec![
				repo_events_filter,
			])
			.await?,
		]
	...
	let mut events: HashMap<Coordinate, nostr::Event> =
		HashMap::new();
	...
	Ok(RepoRef {
		// use all maintainers from all events found, not just
		// maintainers in the most recent event
...
pub async fn get_state_from_cache(
	...
	RepoState::try_from(
		get_events_from_cache(git_repo_path, vec![
			get_filter_state_events(&repo_ref.coordinates()),
		])
		.await?,
	)
}
...
async fn create_relays_request(
	...
	let repo_ref =
		get_repo_ref_from_cache(git_repo_path, repo_coordinates)
			.await;

	let repo_coordinates = {
		// add coordinates of users listed in maintainers to
		// explicitly specified coodinates
		let mut repo_coordinates = repo_coordinates.clone();
		if let Ok(repo_ref) = &repo_ref {
			for c in repo_ref.coordinates() {
				if !repo_coordinates.iter().any(|e| {
					e.identifier.eq(&c.identifier)
						&& e.public_key.eq(&c.public_key)
				}) {
	...
	let mut missing_contributor_profiles: HashSet<PublicKey> =
		HashSet::new();
	...
	if !repo_coordinates_without_relays.is_empty() {
		...
		for event in &get_events_from_cache(git_repo_path, vec![
			nostr::Filter::default()
				.kinds(vec![Kind::GitPatch])
				.custom_tag(
					SingleLetterTag::lowercase(
		...
		let profile_events =
			get_event_from_global_cache(git_repo_path, vec![
				get_filter_contributor_profiles(contributors.clone()),
			])
			.await?;
		for c in &contributors {
			if let Some(event) = profile_events.iter().find(|e| {
				e.kind() == Kind::Metadata && e.author().eq(c)
			}) {
				save_event_in_cache(git_repo_path, event).await?;
			} else {
	...
	let profiles_to_fetch_from_user_relays = {
		...
		if let Ok(Some(current_user)) =
			get_logged_in_user(git_repo_path).await
		{
			user_profiles.insert(current_user);
		}
		let mut map: HashMap<PublicKey, (Timestamp, Timestamp)> =
			HashMap::new();
		for public_key in &user_profiles {
			if let Ok(user_ref) =
				get_user_ref_from_cache(git_repo_path, public_key)
					.await
			{
				map.insert(
					public_key.to_owned(),
	...
	let user_relays_for_profiles = {
		...
		{
			if let Ok(user_ref) =
				get_user_ref_from_cache(git_repo_path, user).await
			{
				for r in user_ref.relays.write() {
					if let Ok(url) = Url::parse(&r) {
						set.insert(url);
	...
	Ok(FetchRequest {
		...
		repo_relays: relays,
		relay_column_width,
		repo_coordinates_without_relays: if let Ok(repo_ref) =
			&repo_ref
		{
		...
		state: if let Ok(repo_ref) = &repo_ref {
			if let Ok(existing_state) =
				get_state_from_cache(git_repo_path, repo_ref).await
			{
				Some((
					existing_state.event.created_at,
					existing_state.event.id,
				))
			} else {
				None
			}
...
async fn process_fetched_events(
	...
	for event in &events {
		if !request.existing_events.contains(&event.id) {
			save_event_in_cache(git_repo_path, event).await?;
			if event.kind().eq(&Kind::GitRepoAnnouncement) {
				save_event_in_global_cache(git_repo_path, event)
					.await?;
					...
					for m in &repo_ref.maintainers {
						if !request
							...
							.union(
								&report
									.repo_coordinates_without_relays,
							) // already added maintainers
							.any(|c| {
								c.identifier.eq(&repo_ref.identifier)
									&& m.eq(&c.public_key)
							}) {
							let c = Coordinate {
								kind: event.kind(),
				...
				if let Some((timestamp, id)) = existing_state {
					if event.created_at.gt(&timestamp)
						|| (event.created_at.eq(&timestamp)
							&& event.id.gt(&id))
					{
						report.updated_state =
							Some((event.created_at, event.id));
					}
				}
			...
			} else if [Kind::RelayList, Kind::Metadata]
				.contains(&event.kind())
			{
				if request
					.missing_contributor_profiles
					.contains(&event.author())
				{
					...
					_,
					(metadata_timestamp, relay_list_timestamp),
				)) = request
					.profiles_to_fetch_from_user_relays
...
pub fn consolidate_fetch_reports(
	reports: Vec<Result<FetchReport>>,
) -> FetchReport {
	let mut report = FetchReport::default();
	for relay_report in reports.into_iter().flatten() {
		...
		for (r, t) in relay_report.updated_repo_announcements {
			...
			{
				let (_, existing_t) =
					&report.updated_repo_announcements[i];
				if t.gt(existing_t) {
		...
		if let Some((timestamp, id)) = relay_report.updated_state {
			if let Some((existing_timestamp, existing_id)) =
				report.updated_state
			{
				if timestamp.gt(&existing_timestamp)
					|| (timestamp.eq(&existing_timestamp)
						&& id.gt(&existing_id))
				{
...
pub fn get_fetch_filters(
	...
	[
		...
		if proposal_ids.is_empty() {
			...
			vec![
				nostr::Filter::default()
					...
					.kinds(
						[
							vec![Kind::GitPatch, Kind::EventDeletion],
							status_kinds(),
						]
						.concat(),
					),
		...
		if required_profiles.is_empty() {
			...
			vec![get_filter_contributor_profiles(
				required_profiles.clone(),
			)]
		},
	]
	.concat()
...
pub fn get_filter_contributor_profiles(
...
impl Display for FetchReport {
	fn fmt(
		...
		// report: "1 new maintainer, 1 announcement, 1 proposal, 3
		// commits, 2 statuses"
...
pub struct FetchRequest {
	...
	repo_coordinates_without_relays:
		Vec<(Coordinate, Option<Timestamp>)>,
	...
	profiles_to_fetch_from_user_relays:
		HashMap<PublicKey, (Timestamp, Timestamp)>,
...
pub async fn get_proposals_and_revisions_from_cache(
	...
	let mut proposals = get_events_from_cache(git_repo_path, vec![
		nostr::Filter::default()
			.kind(nostr::Kind::GitPatch)
			.custom_tag(
				nostr::SingleLetterTag::lowercase(
					nostr_sdk::Alphabet::A,
				),
				repo_coordinates
					.iter()
...
pub async fn get_all_proposal_patch_events_from_cache(
	...
	let permissioned_users: HashSet<PublicKey> =
		[repo_ref.maintainers.clone(), vec![
			commit_events
				.iter()
		...
		.collect();
	commit_events
		.retain(|e| permissioned_users.contains(&e.author()));
	...
	Ok(commit_events
		.iter()
		.filter(|e| {
			!event_is_cover_letter(e)
				&& permissioned_users.contains(&e.author())
		})
		.cloned()
		.collect())
}

pub async fn get_event_from_cache_by_id(
	git_repo: &Repo,
	event_id: &EventId,
) -> Result<Event> {
	Ok(get_events_from_cache(git_repo.get_path()?, vec![
...
pub async fn send_events(
	...
	for r in &repo_read_relays {
		...
		if !my_write_relays
			...
			&& !relays
				.iter()
				.any(|x| r_clean.eq(&remove_trailing_slash(x)))
		{
			relays.push(r);
		}
	}
	...
	let pb_after_style = |symbol| {
		ProgressStyle::with_template(
			format!(" {symbol} {}", "{prefix} {msg}",).as_str(),
		)
	};
