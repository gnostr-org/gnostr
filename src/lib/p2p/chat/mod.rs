...
pub async fn chat(sub_command_args: &ChatSubCommands) -> Result<(), anyhow::Error> {
    ...
    let private_key = gnostr_types::PrivateKey::try_from_hex_string(&nsec_hex).unwrap();
    let keys = gnostr_types::KeySigner::from_private_key(private_key, "", 1).unwrap();
    ...
    let pre_event = gnostr_types::PreEvent {
        ...
        kind: gnostr_types::EventKind::Metadata,
    ...
    println!(
        "pre_event={:?}",
        Into::<gnostr_types::PublicKeyHex>::into(pre_event.pubkey)
    );
