...
- **Command-Line Interface:** Supports starting the chat application with various configurations, including NSEC keys, topics, logging levels, `--headless` background mode, and `--oneshot` message delivery.
...
- **`mod.rs`**: The main module orchestrating the chat application. It handles CLI argument parsing (`ChatCli`, `ChatSubCommands`), Nostr client setup, Git integration, local relay startup, P2P networking initialization (`evt_loop`), TUI setup, and logging.
...
- **`p2p.rs`**: Implements the peer-to-peer networking layer using `libp2p`. It sets up the `MyBehaviour` (combining Gossipsub, relay client support, and mDNS), manages network connections, message publishing, and subscription to topics.
...
The chat functionality can typically be launched via `gnostr chat` with various arguments for configuration. For background publishing, start `gnostr chat --headless` first and wait for a connected peer before sending `--oneshot` messages.
