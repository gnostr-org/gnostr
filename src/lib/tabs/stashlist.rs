...
impl StashList {
    ...
    pub fn new(env: &Environment) -> Self {
        Self {
            visible: false,
            list: CommitList::new(env, &strings::stashlist_title(&env.key_config)),
    ...
    fn drop_stash(&mut self) {
        if self.list.marked_count() > 0 {
            self.queue
                .push(InternalEvent::ConfirmAction(Action::StashDrop(
                    self.list.marked_commits(),
                )));
        } else if let Some(e) = self.list.selected_entry() {
            self.queue
                .push(InternalEvent::ConfirmAction(Action::StashDrop(vec![e.id])));
        }
    }
    ...
    fn pop_stash(&mut self) {
        if let Some(e) = self.list.selected_entry() {
            self.queue
                .push(InternalEvent::ConfirmAction(Action::StashPop(e.id)));
        }
    }
    ...
    fn drop(&mut self, repo: &RepoPath, ids: &[CommitId]) -> Result<()> {
        for id in ids {
...
impl Component for StashList {
    fn commands(&self, out: &mut Vec<CommandInfo>, force_all: bool) -> CommandBlocking {
        if self.visible || force_all {
            ...
            let selection_valid = self.list.selected_entry().is_some();
            ...
            out.push(CommandInfo::new(
                strings::commands::stashlist_drop(&self.key_config, self.list.marked_count()),
                selection_valid,
                true,
            ));
            out.push(CommandInfo::new(
                strings::commands::stashlist_inspect(&self.key_config),
                selection_valid,
                true,
            ));
        }
        ...
        if self.is_visible() {
            ...
            if let Event::Key(k) = ev {
                if key_match(k, self.key_config.keys.enter) {
                    self.pop_stash();
                } else if key_match(k, self.key_config.keys.stash_apply) {
                    self.apply_stash();
                } else if key_match(k, self.key_config.keys.stash_drop) {
                    self.drop_stash();
                } else if key_match(k, self.key_config.keys.stash_open) {
                    self.inspect();
                }
            }
        }
