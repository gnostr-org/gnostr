...
impl Revlog {
    ...
    pub fn new(env: &Environment) -> Self {
        Self {
            ...
            git_local_branches: AsyncSingleJob::new(env.sender_git.clone()),
            git_remote_branches: AsyncSingleJob::new(env.sender_git.clone()),
            visible: false,
    ...
    pub fn update(&mut self) -> Result<()> {
        if self.is_visible() {
            ...
            if self.commit_details.is_visible() {
                ...
                self.commit_details
                    .set_commits(commit.map(CommitFilesParams::from), &tags)?;
            }
        }
        ...
        if self.visible {
            match ev {
                ...
                AsyncGitNotification::Branches => {
                    if let Some(local_branches) = self.git_local_branches.take_last() {
                        if let Some(Ok(local_branches)) = local_branches.result() {
                            self.list.set_local_branches(local_branches);
                            self.update()?;
                        }
                    }

                    if let Some(remote_branches) = self.git_remote_branches.take_last() {
                        if let Some(Ok(remote_branches)) = remote_branches.result() {
                            self.list.set_remote_branches(remote_branches);
                            self.update()?;
                        }
                    }
                }
    ...
    fn inspect_commit(&self) {
        if let Some(commit_id) = self.selected_commit() {
            ...
            self.queue
                .push(InternalEvent::OpenPopup(StackablePopupOpen::InspectCommit(
    ...
    pub fn search(&mut self, options: LogFilterSearchOptions) {
        ...
        if matches!(self.search, LogSearch::Off | LogSearch::Results(_)) {
            log::info!("start search: {:?}", options);

            let filter = filter_commit_by_search(LogFilterSearch::new(options.clone()));
            ...
            self.search = LogSearch::Searching(job, options, None, Arc::clone(&cancellation_flag));
    ...
    fn update_search_state(&mut self) {
        match &mut self.search {
            ...
            LogSearch::Searching(search, options, progress, cancel) => {
                if search.is_pending() {
                    //update progress
                    *progress = search.progress();
                } else if let Some(search) = search.take_last().and_then(|search| search.result()) {
                    match search {
                        Ok(search) => {
                            let was_aborted = cancel.load(Ordering::Relaxed);

                            self.search = if was_aborted {
                                LogSearch::Off
                            } else {
                                self.list.set_highlighting(Some(Rc::new(
                                    search.result.into_iter().collect::<IndexSet<_>>(),
                                )));
                        ...
                        Err(err) => {
                            self.queue
                                .push(InternalEvent::ShowErrorMsg(format!("search error: {err}",)));
...
impl Component for Revlog {
        ...
        if self.visible {
            ...
            if event_used.is_consumed() {
                ...
                if key_match(k, self.key_config.keys.enter) {
                    ...
                    try_or_popup!(self, strings::POPUP_FAIL_COPY, self.list.copy_commit_hash());
                    return Ok(EventState::Consumed);
                } else if key_match(k, self.key_config.keys.push) {
                    self.queue.push(InternalEvent::PushTags);
                    return Ok(EventState::Consumed);
                } else if key_match(k, self.key_config.keys.log_tag_commit) {
                    return self
                        .selected_commit()
                        .map_or(Ok(EventState::NotConsumed), |id| {
                            self.queue.push(InternalEvent::TagCommit(id));
                    ...
                    && !self.is_search_pending()
                {
                    ...
                    && !self.is_search_pending()
                {
                    try_or_popup!(self, "revert error:", self.revert_commit());
                    ...
                    && !self.is_search_pending()
                {
                    return self
                        ...
                        .map_or(Ok(EventState::NotConsumed), |id| {
                            self.queue.push(InternalEvent::OpenPopup(
                                StackablePopupOpen::FileTree(FileTreeOpen::new(id)),
                            ));
                            Ok(EventState::Consumed)
                        });
                    ...
                    return Ok(EventState::Consumed);
                } else if key_match(k, self.key_config.keys.log_reset_commit)
                    && !self.is_search_pending()
                {
                    return self
                        .selected_commit()
                        .map_or(Ok(EventState::NotConsumed), |id| {
                            self.queue.push(InternalEvent::OpenResetPopup(id));
                            Ok(EventState::Consumed)
                        });
                } else if key_match(k, self.key_config.keys.log_reword_commit)
                    && !self.is_search_pending()
                {
                    return self
                        .selected_commit()
                        .map_or(Ok(EventState::NotConsumed), |id| {
                            self.queue.push(InternalEvent::RewordCommit(id));
                            Ok(EventState::Consumed)
                        });
                } else if key_match(k, self.key_config.keys.log_find) && self.can_start_search() {
                    self.queue.push(InternalEvent::OpenLogSearchPopup);
                    return Ok(EventState::Consumed);
                } else if key_match(k, self.key_config.keys.compare_commits)
                    ...
                    && !self.is_search_pending()
                {
                    if self.list.marked_count() == 1 {
                        ...
                        self.queue.push(InternalEvent::OpenPopup(
                            StackablePopupOpen::CompareCommits(InspectCommitOpen::new(
                        ...
                        self.queue.push(InternalEvent::OpenPopup(
                            StackablePopupOpen::CompareCommits(InspectCommitOpen {
    ...
    fn commands(&self, out: &mut Vec<CommandInfo>, force_all: bool) -> CommandBlocking {
        ...
        out.push(
            CommandInfo::new(
                ...
                true,
                (self.visible && (self.can_close_search() || self.is_search_pending()))
                    || force_all,
            )
        ...
        out.push(CommandInfo::new(
            ...
            true,
            (self.visible && self.commit_details.is_visible()) || force_all,
        ));
        ...
        out.push(CommandInfo::new(
            ...
            (self.visible && !self.is_search_pending() && self.list.marked_count() <= 1)
                || force_all,
        ));
        ...
        out.push(CommandInfo::new(
            ...
            true,
            (self.visible && !self.is_search_pending() && self.list.marked_count() == 2)
                || force_all,
        ));
    ...
    fn show(&mut self) -> Result<()> {
        ...
        self.git_local_branches
            .spawn(AsyncBranchesJob::new(self.repo.borrow().clone(), true));

        self.git_remote_branches
            .spawn(AsyncBranchesJob::new(self.repo.borrow().clone(), false));
