...
impl Stashing {
    ...
    pub fn new(env: &Environment) -> Self {
        Self {
            ...
            git_status: AsyncStatus::new(env.repo.borrow().clone(), env.sender_git.clone()),
    ...
    fn get_option_text(&self) -> Vec<Line> {
        ...
        let option_on = Span::styled(Cow::from("x"), self.theme.option(true));

        let option_off = Span::styled(Cow::from("_"), self.theme.option(false));
...
impl DrawableComponent for Stashing {
    fn draw(&self, f: &mut ratatui::Frame, rect: ratatui::layout::Rect) -> Result<()> {
        let chunks = Layout::default()
            ...
            .constraints([Constraint::Min(1), Constraint::Length(22)].as_ref())
            .split(rect);
        ...
        f.render_widget(
            Paragraph::new(self.get_option_text())
                .block(
                    Block::default()
                        .borders(Borders::ALL)
                        .title(strings::stashing_options_title(&self.key_config)),
                )
...
impl Component for Stashing {
    fn commands(&self, out: &mut Vec<CommandInfo>, force_all: bool) -> CommandBlocking {
        if self.visible || force_all {
            ...
            out.push(CommandInfo::new(
                strings::commands::stashing_toggle_indexed(&self.key_config),
                self.visible,
                self.visible || force_all,
            ));
            out.push(CommandInfo::new(
                strings::commands::stashing_toggle_untracked(&self.key_config),
                self.visible,
                self.visible || force_all,
            ));
        }
    ...
    fn event(&mut self, ev: &crossterm::event::Event) -> Result<EventState> {
        if self.visible {
            ...
            if let Event::Key(k) = ev {
                return if key_match(k, self.key_config.keys.stashing_save) && !self.index.is_empty()
                {
                    self.queue.push(InternalEvent::PopupStashing(self.options));

                    Ok(EventState::Consumed)
                } else if key_match(k, self.key_config.keys.stashing_toggle_index) {
    ...
    fn show(&mut self) -> Result<()> {
        let config_untracked_files = sync::untracked_files_config(&self.repo.borrow())?;

        self.options.stash_untracked = !config_untracked_files.include_none();
