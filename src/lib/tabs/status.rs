...
impl Status {
    ...
    pub fn new(env: &Environment) -> Self {
        ...
        Self {
            ...
            index: ChangesComponent::new(env, &strings::title_index(&env.key_config), false, false),
            diff: DiffComponent::new(env, false),
            git_diff: AsyncDiff::new(repo_clone.clone(), &env.sender_git),
            git_status_workdir: AsyncStatus::new(repo_clone.clone(), env.sender_git.clone()),
            git_status_stage: AsyncStatus::new(repo_clone, env.sender_git.clone()),
            git_action_executed: false,
            git_branch_state: None,
            git_branch_name: cached::BranchName::new(env.repo.clone()),
    ...
    fn draw_branch_state(&self, f: &mut ratatui::Frame, chunks: &[ratatui::layout::Rect]) {
        if let Some(branch_name) = self.git_branch_name.last() {
            let ahead_behind = self
                .git_branch_state
                .as_ref()
                .map_or_else(String::new, |state| {
                    format!("\u{2191}{} \u{2193}{} ", state.ahead, state.behind,)
                });

            let w = Paragraph::new(format!("{ahead_behind}{{{branch_name}}}"))
            ...
            rect.height = rect.height.saturating_sub(rect.height.saturating_sub(1));
    ...
    fn repo_state_text(repo: &RepoPath, state: &RepoState) -> String {
        match state {
            RepoState::Merge => {
                let ids = sync::mergehead_ids(repo).unwrap_or_default();

                format!(
                    "Commits: {}",
                    ids.iter().map(sync::CommitId::get_short_string).join(",")
                )
            }
            RepoState::Rebase => sync::rebase_progress(repo).map_or_else(
                |_| String::new(),
                |p| {
                    format!(
                        "Step: {}/{} Current Commit: {}",
    ...
    fn draw_repo_state(&self, f: &mut ratatui::Frame, r: ratatui::layout::Rect) {
        if self.git_state != RepoState::Clean {
            let txt = Self::repo_state_text(&self.repo.borrow(), &self.git_state);
            ...
            let w = Paragraph::new(txt)
                .block(
                    Block::default()
                        ...
                        .title(format!("Pending {:?}", self.git_state)),
                )
    ...
    pub fn update(&mut self) -> Result<()> {
        ...
        if self.is_visible() {
            ...
            self.git_status_workdir
                .fetch(&StatusParams::new(StatusType::WorkingDir, config))?;
            self.git_status_stage
                .fetch(&StatusParams::new(StatusType::Stage, config))?;

            self.git_state = sync::repo_state(&self.repo.borrow()).unwrap_or(RepoState::Clean);
    ...
    fn check_remotes(&mut self) {
        self.remotes.has_remote_for_fetch =
            sync::get_default_remote_for_fetch(&self.repo.borrow().clone()).is_ok();
    ...
    fn update_status(&mut self) -> Result<()> {
        ...
        if self.git_action_executed {
            ...
            } else if self.focus == Focus::Stage && stage_status.items.is_empty() {
    ...
    pub fn reset(&mut self, item: &ResetItem) -> bool {
        if let Err(e) = sync::reset_workdir(&self.repo.borrow(), item.path.as_str()) {
            self.queue
                .push(InternalEvent::ShowErrorMsg(format!("reset failed:\n{e}")));
    ...
    fn push(&self, force: bool) {
        if self.can_push() {
            if let Some(branch) = self.git_branch_name.last() {
                if force {
                    self.queue
                        .push(InternalEvent::ConfirmAction(Action::ForcePush(
                            branch, force,
                        )));
                } else {
                    self.queue
                        .push(InternalEvent::Push(branch, PushType::Branch, force, false));
                }
            }
        }
    }
    ...
    fn branch_compare(&mut self) {
        self.git_branch_state = self.git_branch_name.last().and_then(|branch| {
            sync::branch_compare_upstream(&self.repo.borrow(), branch.as_str()).ok()
        });
    }
    ...
    const fn can_fetch(&self) -> bool {
        self.remotes.has_remote_for_fetch && self.git_branch_state.is_some()
    }
    ...
    fn commands_nav(&self, out: &mut Vec<CommandInfo>, force_all: bool) {
        ...
        out.push(
            CommandInfo::new(
                ...
                !focus_on_diff,
                (self.visible && !focus_on_diff && self.focus == Focus::WorkDir) || force_all,
            )
            .order(strings::order::NAV),
        );
        out.push(
            CommandInfo::new(
                ...
                !focus_on_diff,
                (self.visible && !focus_on_diff && self.focus == Focus::Stage) || force_all,
            )
            .order(strings::order::NAV),
        );
    }
    ...
    fn can_commit(&self) -> bool {
        self.index.focused() && !self.index.is_empty() && !self.pending_rebase()
    }
}
    ...
    fn commands(&self, out: &mut Vec<CommandInfo>, force_all: bool) -> CommandBlocking {
        ...
        if self.visible || force_all {
            ...
            out.push(CommandInfo::new(
                strings::commands::open_branch_select_popup(&self.key_config),
                true,
                !focus_on_diff,
            ));
    ...
    fn event(&mut self, ev: &crossterm::event::Event) -> Result<EventState> {
        if self.visible {
            ...
            if let Event::Key(k) = ev {
                return if key_match(k, self.key_config.keys.open_commit) && self.can_commit() {
                    self.queue.push(InternalEvent::OpenCommit);
                    Ok(EventState::Consumed)
                } else if key_match(k, self.key_config.keys.toggle_workarea)
                    && !self.is_focus_on_diff()
                {
                    self.switch_focus(self.focus.toggled_focus())
                        .map(Into::into)
                } else if key_match(k, self.key_config.keys.move_right) && self.can_focus_diff() {
                    ...
                    && !self.is_focus_on_diff()
                {
                    ...
                    Ok(EventState::Consumed)
                } else if key_match(k, self.key_config.keys.force_push)
                    && !self.is_focus_on_diff()
                    && self.can_push()
                {
                    self.push(true);
                    Ok(EventState::Consumed)
                } else if key_match(k, self.key_config.keys.push) && !self.is_focus_on_diff() {
                    self.push(false);
                    Ok(EventState::Consumed)
                    ...
                    && self.can_fetch()
                {
                    self.pull();
                    Ok(EventState::Consumed)
                } else if key_match(k, self.key_config.keys.undo_commit) && !self.is_focus_on_diff()
                {
                    self.undo_last_commit();
                    self.queue.push(InternalEvent::Update(NeedsUpdate::ALL));
                    Ok(EventState::Consumed)
                } else if key_match(k, self.key_config.keys.abort_merge) {
                    if self.can_abort_merge() {
                        self.queue
                            .push(InternalEvent::ConfirmAction(Action::AbortMerge));
                    } else if self.pending_rebase() {
                        self.queue
                            .push(InternalEvent::ConfirmAction(Action::AbortRebase));
                    } else if self.pending_revert() {
                        self.queue
                            .push(InternalEvent::ConfirmAction(Action::AbortRevert));
                    }

                    Ok(EventState::Consumed)
                } else if key_match(k, self.key_config.keys.rebase_branch) && self.pending_rebase()
                {
                    self.continue_rebase();
                    self.queue.push(InternalEvent::Update(NeedsUpdate::ALL));
                    Ok(EventState::Consumed)
                } else if key_match(k, self.key_config.keys.view_submodules) {
