...
impl App {
    ...
    pub fn draw(&self, f: &mut Frame) -> Result<()> {
        ...
        if !fullscreen_popup_open {
            ...
            match self.tab {
                0 => self.chat_tab.draw(f, chunks_main[1])?,
...
impl App {
    ...
    #[allow(clippy::too_many_lines)]
    fn process_internal_event(&mut self, ev: InternalEvent) -> Result<NeedsUpdate> {
        let mut flags = NeedsUpdate::empty();
        match ev {
            ...
            InternalEvent::RenameBranch(branch_ref, cur_name) => {
                self.rename_branch_popup.open(branch_ref, cur_name)?;
            }
            ...
            InternalEvent::FuzzyFinderChanged(idx, content, target) => {
                match target {
                    ...
                    FuzzyFinderTarget::Files => {
                        self.files_tab
                            .file_finder_update(&PathBuf::from(content.clone()));
                        self.revision_files_popup
                            .file_finder_update(&PathBuf::from(content));
                    }
                }
            ...
            InternalEvent::OpenRepo { path } => {
                let submodule_repo_path =
                    RepoPath::Path(Path::new(&repo_work_dir(&self.repo.borrow())?).join(path));
                //TODO: validate this is a valid repo first, so we
                // can show proper error otherwise
                self.do_quit = QuitState::OpenSubmodule(submodule_repo_path);
            }
    ...
    fn process_confirmed_action(&mut self, action: Action, flags: &mut NeedsUpdate) -> Result<()> {
        match action {
            Action::Reset(r) => {
            ...
            Action::ResetLines(path, lines) => {
                sync::discard_lines(&self.repo.borrow(), &path, &lines)?;
            }
            Action::DeleteLocalBranch(branch_ref) => {
                if let Err(e) = sync::delete_branch(&self.repo.borrow(), &branch_ref) {
                    self.queue.push(InternalEvent::ShowErrorMsg(e.to_string()));
                }
            ...
            Action::DeleteRemoteTag(tag_name, _remote) => {
                self.queue
                    .push(InternalEvent::Push(tag_name, PushType::Tag, false, true));
            }
    ...
    fn delete_tag(&mut self, tag_name: String) -> Result<()> {
        if let Err(error) = sync::delete_tag(&self.repo.borrow(), &tag_name) {
            self.queue
                .push(InternalEvent::ShowErrorMsg(error.to_string()));
        } else {
            let remote = sync::get_default_remote(&self.repo.borrow())?;

            self.queue
                .push(InternalEvent::ConfirmAction(Action::DeleteRemoteTag(
                    tag_name, remote,
                )));
    ...
    fn delete_remote_branch(&mut self, branch_ref: &str) -> Result<()> {
        self.queue.push(
            ...
            branch_ref.rsplit('/').next().map_or_else(
                ...
                |name| InternalEvent::Push(name.to_string(), PushType::Branch, false, true),
            ),
        );
    ...
    fn commands(&self, force_all: bool) -> Vec<CommandInfo> {
        ...
        res.push(CommandInfo::new(
            ...
            (!self.any_popup_visible() && self.chat_tab.is_visible())
        ...
        res.push(
            CommandInfo::new(
                strings::commands::toggle_tabs_direct(&self.key_config),
                true,
                !self.any_popup_visible(),
            )
            .order(order::NAV),
