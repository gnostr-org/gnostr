    ...
    fn draw(&self, f: &mut Frame, rect: Rect) -> Result<()> {
        if self.visible {
            ...
            let tag_name_width = self.tags.as_ref().map_or(0, |tags| {
                tags.iter().fold(0, |acc, tag| acc.max(tag.name.len()))
            });
            ...
            let table = Table::new(rows, constraints)
                ...
                .block(
                    Block::default()
                        .borders(Borders::ALL)
                        .title(Span::styled(strings::title_tags(), self.theme.title(true)))
...
impl Component for TagListPopup {
    fn commands(&self, out: &mut Vec<CommandInfo>, force_all: bool) -> CommandBlocking {
        if self.visible || force_all {
            ...
            out.push(CommandInfo::new(
                strings::commands::show_tag_annotation(&self.key_config),
    ...
    fn event(&mut self, event: &Event) -> Result<EventState> {
        if self.visible {
            if let Event::Key(key) = event {
                ...
                } else if key_match(key, self.key_config.keys.delete_tag) {
                    return self
                        .selected_tag()
                        .map_or(Ok(EventState::NotConsumed), |tag| {
                            self.queue
                                .push(InternalEvent::ConfirmAction(Action::DeleteTag(
                                    tag.name.clone(),
                                )));
                            Ok(EventState::Consumed)
                        });
                } else if key_match(key, self.key_config.keys.select_tag) {
                    return self
                        .selected_tag()
                        .map_or(Ok(EventState::NotConsumed), |tag| {
                            self.queue
                                .push(InternalEvent::SelectCommitInRevlog(tag.commit_id));
                            Ok(EventState::Consumed)
                        });
                } else if key_match(key, self.key_config.keys.push) && self.has_remotes {
...
impl TagListPopup {
    pub fn new(env: &Environment) -> Self {
        Self {
            ...
            async_remote_tags: AsyncSingleJob::new(env.sender_git.clone()),
    ...
    pub fn open(&mut self) -> Result<()> {
        ...
        self.has_remotes = sync::get_branches_info(&self.repo.borrow(), false)
            .map(|branches| !branches.is_empty())
            .unwrap_or(false);

        let basic_credential = if self.has_remotes {
            if need_username_password(&self.repo.borrow())? {
                let credential = extract_username_password(&self.repo.borrow())?;
        ...
        if matches!(ev, AsyncNotification::Git(AsyncGitNotification::RemoteTags)) {
            if let Some(job) = self.async_remote_tags.take_last() {
                if let Some(Ok(missing_remote_tags)) = job.result() {
                    self.missing_remote_tags = Some(missing_remote_tags);
                }
            }
        } else if matches!(ev, AsyncNotification::Git(AsyncGitNotification::PushTags)) {
            self.update_missing_remote_tags();
        }
    }
    ...
    fn move_selection(&mut self, scroll_type: ScrollType) -> bool {
        ...
        let max_selection = self.tags.as_ref().map_or(0, |tags| tags.len() - 1);
    ...
    fn show_annotation(&self) {
        if let Some(tag) = self.selected_tag() {
            if let Some(annotation) = &tag.annotation {
                self.queue
                    .push(InternalEvent::ShowInfoMsg(annotation.clone()));
            }
        }
    }
    ...
    fn get_row(&self, tag: &TagWithMetadata) -> Row {
        ...
        let cells: Vec<Cell> = vec![
            Cell::from(has_remote_str).style(self.theme.commit_author(false)),
            Cell::from(tag.name.clone()).style(self.theme.text(true, false)),
            Cell::from(time_to_string(tag.time, true)).style(self.theme.commit_time(false)),
            Cell::from(tag.author.clone()).style(self.theme.commit_author(false)),
            Cell::from(has_attachement_str).style(self.theme.text_danger()),
            Cell::from(tag.message.clone()).style(self.theme.text(true, false)),
        ];
