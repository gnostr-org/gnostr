...
impl OptionsPopup {
    ...
    fn add_entry(
        ...
        txt.push(Line::from(vec![
            ...
            Span::styled(format!("{value:^half$}"), self.theme.text(true, selected)),
        ]));
    }
    ...
    fn move_selection(&mut self, up: bool) {
        if up {
            self.selection = match self.selection {
                AppOption::StatusShowUntracked => AppOption::DiffInterhunkLines,
                AppOption::DiffIgnoreWhitespaces => AppOption::StatusShowUntracked,
                AppOption::DiffContextLines => AppOption::DiffIgnoreWhitespaces,
                AppOption::DiffInterhunkLines => AppOption::DiffContextLines,
            };
        } else {
            self.selection = match self.selection {
                AppOption::StatusShowUntracked => AppOption::DiffIgnoreWhitespaces,
                AppOption::DiffIgnoreWhitespaces => AppOption::DiffContextLines,
                AppOption::DiffContextLines => AppOption::DiffInterhunkLines,
                AppOption::DiffInterhunkLines => AppOption::StatusShowUntracked,
            };
        }
    }
        ...
        if right {
            match self.selection {
                AppOption::StatusShowUntracked => {
                    let untracked = self.options.borrow().status_show_untracked();

                    let untracked = match untracked {
                        None => Some(ShowUntrackedFilesConfig::Normal),
                        Some(ShowUntrackedFilesConfig::Normal) => {
                            Some(ShowUntrackedFilesConfig::All)
                        }
                        Some(ShowUntrackedFilesConfig::All) => Some(ShowUntrackedFilesConfig::No),
                ...
                AppOption::DiffIgnoreWhitespaces => {
                    self.options.borrow_mut().diff_toggle_whitespace();
                }
                AppOption::DiffContextLines => {
                    self.options.borrow_mut().diff_context_change(true);
                }
                AppOption::DiffInterhunkLines => {
                    self.options.borrow_mut().diff_hunk_lines_change(true);
                }
            };
        } else {
            match self.selection {
                AppOption::StatusShowUntracked => {
                    ...
                    let untracked = match untracked {
                        ...
                        Some(ShowUntrackedFilesConfig::No) => Some(ShowUntrackedFilesConfig::All),
                        Some(ShowUntrackedFilesConfig::All) => {
                            Some(ShowUntrackedFilesConfig::Normal)
                        }
                        Some(ShowUntrackedFilesConfig::Normal) => None,
                    };
                ...
                AppOption::DiffContextLines => {
                    self.options.borrow_mut().diff_context_change(false);
                }
                AppOption::DiffInterhunkLines => {
                    self.options.borrow_mut().diff_hunk_lines_change(false);
                }
            };
        }
            ...
            f.render_widget(
                Paragraph::new(self.get_text(width))
                    .block(
                        Block::default()
                            .borders(Borders::ALL)
                            .title(Span::styled("Options", self.theme.title(true)))
