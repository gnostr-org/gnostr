...
impl FileRevlogPopup {
    ...
    pub fn any_work_pending(&self) -> bool {
        self.git_diff.is_pending() || self.git_log.as_ref().map_or(false, AsyncLog::is_pending)
    }
                ...
                if let Some(open_request) = &self.open_request {
                    ...
                    if let Some((params, last)) = self.git_diff.last()? {
                        if params == diff_params {
                            self.diff
                                .update(open_request.file_path.to_string(), false, last);
    ...
    fn fetch_commits(&mut self, new_offset: usize, new_max_offset: usize) -> Result<()> {
        if let Some(git_log) = &mut self.git_log {
            let amount = new_max_offset.saturating_sub(new_offset).max(SLICE_SIZE);
    ...
    fn selected_commit(&self) -> Option<CommitId> {
        ...
        let commit_id = table_state.selected().and_then(|selected| {
            self.items
                .iter()
                .nth(selected.saturating_sub(self.items.index_offset()))
                .as_ref()
                .map(|entry| entry.id)
        });
    ...
    fn get_title(&self) -> String {
        ...
        self.open_request
            .as_ref()
            .map_or("<no history available>".into(), |open_request| {
                strings::file_log_title(&open_request.file_path, selected, revisions)
            })
    }
        ...
        self.items
            ...
            .map(|entry| {
                let spans = Line::from(vec![
                    Span::styled(entry.hash_short.to_string(), self.theme.commit_hash(false)),
                    Span::raw(" "),
                    Span::styled(entry.time_to_string(now), self.theme.commit_time(false)),
                    Span::raw(" "),
                    Span::styled(entry.author.to_string(), self.theme.commit_author(false)),
                ]);
    ...
    fn get_max_selection(&self) -> usize {
        self.git_log
            .as_ref()
            .map_or(0, |log| log.count().unwrap_or(0).saturating_sub(1))
    }

    fn move_selection(&mut self, scroll_type: ScrollType) -> Result<()> {
        let old_selection = self.table_state.get_mut().selected().unwrap_or(0);
        ...
        let new_selection = match scroll_type {
            ...
            ScrollType::PageUp => old_selection.saturating_sub(height_in_items.saturating_sub(2)),
    ...
    fn set_selection(&mut self, selection: usize) {
        let height_in_items = (self.current_height.get().saturating_sub(2)) / 4;

        let offset = *self.table_state.get_mut().offset_mut();
        let min_offset = selection.saturating_sub(height_in_items.saturating_sub(1));
    ...
    fn fetch_commits_if_needed(&mut self) -> Result<()> {
        let selection = self.table_state.get_mut().selected().unwrap_or(0);
        let offset = *self.table_state.get_mut().offset_mut();
        let height_in_items = (self.current_height.get().saturating_sub(2)) / 4;
        let new_max_offset = selection.saturating_add(height_in_items);
    ...
    fn draw_revlog(&self, f: &mut Frame, area: Rect) {
        ...
        let mut adjusted_table_state = TableState::default()
            .with_selected(
                table_state
                    .selected()
                    .map(|selected| selected.saturating_sub(self.items.index_offset())),
            )
            .with_offset(
                table_state
                    .offset()
        ...
        f.render_stateful_widget(table, area, &mut adjusted_table_state);
...
impl Component for FileRevlogPopup {
    fn event(&mut self, event: &Event) -> Result<EventState> {
        if self.is_visible() {
            if event_pump(event, self.components_mut().as_mut_slice())?.is_consumed() {
            ...
            if let Event::Key(key) = event {
                if key_match(key, self.key_config.keys.exit_popup) {
                ...
                } else if key_match(key, self.key_config.keys.move_right) && self.can_focus_diff() {
                    ...
                    if let Some(open_request) = self.open_request.clone() {
                        self.hide_stacked(true);
                        self.queue
                            .push(InternalEvent::OpenPopup(StackablePopupOpen::BlameFile(
                                BlameFileOpen {
                                    file_path: open_request.file_path,
