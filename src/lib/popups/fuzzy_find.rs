...
impl FuzzyFindPopup {
    ...
    fn update_query(&mut self) {
        ...
        {
            self.set_query(Some(self.find_text.get_text().to_string()));
        }
    }
    ...
    fn set_query(&mut self, query: Option<String>) {
        ...
        if let Some(q) = &self.query {
            ...
            contents.sort_by(|(score1, _, _), (score2, _, _)| score2.cmp(score1));

            self.filtered
                .extend(contents.into_iter().map(|entry| (entry.1, entry.2)));
        }
    ...
    fn refresh_selection(&mut self) {
        let selection = self.filtered.get(self.selection).map(|a| a.0);
        ...
        if self.selected_index != selection {
            ...
            if let Some(idx) = self.selected_index {
                if let Some(target) = self.target {
                    self.queue.push(InternalEvent::FuzzyFinderChanged(
                        idx,
    ...
    pub fn open(&mut self, contents: Vec<String>, target: FuzzyFinderTarget) -> Result<()> {
        self.show()?;
        self.find_text.show()?;
    ...
    fn draw_matches_list(&self, f: &mut Frame, mut area: Rect) {
        {
            ...
            let scroll_skip = self.selection.saturating_sub(list_height);
            ...
            let items =
                self.filtered
                    ...
                    .take(height)
                    .map(|(idx, indices)| {
                        let selected = self.selected_index.map_or(false, |index| index == *idx);
                        let full_text = trim_length_left(&self.contents[*idx], width);
                        let trim_length = self.contents[*idx].graphemes(true).count()
                            ...
                            full_text
                                ...
                                .map(|(c_idx, c)| {
                                    Span::styled(
                                        ...
                                        self.theme.text(
                                            selected,
                                            indices.contains(&(c_idx + trim_length)),
                                        ),
                                    )
                                })
                                .collect::<Vec<_>>(),
...
impl DrawableComponent for FuzzyFindPopup {
    fn draw(&self, f: &mut Frame, area: Rect) -> Result<()> {
        if self.is_visible() {
            ...
            let area = ui::centered_rect_absolute(MAX_SIZE.0, MAX_SIZE.1, area);
...
impl Component for FuzzyFindPopup {
    fn commands(&self, out: &mut Vec<CommandInfo>, force_all: bool) -> CommandBlocking {
        if self.is_visible() || force_all {
            ...
            out.push(CommandInfo::new(
                strings::commands::close_fuzzy_finder(&self.key_config),
                true,
                true,
            ));
        }
