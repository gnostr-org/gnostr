...
//InspectChatPopup
pub struct InspectChatPopup {
    queue: Queue,
    open_request: Option<InspectChatOpen>,
    //diff
    //DiffComponent
    diff: DiffComponent,
    //CommitDetailsComponent
...
impl DrawableComponent for InspectChatPopup {
...
impl Component for InspectChatPopup {
        ...
        if stack {
            if let Some(open_request) = self.open_request.take() {
                self.queue.push(InternalEvent::PopupStackPush(
                    StackablePopupOpen::InspectChat(open_request),
                ));
            }
        } else {
