...
impl Component for BranchListPopup {
    fn commands(&self, out: &mut Vec<CommandInfo>, force_all: bool) -> CommandBlocking {
        if self.visible || force_all {
            ...
            out.push(CommandInfo::new(
                strings::commands::toggle_branch_popup(&self.key_config, self.local),
                true,
                true,
            ));
            ...
            out.push(CommandInfo::new(
                strings::commands::open_branch_create_popup(&self.key_config),
                true,
                self.local,
            ));
    ...
    fn event(&mut self, ev: &Event) -> Result<EventState> {
        ...
        if let Event::Key(e) = ev {
            ...
            } else if key_match(e, self.key_config.keys.create_branch) && self.local {
                self.queue.push(InternalEvent::CreateBranch);
            } else if key_match(e, self.key_config.keys.rename_branch) && self.valid_selection() {
                self.rename_branch();
                ...
                && self.valid_selection()
            {
                ...
                && self.valid_selection()
            {
                try_or_popup!(self, "merge branch error:", self.merge_branch());
            } else if key_match(e, self.key_config.keys.rebase_branch)
                && !selection_is_cur_branch
                && self.valid_selection()
            {
                try_or_popup!(self, "rebase error:", self.rebase_branch());
            } else if key_match(e, self.key_config.keys.move_right) && self.valid_selection() {
                self.inspect_head_of_branch();
            } else if key_match(e, self.key_config.keys.compare_commits) && self.valid_selection() {
                self.hide();
                if let Some(commit_id) = self.get_selected_commit() {
                    self.queue.push(InternalEvent::OpenPopup(
                        StackablePopupOpen::CompareCommits(InspectCommitOpen::new(commit_id)),
                    ));
                }
            } else if key_match(e, self.key_config.keys.fetch) && self.has_remotes {
                self.queue.push(InternalEvent::FetchRemotes);
            } else if key_match(e, self.key_config.keys.reset_branch) {
                if let Some(commit_id) = self.get_selected_commit() {
                    self.queue.push(InternalEvent::OpenResetPopup(commit_id));
                }
            } else if key_match(e, self.key_config.keys.cmd_bar_toggle) {
                //do not consume if its the more key
                return Ok(EventState::NotConsumed);
            } else if key_match(e, self.key_config.keys.branch_find) {
                let branches = self.branches.iter().map(|b| b.name.clone()).collect();
...
impl BranchListPopup {
    ...
    fn move_event(&mut self, e: &KeyEvent) -> Result<EventState> {
        if key_match(e, self.key_config.keys.exit_popup) {
            ...
            return self.move_selection(ScrollType::PageDown).map(Into::into);
        } else if key_match(e, self.key_config.keys.page_up) {
            return self.move_selection(ScrollType::PageUp).map(Into::into);
        } else if key_match(e, self.key_config.keys.home) {
            return self.move_selection(ScrollType::Home).map(Into::into);
        } else if key_match(e, self.key_config.keys.end) {
            return self.move_selection(ScrollType::End).map(Into::into);
    ...
    fn check_remotes(&mut self) {
        if self.visible {
            self.has_remotes = get_branches_info(&self.repo.borrow(), false)
    ...
    pub fn update_branches(&mut self) -> Result<()> {
        if self.is_visible() {
            self.check_remotes();
            self.branches = get_branches_info(&self.repo.borrow(), self.local)?;
    ...
    fn merge_branch(&mut self) -> Result<()> {
        if let Some(branch) = self.branches.get(usize::from(self.selection)) {
            sync::merge_branch(&self.repo.borrow(), &branch.name, self.get_branch_type())?;
    ...
    fn rebase_branch(&mut self) -> Result<()> {
        if let Some(branch) = self.branches.get(usize::from(self.selection)) {
            sync::rebase_branch(&self.repo.borrow(), &branch.name, self.get_branch_type())?;
    ...
    fn selection_is_cur_branch(&self) -> bool {
        self.branches
            ...
            .filter(|(index, b)| {
                b.local_details()
                    .is_some_and(|details| details.is_head && *index == self.selection as usize)
            })
            .count()
            > 0
    }
    ...
    fn move_selection(&mut self, scroll: ScrollType) -> Result<bool> {
        let new_selection = match scroll {
            ...
            ScrollType::PageUp => self.selection.saturating_sub(self.current_height.get()),
            ScrollType::Home => 0,
            ScrollType::End => {
    ...
    fn switch_to_selected_branch(&mut self) -> Result<()> {
        ...
        if self.local {
            ...
            checkout_remote_branch(&self.repo.borrow(), &self.branches[self.selection as usize])?;
            self.local = true;
            self.update_branches()?;
        }
    ...
    fn draw_tabs(&self, f: &mut Frame, r: Rect) {
        let tabs: Vec<Line> = [Span::raw("Local"), Span::raw("Remote")]
            .iter()
            .cloned()
            .map(Line::from)
            .collect();
    ...
    fn draw_list(&self, f: &mut Frame, r: Rect) -> Result<()> {
        ...
        f.render_widget(
            Paragraph::new(self.get_text(&self.theme, r.width, height_in_lines))
    ...
    fn delete_branch(&mut self) {
        let reference = self.branches[self.selection as usize].reference.clone();

        self.queue.push(InternalEvent::ConfirmAction(if self.local {
