...
impl DrawableComponent for ConfirmPopup {
    fn draw(&self, f: &mut Frame, _rect: Rect) -> Result<()> {
        if self.visible {
            ...
            let txt = Text::styled(Cow::from(msg), self.theme.text_danger());

            let area = ui::centered_rect(50, 20, f.size());
            f.render_widget(Clear, area);
            f.render_widget(popup_paragraph(&title, txt, &self.theme, true, true), area);
        }
...
impl ConfirmPopup {
    ...
    fn get_text(&self) -> (String, String) {
        if let Some(ref a) = self.target {
            return match a {
                Action::Reset(_) => (strings::confirm_title_reset(), strings::confirm_msg_reset()),
                Action::StashDrop(ids) => (
                    strings::confirm_title_stashdrop(&self.key_config, ids.len() > 1),
                ...
                Action::DeleteLocalBranch(branch_ref) => (
                    strings::confirm_title_delete_branch(&self.key_config),
                    strings::confirm_msg_delete_branch(&self.key_config, branch_ref),
                ),
                Action::DeleteRemoteBranch(branch_ref) => (
                    strings::confirm_title_delete_remote_branch(&self.key_config),
                    strings::confirm_msg_delete_remote_branch(&self.key_config, branch_ref),
                ),
                Action::DeleteTag(tag_name) => (
                    strings::confirm_title_delete_tag(&self.key_config),
                    strings::confirm_msg_delete_tag(&self.key_config, tag_name),
                ),
                ...
                Action::ForcePush(branch, _force) => (
                    strings::confirm_title_force_push(&self.key_config),
                    strings::confirm_msg_force_push(
                        &self.key_config,
                        branch.rsplit('/').next().expect(
                            "There was no / in the head reference which is impossible in git",
                        ),
                    ),
                ),
