...
impl ChatPopup {
    ...
    pub fn new(env: &Environment) -> Self {
        Self {
            ...
            mode: Mode::Normal,
            input: TextInputComponent::new(env, "", &strings::commit_msg(&env.key_config), true),
    ...
    fn commit(&mut self) -> Result<()> {
        ...
        if matches!(self.commit_with_msg(msg)?, CommitResult::CommitDone) {
            self.options
                .borrow_mut()
    ...
    fn commit_with_msg(&mut self, msg: String) -> Result<CommitResult> {
        ...
        if verify {
            ...
            if let HookResult::NotOk(e) = sync::hooks_pre_commit(&self.repo.borrow())? {
        ...
        if verify {
            ...
            if let HookResult::NotOk(e) = sync::hooks_commit_msg(&self.repo.borrow(), &mut msg)? {
        ...
        if let HookResult::NotOk(e) = sync::hooks_post_commit(&self.repo.borrow())? {
    ...
    fn do_commit(&self, msg: &str) -> Result<()> {
        match &self.mode {
            ...
            Mode::Amend(amend) => sync::amend(&self.repo.borrow(), *amend, msg)?,
            Mode::Merge(ids) => sync::merge_commit(&self.repo.borrow(), msg, ids)?,
            Mode::Revert => sync::commit_revert(&self.repo.borrow(), msg)?,
            Mode::Reword(id) => {
                let commit = sync::reword(&self.repo.borrow(), *id, msg)?;
    ...
    fn is_changed(&self) -> bool {
        Some(self.input.get_text().trim()) != self.commit_template.as_ref().map(|s| s.trim())
    }
    ...
    fn amend(&mut self) -> Result<()> {
        if self.can_amend() {
            ...
            let details = sync::get_commit_details(&self.repo.borrow(), id)?;
    ...
    pub fn open(&mut self, reword: Option<CommitId>) -> Result<()> {
        ...
        let (mode, msg_source) = if repo_state != RepoState::Clean && reword.is_some() {
            bail!("cannot reword while repo is not in a clean state");
        } else if let Some(reword_id) = reword {
            self.input.set_text(
                sync::get_commit_details(&self.repo.borrow(), reword_id)?
                    .message
                    .unwrap_or_default()
                    .combine(),
            );
            ...
            match repo_state {
                RepoState::Merge => {
                    ...
                    self.input.set_text(sync::merge_msg(&self.repo.borrow())?);
                        ...
                        get_config_string(&self.repo.borrow(), "commit.template")
                            ...
                            .and_then(|path| {
                                shellexpand::full(path.as_str())
                                    .ok()
                                    .and_then(|path| PathBuf::from_str(path.as_ref()).ok())
                            })
                            .and_then(|path| {
                                read_to_string(&path)
                                    .map_err(|e| {
        ...
        if let HookResult::NotOk(e) =
            sync::hooks_prepare_commit_msg(&self.repo.borrow(), msg_source, &mut msg)?
        {
    ...
    fn add_sign_off(&self, msg: &str) -> Result<String> {
        ...
        let user = get_config_string(&self.repo.borrow(), CONFIG_KEY_USER_NAME)?;

        let mail = get_config_string(&self.repo.borrow(), CONFIG_KEY_USER_MAIL)?;
...
impl DrawableComponent for ChatPopup {
...
impl Component for ChatPopup {
    fn commands(&self, out: &mut Vec<CommandInfo>, force_all: bool) -> CommandBlocking {
        ...
        if self.is_visible() || force_all {
            ...
            out.push(CommandInfo::new(
                strings::commands::toggle_verify(&self.key_config, self.verify),
                self.can_commit(),
                true,
            ));
            ...
            out.push(CommandInfo::new(
                strings::commands::commit_open_editor(&self.key_config),
                true,
                true,
            ));

            out.push(CommandInfo::new(
                strings::commands::commit_next_msg_from_history(&self.key_config),
    ...
    fn event(&mut self, ev: &Event) -> Result<EventState> {
        if self.is_visible() {
            if let Event::Key(e) = ev {
                let input_consumed = if key_match(e, self.key_config.keys.commit)
                    && self.can_commit()
                {
                    try_or_popup!(self, "commit error:", self.commit());
                    true
                } else if key_match(e, self.key_config.keys.toggle_verify) && self.can_commit() {
                    self.toggle_verify();
                    true
                } else if key_match(e, self.key_config.keys.commit_amend) && self.can_amend() {
                    self.amend()?;
                    true
                    ...
                    true
                } else if key_match(e, self.key_config.keys.toggle_signoff) {
                    self.signoff_commit();
                    true
                } else {
                    false
