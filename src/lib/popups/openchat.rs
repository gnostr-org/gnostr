...
impl ExternalChatPopup {
    ...
    pub fn open_file_in_chat(repo: &RepoPath, path: &Path) -> Result<()> {
        ...
        let environment_options = ["gnostr-chat"];

        let editor = env::var(environment_options[0])
            .ok()
            .unwrap_or_else(|| String::from("gnostr-chat"));
        ...
        let mut args: Vec<&OsStr> = remainder.map(OsStr::new).collect();
...
impl DrawableComponent for ExternalChatPopup {
...
impl Component for ExternalChatPopup {
