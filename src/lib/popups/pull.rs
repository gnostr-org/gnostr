...
impl PullPopup {
    ...
    pub fn new(env: &Environment) -> Self {
        Self {
            ...
            visible: false,
            branch: String::new(),
            git_fetch: AsyncPull::new(env.repo.borrow().clone(), &env.sender_git),
            progress: None,
    ...
    pub fn fetch(&mut self, branch: String) -> Result<()> {
        ...
        if need_username_password_for_fetch(&self.repo.borrow())? {
            let cred = extract_username_password_for_fetch(&self.repo.borrow())
    ...
    fn fetch_from_remote(&mut self, cred: Option<BasicAuthCredential>) -> Result<()> {
        self.pending = true;
        self.progress = None;
        self.git_fetch.request(FetchRequest {
            remote: get_default_remote_for_fetch(&self.repo.borrow())?,
    ...
    fn update(&mut self) -> Result<()> {
        ...
        if !self.pending {
            if let Some((_bytes, err)) = self.git_fetch.last_result()? {
                if err.is_empty() {
                    self.try_ff_merge()?;
                } else {
                    anyhow::bail!(err);
    ...
    fn try_ff_merge(&mut self) -> Result<()> {
        let branch_compare = sync::branch_compare_upstream(&self.repo.borrow(), &self.branch)?;
        if branch_compare.behind > 0 {
            let ff_res = sync::branch_merge_upstream_fastforward(&self.repo.borrow(), &self.branch);
    ...
    pub fn try_conflict_free_merge(&self, rebase: bool) {
        if rebase {
            try_or_popup!(
                self,
                "rebase failed:",
                sync::merge_upstream_rebase(&self.repo.borrow(), &self.branch)
            );
        } else {
            try_or_popup!(
                self,
                "merge failed:",
                sync::merge_upstream_commit(&self.repo.borrow(), &self.branch)
            );
        }
    }
    ...
    fn confirm_merge(&mut self, incoming: usize) {
        self.queue
            .push(InternalEvent::ConfirmAction(Action::PullMerge {
                incoming,
                rebase: sync::config_is_pull_rebase(&self.repo.borrow()).unwrap_or_default(),
            }));
        self.hide();
    }
}
            ...
            if let Event::Key(_) = ev {
                if self.input_cred.is_visible() {
                    ...
                    if self.input_cred.get_cred().is_complete() || !self.input_cred.is_visible() {
                        self.fetch_from_remote(Some(self.input_cred.get_cred().clone()))?;
