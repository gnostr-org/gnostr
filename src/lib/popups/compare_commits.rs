...
impl CompareCommitsPopup {
    ...
    fn get_ids(&self) -> Option<OldNew<CommitId>> {
        let other = self.open_request.as_ref().and_then(|open| open.compare_id);

        let this = self.open_request.as_ref().map(|open| open.commit_id);
    ...
    fn update(&mut self) -> Result<()> {
        self.details
            .set_commits(self.get_ids().map(CommitFilesParams::from), &None)?;
        self.update_diff()?;
