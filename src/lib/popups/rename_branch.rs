    ...
    fn commands(&self, out: &mut Vec<CommandInfo>, force_all: bool) -> CommandBlocking {
        if self.is_visible() || force_all {
            ...
            out.push(CommandInfo::new(
                strings::commands::rename_branch_confirm_msg(&self.key_config),
                true,
                true,
            ));
        }
...
impl RenameBranchPopup {
    ...
    pub fn rename_branch(&mut self) {
        if let Some(br) = &self.branch_ref {
            ...
            self.queue.push(InternalEvent::ShowErrorMsg(
                "rename branch error: No branch selected to rename".to_string(),
            ));
        }
    ...
    fn draw_warnings(&self, f: &mut Frame) {
        ...
        if !current_text.is_empty() {
            let valid = sync::validate_branch_name(current_text).unwrap_or_default();
