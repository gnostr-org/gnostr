...
impl DrawableComponent for SubmodulesListPopup {
    fn draw(&self, f: &mut Frame, rect: Rect) -> Result<()> {
        if self.is_visible() {
            ...
            let area = ui::centered_rect(PERCENT_SIZE.width, PERCENT_SIZE.height, rect);
            ...
            let chunks = Layout::default()
                ...
                .constraints([Constraint::Min(40), Constraint::Length(60)].as_ref())
...
impl Component for SubmodulesListPopup {
    fn commands(&self, out: &mut Vec<CommandInfo>, force_all: bool) -> CommandBlocking {
        if self.visible || force_all {
            ...
            out.push(CommandInfo::new(
                strings::commands::open_submodule_parent(&self.key_config),
    ...
    fn event(&mut self, ev: &Event) -> Result<EventState> {
        ...
        if let Event::Key(e) = ev {
            ...
            } else if key_match(e, self.key_config.keys.update_submodule) {
                if let Some(submodule) = self.selected_entry() {
                    try_or_popup!(
                        self,
                        "update submodule:",
                        update_submodule(&self.repo.borrow(), &submodule.name,)
                    );
            ...
            } else if key_match(e, self.key_config.keys.view_submodule_parent) {
...
impl SubmodulesListPopup {
    ...
    pub fn update_submodules(&mut self) -> Result<()> {
        if self.is_visible() {
            ...
            self.submodule_parent = submodule_parent_info(&self.repo.borrow())?;
    ...
    fn get_text(&self, theme: &SharedTheme, width_available: u16, height: usize) -> Text {
        ...
        for (i, submodule) in self
            ...
            .enumerate()
        {
            let mut module_path = submodule.path.as_os_str().to_string_lossy().to_string();

            if module_path.len() > name_length {
                module_path.unicode_truncate(name_length.saturating_sub(THREE_DOTS_LENGTH));
            ...
            let span_hash = Span::styled(
                format!(
                    "{} ",
                    submodule.head_id.unwrap_or_default().get_short_string()
                ),
    ...
    fn get_info_text(&self, theme: &SharedTheme) -> Text {
        self.selected_entry()
            .map_or_else(Text::default, |submodule| {
                ...
                let span_path =
                    Span::styled(submodule.path.to_string_lossy(), theme.text(true, false));

                let span_title_commit = Span::styled("Commit:", theme.text(false, false));
                let span_commit = Span::styled(
                    submodule.id.unwrap_or_default().to_string(),
                    theme.commit_hash(false),
                );

                let span_title_url = Span::styled("Url:", theme.text(false, false));
                let span_url = Span::styled(
                    submodule.url.clone().unwrap_or_default(),
                    theme.text(true, false),
                );

                let span_title_status = Span::styled("Status:", theme.text(false, false));
                let span_status =
                    Span::styled(format!("{:?}", submodule.status), theme.text(true, false));
    ...
    fn get_local_info_text(&self, theme: &SharedTheme) -> Text {
        let mut spans = vec![
            ...
            Line::from(vec![Span::styled("Parent:", theme.text(false, false))]),
        ];
    ...
    fn draw_list(&self, f: &mut Frame, r: Rect) -> Result<()> {
        ...
        f.render_widget(
            Paragraph::new(self.get_text(&self.theme, r.width.saturating_add(1), height_in_lines))
    ...
    fn draw_info(&self, f: &mut Frame, r: Rect) {
        f.render_widget(
            Paragraph::new(self.get_info_text(&self.theme)).alignment(Alignment::Left),
            r,
        );
    }
