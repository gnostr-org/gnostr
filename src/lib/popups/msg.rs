...
impl DrawableComponent for MsgPopup {
    fn draw(&self, f: &mut Frame, _rect: Rect) -> Result<()> {
        ...
        let wrapped_msg =
            bwrap::wrap_maybrk!(&self.msg, area.width.saturating_sub(BORDER_WIDTH).into());
        ...
        f.render_widget(
            Paragraph::new(scrolled_lines)
                .block(
                    Block::default()
                        .title(Span::styled(self.title.as_str(), self.theme.text_danger()))
                        .borders(Borders::ALL)
...
impl MsgPopup {
    ...
    fn set_new_msg(&mut self, msg: &str, title: String) -> Result<()> {
        self.title = title;
