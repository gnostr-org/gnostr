...
use crate::{
    ...
    strings,
    tui::tui::{AsyncAppNotification, AsyncNotification, SyntaxHighlightProgress},
    ...
    fn draw(&self, f: &mut Frame, area: Rect) -> Result<()> {
        if self.is_visible() {
            ...
            let constraints = [
                ...
                Constraint::Length((self.get_line_number_width().saturating_add(1)).try_into()?),
            ...
            let syntax_highlight_progress = match self.blame {
                Some(BlameProcess::SyntaxHighlighting { ref job, .. }) => job
                    .progress()
                    .map(|p| format!(" ({}%)", p.progress))
                    .unwrap_or_default(),
                _ => String::new(),
            };
            let title_with_highlight_progress = format!("{title}{syntax_highlight_progress}");
                    ...
                    || key_match(key, self.key_config.keys.end)
                {
                    ...
                    if let Some(commit_id) = self.selected_commit() {
                        ...
                        self.queue.push(InternalEvent::OpenPopup(
                            StackablePopupOpen::InspectCommit(InspectCommitOpen::new(commit_id)),
                        ));
                    }
                } else if key_match(key, self.key_config.keys.file_history) {
                    if let Some(filepath) = self.params.as_ref().map(|p| p.file_path.clone()) {
                        self.hide_stacked(true);
                        self.queue
                            .push(InternalEvent::OpenPopup(StackablePopupOpen::FileRevlog(
...
impl BlameFilePopup {
    ...
    pub fn open(&mut self, open: BlameFileOpen) -> Result<()> {
        ...
        self.blame = Some(BlameProcess::GettingBlame(AsyncBlame::new(
    ...
    fn update_git(&mut self, event: AsyncGitNotification) -> Result<()> {
    ...
    fn update(&mut self) -> Result<()> {
        if self.is_visible() {
            if let Some(BlameProcess::GettingBlame(ref mut async_blame)) = self.blame {
                if let Some(params) = &self.params {
                    if let Some((previous_blame_params, last_file_blame)) = async_blame.last()? {
                        if previous_blame_params == *params {
                            self.blame = Some(BlameProcess::SyntaxHighlighting {
                                ...
                                job: AsyncSingleJob::new(self.app_sender.clone()),
                            });
                ...
                SyntaxHighlightProgress::Done => {
                    if let Some(job) = job.take_last() {
                        if let Some(syntax) = job.result() {
                            if syntax.path() == Path::new(unstyled_file_blame.path()) {
                                self.blame = Some(BlameProcess::Result(SyntaxFileBlame {
                                    file_blame: unstyled_file_blame.file_blame.clone(),
    ...
    fn highlight_blame_lines(&mut self) {
        ...
        job.spawn(AsyncSyntaxJob::new(text, params.file_path.clone()));
    }
    ...
    fn get_line_blame<'a>(
        ...
        let show_metadata = if line_number == 0 {
            ...
            let hunk_for_previous_line = &file_blame.lines()[line_number - 1];

            match (hunk_for_previous_line, hunk_for_line) {
                ((Some(previous), _), Some(current)) => previous.commit_id != current.commit_id,
                _ => true,
            }
        };
        ...
        let text_cell = styled_text.as_ref().map_or_else(
            || Cell::from(tabs_to_spaces(String::from(line))).style(self.theme.text(true, false)),
            |styled_text| {
                let styled_text = styled_text.lines[line_number].clone();
                Cell::from(styled_text)
            },
        );

        cells.push(
            Cell::from(format!("{line_number:>line_number_width$}{VERTICAL}",))
    ...
    fn get_metadata_for_line_blame(
        ...
        let time = blame_hunk.map_or_else(String::new, |hunk| time_to_string(hunk.time, true));

        let file_blame = self.blame.as_ref().and_then(|blame| blame.result());
        let is_blamed_commit = file_blame
            .and_then(|file_blame| blame_hunk.map(|hunk| file_blame.commit_id() == &hunk.commit_id))
            .unwrap_or(false);

        vec![
            Cell::from(commit_hash).style(self.theme.commit_hash_in_blame(is_blamed_commit)),
    ...
    fn move_selection(&mut self, scroll_type: ScrollType) -> bool {
        ...
        let new_selection = match scroll_type {
            ...
            ScrollType::PageUp => {
                old_selection.saturating_sub(self.current_height.get().saturating_sub(2))
            }
            ScrollType::PageDown => old_selection
                .saturating_add(self.current_height.get().saturating_sub(2))
                .min(max_selection),
        };
    ...
    fn set_open_selection(&mut self) {
        if let Some(selection) = self.open_request.as_ref().and_then(|req| req.selection) {
            let mut table_state = self.table_state.take();
...
fn get_author_width(width: usize) -> usize {
    (width.saturating_sub(19) / 3).clamp(MIN_AUTHOR_WIDTH, MAX_AUTHOR_WIDTH)
}
