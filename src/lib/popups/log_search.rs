...
impl LogSearchPopupPopup {
    ...
    pub fn new(env: &Environment) -> Self {
        ...
        Self {
            ...
            options: (SearchFields::default(), SearchOptions::default()),
    ...
    fn execute_confirm(&mut self) {
        ...
        match self.mode {
            PopupMode::Search => {
                self.queue
                    .push(InternalEvent::CommitSearch(LogFilterSearchOptions {
            ...
            PopupMode::JumpCommitSha => {
                ...
                self.queue
                    .push(InternalEvent::SelectCommitInRevlog(commit_id));
            }
        }
    }
    ...
    fn is_valid(&self) -> bool {
        match self.mode {
            PopupMode::Search => !self.find_text.get_text().trim().is_empty(),
    ...
    fn validate_commit_sha(&mut self) {
        ...
        if let Ok(commit_id) = CommitId::from_revision(&path, self.find_text.get_text().trim()) {
    ...
    fn get_text_options(&self) -> Vec<Line> {
        let x_summary = if self.options.0.contains(SearchFields::MESSAGE_SUMMARY) {
            "X"
        } else {
            " "
        };

        let x_body = if self.options.0.contains(SearchFields::MESSAGE_BODY) {
            "X"
        } else {
            " "
        };

        let x_files = if self.options.0.contains(SearchFields::FILENAMES) {
            "X"
        } else {
            " "
        };

        let x_authors = if self.options.0.contains(SearchFields::AUTHORS) {
            "X"
        } else {
            " "
        };

        let x_opt_fuzzy = if self.options.1.contains(SearchOptions::FUZZY_SEARCH) {
            "X"
        } else {
            " "
        };

        let x_opt_casesensitive = if self.options.1.contains(SearchOptions::CASE_SENSITIVE) {
            "X"
        } else {
            " "
        };
        ...
        vec![
            Line::from(vec![Span::styled(
                ...
                self.theme
                    .text(matches!(self.selection, Selection::FuzzyOption), false),
            )]),
            Line::from(vec![Span::styled(
                format!("[{x_opt_casesensitive}] case sensitive"),
                self.theme
                    .text(matches!(self.selection, Selection::CaseOption), false),
            )]),
            Line::from(vec![Span::styled(
                format!("[{x_summary}] summary",),
                self.theme
                    .text(matches!(self.selection, Selection::SummarySearch), false),
            )]),
            Line::from(vec![Span::styled(
                format!("[{x_body}] message body",),
                self.theme.text(
                    matches!(self.selection, Selection::MessageBodySearch),
                    false,
                ),
            )]),
            Line::from(vec![Span::styled(
                ...
                self.theme
                    .text(matches!(self.selection, Selection::FilenameSearch), false),
            )]),
            Line::from(vec![Span::styled(
                format!("[{x_authors}] authors",),
                self.theme
                    .text(matches!(self.selection, Selection::AuthorsSearch), false),
            )]),
        ]
    }
        ...
        match self.selection {
            ...
            Selection::SummarySearch => {
                ...
                if self.options.0.is_empty() {
                    self.options.0.set(SearchFields::MESSAGE_BODY, true);
                }
            }
            ...
            Selection::AuthorsSearch => {
                ...
                if self.options.0.is_empty() {
                    self.options.0.set(SearchFields::MESSAGE_SUMMARY, true);
                }
            }
        }
    }
    ...
    fn move_selection(&mut self, arg: bool) {
        if arg {
            ...
            self.selection = match self.selection {
                ...
                Selection::MessageBodySearch => Selection::SummarySearch,
                Selection::FilenameSearch => Selection::MessageBodySearch,
            ...
            self.selection = match self.selection {
                ...
                Selection::SummarySearch => Selection::MessageBodySearch,
                Selection::MessageBodySearch => Selection::FilenameSearch,
    ...
    fn draw_search_mode(&self, f: &mut Frame, area: Rect) -> Result<()> {
    ...
    fn draw_commit_sha_mode(&self, f: &mut Frame, area: Rect) -> Result<()> {
        const SIZE: (u16, u16) = (60, 3);
        let area = ui::centered_rect_absolute(SIZE.0, SIZE.1, area);

        let mut block_style = self.theme.title(true);

        let show_invalid = !self.is_valid() && !self.find_text.get_text().trim().is_empty();
    ...
    fn draw_invalid_sha(&self, f: &mut Frame) {
        ...
        let w = Paragraph::new(POPUP_COMMIT_SHA_INVALID).style(self.theme.text_danger());
    ...
    #[inline]
    fn event_search_mode(&mut self, event: &crossterm::event::Event) -> Result<EventState> {
        if let Event::Key(key) = &event {
            if key_match(key, self.key_config.keys.exit_popup) {
                self.hide();
            } else if key_match(key, self.key_config.keys.enter) && self.is_valid() {
                self.execute_confirm();
            } else if key_match(key, self.key_config.keys.popup_up) {
                self.move_selection(true);
            } else if key_match(key, self.key_config.keys.find_commit_sha) {
                self.set_mode(&PopupMode::JumpCommitSha);
            } else if key_match(key, self.key_config.keys.popup_down) {
                self.move_selection(false);
            } else if key_match(key, self.key_config.keys.log_mark_commit) && self.option_selected()
            {
                self.toggle_option();
    ...
    #[inline]
    fn event_commit_sha_mode(&mut self, event: &crossterm::event::Event) -> Result<EventState> {
        if let Event::Key(key) = &event {
            if key_match(key, self.key_config.keys.exit_popup) {
                self.set_mode(&PopupMode::Search);
            } else if key_match(key, self.key_config.keys.enter) && self.is_valid() {
                ...
                self.find_text
                    .enabled(!self.find_text.get_text().trim().is_empty());
            }
        }
...
impl Component for LogSearchPopupPopup {
    fn commands(&self, out: &mut Vec<CommandInfo>, force_all: bool) -> CommandBlocking {
        if self.is_visible() || force_all {
            ...
            if matches!(self.mode, PopupMode::Search) {
                ...
                out.push(
                    CommandInfo::new(
                        strings::commands::find_commit_sha(&self.key_config),
                        true,
                        true,
                    )
                    .order(1),
    ...
    fn event(&mut self, event: &crossterm::event::Event) -> Result<EventState> {
        if !self.is_visible() {
        ...
        match self.mode {
            ...
            PopupMode::JumpCommitSha => self.event_commit_sha_mode(event),
        }
    }
