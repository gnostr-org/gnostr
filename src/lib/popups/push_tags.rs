...
impl PushTagsPopup {
    ...
    pub fn new(env: &Environment) -> Self {
        Self {
            ...
            pending: false,
            visible: false,
            git_push: AsyncPushTags::new(env.repo.borrow().clone(), &env.sender_git),
            progress: None,
    ...
    fn push_to_remote(&mut self, cred: Option<BasicAuthCredential>) -> Result<()> {
        self.pending = true;
        self.progress = None;
    ...
    fn progress_state_name(progress: &PushTagsProgress) -> String {
        match progress {
            PushTagsProgress::CheckRemote => strings::PUSH_TAGS_STATES_FETCHING,
            PushTagsProgress::Push { .. } => strings::PUSH_TAGS_STATES_PUSHING,
            ...
            if let Event::Key(e) = ev {
                if self.input_cred.is_visible() {
                    ...
                    if self.input_cred.get_cred().is_complete() || !self.input_cred.is_visible() {
                        self.push_to_remote(Some(self.input_cred.get_cred().clone()))?;
