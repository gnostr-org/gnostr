...
const fn type_to_string(kind: ResetType) -> (&'static str, &'static str) {
    const RESET_TYPE_DESC_SOFT: &str = "  🟢 Keep all changes. Stage differences";
    const RESET_TYPE_DESC_MIXED: &str = " 🟡 Keep all changes. Unstage differences";
    const RESET_TYPE_DESC_HARD: &str = "  🔴 Discard all local changes";
...
impl ResetPopup {
    ...
    fn get_text(&self, _width: u16) -> Vec<Line> {
        ...
        txt.push(Line::from(vec![
            Span::styled(String::from("Branch: "), self.theme.text(true, false)),
            Span::styled(
        ...
        txt.push(Line::from(vec![
            Span::styled(String::from("Reset to: "), self.theme.text(true, false)),
            Span::styled(
                self.commit.map(|c| c.to_string()).unwrap_or_default(),
        ...
        txt.push(Line::from(vec![
            Span::styled(String::from("How: "), self.theme.text(true, false)),
...
impl Component for ResetPopup {
    fn commands(&self, out: &mut Vec<CommandInfo>, force_all: bool) -> CommandBlocking {
        if self.is_visible() || force_all {
            ...
            out.push(
                CommandInfo::new(strings::commands::reset_type(&self.key_config), true, true)
                    .order(1),
            );
        }
