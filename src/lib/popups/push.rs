...
impl PushPopup {
    ...
    pub fn new(env: &Environment) -> Self {
        Self {
            ...
            git_push: AsyncPush::new(env.repo.borrow().clone(), &env.sender_git),
            progress: None,
    ...
    pub fn push(
        ...
        if need_username_password_for_push(&self.repo.borrow())? {
            let cred = extract_username_password_for_push(&self.repo.borrow())
    ...
    fn push_to_remote(&mut self, cred: Option<BasicAuthCredential>, force: bool) -> Result<()> {
        let remote = if let Ok(Some(remote)) = get_branch_remote(&self.repo.borrow(), &self.branch)
        {
            log::info!(
            ...
            let remote = get_default_remote_for_push(&self.repo.borrow())?;
            log::info!("push: branch '{}' to remote '{}'", self.branch, remote);
            remote
        };
    ...
    fn update(&mut self) -> Result<()> {
        ...
        if !self.pending {
            if let Some(err) = self.git_push.last_result()? {
                self.queue
                    .push(InternalEvent::ShowErrorMsg(format!("push failed:\n{err}")));
            }
            self.hide();
        }
    ...
    fn progress_state_name(state: &RemoteProgressState) -> String {
        match state {
            RemoteProgressState::PackingAddingObject => strings::PUSH_POPUP_STATES_ADDING,
            RemoteProgressState::PackingDeltafiction => strings::PUSH_POPUP_STATES_DELTAS,
            RemoteProgressState::Pushing => strings::PUSH_POPUP_STATES_PUSHING,
            RemoteProgressState::Transfer => strings::PUSH_POPUP_STATES_TRANSFER,
            RemoteProgressState::Done => strings::PUSH_POPUP_STATES_DONE,
        }
        .into()
    }
}
        ...
        if self.visible {
            if let Event::Key(e) = ev {
                if self.input_cred.is_visible() {
                    ...
                    if self.input_cred.get_cred().is_complete() || !self.input_cred.is_visible() {
                        self.push_to_remote(
                ...
                } else if key_match(e, self.key_config.keys.exit_popup) && !self.pending {
                    self.hide();
                }
            }
