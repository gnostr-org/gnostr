use anyhow::Result;
use asyncgit::sync::{self, get_config_string, CommitId, RepoPathRef};
...
impl Component for TagCommitPopup {
    fn commands(&self, out: &mut Vec<CommandInfo>, force_all: bool) -> CommandBlocking {
        if self.is_visible() || force_all {
            ...
            let is_annotation_mode = matches!(self.mode, Mode::Annotation { .. });

            out.push(CommandInfo::new(
                strings::commands::tag_commit_confirm_msg(&self.key_config, is_annotation_mode),
                self.is_valid_tag(),
                true,
            ));
    ...
    fn event(&mut self, ev: &Event) -> Result<EventState> {
        if self.is_visible() {
            if let Event::Key(e) = ev {
                ...
                if is_annotation_mode && key_match(e, self.key_config.keys.commit) {
                    try_or_popup!(self, "tag error:", self.tag());
                    return Ok(EventState::Consumed);
                } else if key_match(e, self.key_config.keys.tag_annotate) && self.is_valid_tag() {
...
impl TagCommitPopup {
    ...
    fn tag_info(&self) -> (String, Option<String>) {
        match &self.mode {
            ...
            Mode::Annotation { tag_name } => (tag_name.clone(), Some(self.input.get_text().into())),
        }
    }

    pub fn tag(&mut self) -> Result<()> {
        let gpgsign = get_config_string(&self.repo.borrow(), "tag.gpgsign")
            .ok()
            .flatten()
        ...
        if let Some(commit_id) = self.commit_id {
            ...
            match result {
                ...
                Err(e) => {
                    ...
                    log::error!("e: {}", e,);
                    self.queue
                        .push(InternalEvent::ShowErrorMsg(format!("tag error:\n{e}",)));
                }
            }
        }
    ...
    fn start_annotate_mode(&mut self) {
        ...
        self.input
            .set_title(strings::tag_popup_annotation_title(&tag_name));
        self.input
