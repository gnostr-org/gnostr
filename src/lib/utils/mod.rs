...
use gnostr_types::{Client, Filter, Id, Keys, Nip19, nip19::Nip19Profile, Options, PrivateKey, PublicKey};
...
pub async fn parse_private_key(
    ...
    print_keys: bool,
) -> Result<gnostr_types::Keys, AnyhowError> {
    // Parse and validate private key
    let keys = match private_key {
        Some(pk) => {
            if pk.starts_with("nsec") {
                let pk_obj = gnostr_types::PrivateKey::try_from_bech32_string(&pk)?;
                gnostr_types::Keys::new(pk_obj)
            } else {
                // We assume it's a hex formatted private key
                let pk_obj = gnostr_types::PrivateKey::try_from_hex_string(&pk)?;
                gnostr_types::Keys::new(pk_obj)
            }
        }
        None => {
...
pub async fn create_client(
    keys: &gnostr_types::Keys,
    relays: Vec<String>,
    difficulty: u8,
) -> Result<gnostr_types::Client, AnyhowError> {
    let opts = gnostr_types::Options::new()
        .send_timeout(Some(Duration::from_secs(15)))
        .wait_for_send(true)
        .difficulty(difficulty);
    let mut client = gnostr_types::Client::new(keys, opts);
...
pub async fn parse_key_or_id_to_hex_string(input: String) -> Result<String, AnyhowError> {
    let hex_key_or_id = if input.starts_with("npub") {
        gnostr_types::PublicKey::try_from_bech32_string(&input, true)?.as_hex_string()
    } else if input.starts_with("nsec") {
        gnostr_types::PrivateKey::try_from_bech32_string(&input)?.as_hex_string()
    } else if input.starts_with("note") {
        gnostr_types::Id::try_from_bech32_string(&input)?.as_hex_string()
    } else if input.starts_with("nprofile") {
        if let gnostr_types::Nip19::Profile(profile) = gnostr_types::Nip19::decode(&input)? {
...
pub fn generate_nostr_keys_from_commit_hash(
    ...
    let private_key = gnostr_types::PrivateKey::try_from_hex_string(&padded_commit_id);
    let keys = gnostr_types::Keys::new(private_key?);
    Ok(keys)
}
