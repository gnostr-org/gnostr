...
pub fn decrypt_key(
	...
	let encrypted_key =
		nostr::nips::nip49::EncryptedSecretKey::from_bech32(
			encrypted_key,
		)?;
...
mod tests {
	...
	fn decrypts_with_strong_password_from_reference_string()
	-> Result<()> {
		let decrypted_key =
	...
	#[test]
	// ensures password encryption hasn't changed
	fn decrypts_with_weak_password_from_reference_string()
	-> Result<()> {
		let decrypted_key = decrypt_key(
			TEST_KEY_1_ENCRYPTED_WEAK,
			TEST_WEAK_PASSWORD,
		)?;
	...
	fn decrypts_key_encrypted_using_encrypt_key() -> Result<()> {
		...
		assert_eq!(
			format!(
				"{}",
				key.secret_key().unwrap().to_bech32().unwrap()
			),
			format!(
				"{}",
				newkey.secret_key().unwrap().to_bech32().unwrap()
			),
		);
		Ok(())
	}
	...
	#[test]
	fn decrypt_key_successfully_decrypts_key_encrypted_using_encrypt_key()
	-> Result<()> {
		let key = nostr::Keys::generate();
		let s = encrypt_key(&key, TEST_PASSWORD)?;
		let newkey = decrypt_key(s.as_str(), TEST_PASSWORD)?;
