...
pub fn identify_ahead_behind(
	...
	match git_repo.get_commits_ahead_behind(&to_tip, &from_tip) {
		...
		Ok((ahead, behind)) => {
			Ok((from_branch, to_branch, ahead, behind))
		}
	}
}
...
mod tests {
	...
	#[test]
	fn when_to_branch_is_none_and_no_main_or_master_branch_return_error()
	-> Result<()> {
	...
	#[test]
	fn when_from_branch_is_not_head_return_as_from_branch()
	-> Result<()> {
		let test_repo = GitTestRepo::default();
		let git_repo = Repo::from_path(&test_repo.dir)?;
