...
impl FileTreeItems {
    ///
    pub fn new(list: &[&Path], collapsed: &BTreeSet<&String>) -> Result<Self> {
    ...
    fn create_items<'a>(
        ...
        let mut paths_added: HashMap<&Path, usize> = HashMap::with_capacity(list.len());

        for e in list {
            {
                Self::push_dirs(e, &mut items, &mut paths_added, collapsed)?;
            }
    ...
    fn push_dirs<'a>(
        ...
        let mut ancestors = item_path.ancestors().skip(1).collect::<Vec<_>>();
        ancestors.reverse();
    ...
    fn path_to_string(p: &Path) -> Result<String> {
        Ok(p.to_str()
            .map_or_else(|| Err(Error::InvalidPath(p.to_path_buf())), Ok)?
            .to_string())
    }
    ...
    pub fn collapse(&mut self, index: usize, recursive: bool) {
        if self.tree_items[index].kind().is_path() {
            ...
            let path = PathBuf::from(self.tree_items[index].info().full_path_str());
            ...
            for i in index + 1..self.tree_items.len() {
                ...
                let item_path = Path::new(item.info().full_path_str());
    ...
    pub fn expand(&mut self, index: usize, recursive: bool) {
        if self.tree_items[index].kind().is_path() {
            ...
            let full_path = PathBuf::from(self.tree_items[index].info().full_path_str());

            if recursive {
                for i in index + 1..self.tree_items.len() {
                    let item = &mut self.tree_items[i];

                    if !Path::new(item.info().full_path_str()).starts_with(&full_path) {
                        break;
                    }
            ...
            self.update_visibility(Some(full_path).as_ref(), index + 1, false);
        }
    }
    ...
    pub fn show_element(&mut self, index: usize) -> Option<usize> {
        Some(self.show_element_upward(index)? + self.show_element_downward(index)?)
    }
    ...
    fn show_element_downward(&mut self, index: usize) -> Option<usize> {
        let mut shown = 0_usize;
    ...
    fn update_visibility(
        ...
        for i in start_idx..self.tree_items.len() {
            if let Some(ref collapsed_path) = inner_collapsed {
                let p = Path::new(self.tree_items[i].info().full_path_str());
            ...
            let item_path = Path::new(self.tree_items[i].info().full_path_str());
    ...
    fn fold_paths(items: &mut Vec<FileTreeItem>, paths: &HashMap<&Path, usize>) {
        let mut i = 0;

        while i < items.len() {
            let item = &items[i];
            if item.kind().is_path() {
                let children = paths.get(&Path::new(item.info().full_path_str()));
                ...
                if let Some(children) = children {
                    if *children == 1 {
                        ...
                        if items.get(i + 1).is_some_and(|item| item.kind().is_path()) {
                            let next_item = items.remove(i + 1);
                            let item_mut = &mut items[i];
                            item_mut.fold(next_item);

                            let prefix = item_mut.info().full_path_str().to_owned();
    ...
    fn unindent(items: &mut [FileTreeItem], prefix: &str, start: usize) {
...
mod tests {
    ...
    fn test_simple() {
        ...
        let res = FileTreeItems::new(&items, &BTreeSet::new()).unwrap();
    ...
    fn test_push_path() {
        ...
        FileTreeItems::push_dirs(Path::new("a/b/c"), &mut items, &mut paths, &BTreeSet::new())
            .unwrap();
    ...
    fn test_push_path2() {
        ...
        FileTreeItems::push_dirs(Path::new("a/b/d"), &mut items, &mut paths, &BTreeSet::new())
            .unwrap();
    ...
    fn test_folder() {
        ...
        assert_eq!(res, vec![String::from("a"), String::from("a/file.txt"),]);
    }
    ...
    fn test_indent() {
        ...
        let list = FileTreeItems::new(&items, &BTreeSet::new()).unwrap();
        let mut res = list
            .tree_items
            .iter()
    ...
    fn test_folder_dup() {
        ...
        let tree = FileTreeItems::new(&items, &BTreeSet::new()).unwrap();
    ...
    fn test_collapse() {
        ...
        let mut tree = FileTreeItems::new(&items, &BTreeSet::new()).unwrap();
...
mod test_merging {
    ...
    fn test_merge_simple() {
        ...
        let (mut items, paths) = FileTreeItems::create_items(&list, &BTreeSet::new()).unwrap();
    ...
    fn test_merge_indent() {
        ...
        let indents: Vec<u8> = items.iter().map(|i| i.info().indent()).collect();
