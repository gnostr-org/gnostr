gnostr-verify-keypair:## 	with install sequence
	[ -x "$(shell which gnostr)" ] || $(MAKE) gnostr
	[ -x "$(shell which gnostr-sha256)" ] || $(MAKE) gnostr-sha256 || [ -x "$(shell which cargo)" ] && cargo install  -q gnostr-sha256
	[ -x "$(shell which gnostr-weeble)" ] || $(MAKE) gnostr-install
##CASE 1
	echo CASE 1
